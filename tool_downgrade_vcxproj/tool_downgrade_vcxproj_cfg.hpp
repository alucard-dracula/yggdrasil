// tool_downgrade_vcxproj_cfg.hpp
...
struct tool_downgrade_vcxproj_cfg_sample {};

struct tool_downgrade_vcxproj_cfg
{
public:
...
private:
	typedef tool_downgrade_vcxproj_cfg this_type;
	BOOST_COPYABLE_AND_MOVABLE(this_type);

public:
	tool_downgrade_vcxproj_cfg(void);
	tool_downgrade_vcxproj_cfg(tool_downgrade_vcxproj_cfg_sample);

	tool_downgrade_vcxproj_cfg(BOOST_RV_REF(this_type) right)
		: root_dir_(boost::move(right.root_dir_)),
			lang_tag_(boost::move(right.lang_tag_)),
			dir_filter_(boost::move(right.dir_filter_)),
			file_filter_(boost::move(right.file_filter_))
	{
	}

	tool_downgrade_vcxproj_cfg(const this_type& right);
	~tool_downgrade_vcxproj_cfg(void);
...
public:
	inline this_type& operator=(BOOST_RV_REF(this_type) right)
	{
		...
		yggr::copy_or_move_or_swap(lang_tag_, boost::move(right_ref.lang_tag_));
...
private:
	...
	void serialize(Archive& ar, const yggr::u32 version)
	{
		...
		ar & YGGR_SERIALIZATION_NVP(lang_tag_);
...
{
	YGGR_PP_SWAP_THIS_REF_DEF(tool_downgrade_vcxproj_cfg)
} // namespace swap_support
