...
#ifndef __CLT_TEST_STATIC_TASK_CENTER_BLANCE_CALCULATOR_HPP__
#define __CLT_TEST_STATIC_TASK_CENTER_BLANCE_CALCULATOR_HPP__
...
public:
	...
	void register_dispatchers(CtrlCenter& cc) 
	{
		...
		// this is fake fixer
		CTRL_HANDLER(Tag, cc, boost::asio::error::not_connected, // 10057
		...
		CTRL_HANDLER(Tag, cc, boost::asio::error::connection_refused, // boost::asio::error::connection_refused
						yggr::system_controller::system_code::E_TCP_BASE_NETWORK_SYSTEM,
						boost::bind(&this_type::cal_network_dispath,
										this_type::shared_from_this(), _1));
	...
	void cal_test_pak_type(const owner_info_type& owner, const test_pak_type& cdt, Runner* prunner, const Handler& handler)
	{
		...
		if(now_count < send_count())
		{
			...
			{
#	if 0
				clt_sender_type::send_packet(cdt.id(), *i, cdt,
...
#endif // __CLT_TEST_STATIC_TASK_CENTER_BLANCE_CALCULATOR_HPP__