...
inline ptree_string_type format_cfg_mark(const ptree_string_type& str_cfg_mark)
{
	...
	if(str_cfg_mark == "Debug-Win32")
	{
		return cb_cfg_marks::s_mark_debug_x32();
	}
	else if(str_cfg_mark == "Release-Win32")
	{
		return cb_cfg_marks::s_mark_release_x32();
	}
	else
...
inline ptree_string_type vc_outdir_to_cb_outdir_dll(const ptree_string_type& outdir)
{
	static const ptree_string_type s_str_slndir_vc = "$(SolutionDir)";
	static const ptree_string_type s_str_slndir_cb = "$(WORKSPACE_DIR)/";
	...
	static const ptree_string_type s_str_cfg_bin_dir_vc = "/bin";
	static const ptree_string_type s_str_cfg_lib_dir_cb = "/lib";
	...
	find_and_replace(tmp, s_str_tool_ver_vc, s_str_tool_ver_cb);
	find_and_replace(tmp, s_str_cfg_mark_vc, s_str_cfg_mark_cb);
	find_and_replace(tmp, s_str_cfg_bin_dir_vc, s_str_cfg_lib_dir_cb);
...
ptree_string_type gen_output_dll(const ptree_string_type& outdir_vc,
									const ptree_string_type& target_name_vc)
{
	ptree_string_type outdir_cb = vc_outdir_to_cb_outdir_dll(outdir_vc);
	ptree_string_type target_name_cb = vc_target_name_to_cb_target_name(target_name_vc);
											...
											const ptree_string_type& cfg_mark)
{
	...
	{
		return gen_output_dll(outdir_vc, target_name_vc);
	}
}
...
inline const ptree_string_type& gen_target_opt_compiler(const ptree_string_type& cfg_mark)
{
	...
	//static const ptree_string_type s_str_x32 = "gcc";
													...
													const string_set_type& inc_dirs)
{
	...
	if(is_debug)
	{
		...
		// if(is_yggr_proj) //boost/regex/v4/icu.hpp has unused-variable
		// {
...
#define ERASE_PP( __set__ ) \
	...
	__set__.erase("WIN32"); \
	__set__.erase("_WIN32"); \
	__set__.erase("WIN64"); \
	__set__.erase("_WIN64"); \
	__set__.erase("_LIB"); \
	__set__.erase("_USRDLL"); \
	__set__.erase("_CONSOLE"); \
	__set__.erase("_WINDOWS"); \
	__set__.erase("LUA_BUILD_AS_DLL"); \
	__set__.erase("BOOST_WINDOWS");
...
void format_opt_items_pp_root_namespace(const ptree_type& vc_ptree,
											string_set_type& glo_set,
											string_set_type& d32_set, string_set_type& r32_set,
											string_set_type& d64_set, string_set_type& r64_set)
{
	...
	ptree_string_type str_root_ns = vcproj::get_root_namespace(vc_ptree);

	if(str_set.find(str_root_ns) != str_set.end())
	{
		glo_set.insert("LUA_USE_LINUX");
	}
}
					...
					string_set_type& pp_d64, string_set_type& pp_r64)
{
			...
			|| (vc_app_tag == vcproj::vcproj_app_tags::s_tag_makefile())))
	{
		...
		format_opt_items_pp_root_namespace(vc_ptree, pp, pp_d32, pp_r32, pp_d64, pp_r64);
	}
}
								...
								const string_set_type& inc_dirs)
{
...

	{
		...
		if(vc_app_tag == vcproj::vcproj_app_tags::s_tag_static_lib())
		{
			cb_tree.add_child("Add", ptree_type()).add_child("<xmlattr>.option", ptree_type("-fPIC"));
		}
...
vcproj::code_file_set_type& format_file_list(vcproj::code_file_set_type& file_list, const yggr::utf8_string& arg_proj_dir)
{
	...
	if(proj_dir.empty())
	{
		...
		{
			ptree_string_type tmp_file_path = (*i);
			tmp.insert(find_and_replace(tmp_file_path, "\\", "/"));
		}
	...
	else
	{
		...
		{
			ptree_string_type tmp_file_path = (*i);
			tmp.insert(proj_dir.org_str() + "/" + find_and_replace(tmp_file_path, "\\", "/"));
		}
	}
...
bool need_add_dir_charset_utf8_files(const ptree_string_type& proj_root, const ptree_string_type& str_path)
{
	...
	{
		return
			(4 < str_fname.size()) && (str_fname.substr(str_fname.size() - 4, 4) == ".cpp")
