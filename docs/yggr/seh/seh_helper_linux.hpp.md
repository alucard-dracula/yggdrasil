# `seh_helper_linux.hpp`

## 概述

该头文件定义了 `yggr::seh::seh_helper_linux`，用于 Linux 平台的调用栈采集和格式化。

类本身只声明静态辅助接口，具体实现位于 `seh_helper_linux.ipp`。
...
当前 Linux 实现会按条目逐行输出，并使用递增数量的制表符表示栈帧层级。具体输出格式以
`seh_helper_linux.ipp` 为准。
...
返回值表示采集是否成功。当前实现最多采集 1024 个栈帧，使用 Linux 提供的
...
{
    yggr::seh::seh_helper_linux::dump_call_stack_type call_stack;

    if(yggr::seh::seh_helper_linux::dump_call_stack(call_stack))
    {
        const std::string message =
            yggr::seh::seh_helper_linux::format_dump_call_stack_msg(
                call_stack);
        (void)message;
    }
}
...
`seh_env_linux::handler_recv_signal()` 使用该类生成异常诊断信息：

1. 调用 `dump_call_stack()` 获取调用栈；
2. 调用 `format_dump_call_stack_msg()` 转换为文本；
3. 将调用栈文本与 Linux 信号代码组合到异常消息中。
...
5. 符号解析结果依赖 Linux 运行库、构建符号和当前执行环境，可能不完整。
6. 采集失败时应检查返回值，不应假定 `call_stack` 一定包含有效栈帧。
7. 调用栈采集通常发生在异常处理路径，使用时应评估信号处理上下文中的安全性和性能影响。
8. 具体输出格式和资源释放行为以 `seh_helper_linux.ipp` 的实现为准。

## 相关组件

- `yggr/seh/seh.hpp`
- `yggr/seh/seh_env_linux.hpp`
- `yggr/seh/ipps/seh_helper_linux.ipp`
