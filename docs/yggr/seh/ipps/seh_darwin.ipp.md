...
该实现文件提供 `yggr::seh::seh_darwin` 的 Darwin 平台实现，包含：

- 构造函数和析构函数；
- 异常/信号代码注册；
- 异常/信号代码注销。

文件通过标准 `signal()` 接口，将指定信号交由 Darwin 异常环境的处理函数接收。
...
通常应通过 `seh_darwin::safe_invoke()` 配合 Darwin 异常环境使用，具体调用方式请参考对应头文件文档。
...
1. 接收调用者提供的信号代码；
2. 调用标准 `signal()`；
3. 将 `seh_env_darwin::handler_recv_signal` 设置为处理函数；
4. 返回 `true`。
...
2. `code` 必须是 Darwin 平台支持的有效信号代码；传入无效代码时，平台行为应以 C/C++ 运行库为准。
...
7. 该文件依赖 `seh_darwin.hpp`、`seh_env_darwin` 和平台提供的 `signal()`/`SIG_DFL` 定义。
