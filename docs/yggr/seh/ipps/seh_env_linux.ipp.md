...
该实现文件提供 `yggr::seh::seh_env_linux` 的 Linux 平台静态成员定义和信号处理实现。
...
```cpp
#include <yggr/seh/seh_env_linux.hpp>
```
...
```cpp
seh_env_linux::mutex_type seh_env_linux::_jmp_buf_mutex;
```
...
```cpp
seh_env_linux::mutex_type seh_env_linux::_signal_mutex;
```
...
```cpp
seh_env_linux::mutex_type seh_env_linux::_call_stack_mutex;
```
...
```cpp
seh_env_linux::jmp_buf_map_type seh_env_linux::_jmp_buf_map;
```
...
```cpp
seh_env_linux::now_signal_map_type seh_env_linux::_now_sig_map;
```
...
该函数是 Linux 信号处理入口，由 `seh_linux::register_code()` 通过标准 `signal()` 注册。
...
调用栈由 `seh_helper_linux::dump_call_stack()` 采集，并由
`seh_helper_linux::format_dump_call_stack_msg()` 格式化。

## 与安全调用的关系

该实现依赖 `seh_env_linux` 头文件中定义的安全调用流程：

- `seh_linux::safe_invoke()` 设置当前线程的跳转环境；
...
| 组件 | 用途 |
| --- | --- |
| `seh_env_linux` | 保存异常环境的静态状态和跳转映射。 |
| `seh_helper_linux` | 采集和格式化调用栈。 |
...
2. 信号处理函数中执行调用栈采集、字符串流操作和异常抛出，是否满足 Linux 运行环境的异步信号安全要求，取决于项目运行时设计。
3. `_jmp_buf_map`、`_now_sig_map` 和互斥量是进程级静态状态，不同 `seh_env_linux` 实例之间共享。
4. `siglongjmp` 要求对应线程的跳转缓冲区已经正确建立；否则可能导致未定义行为。
5. 信号处理函数通过线程 ID选择跳转缓冲区，因此注册和触发应发生在一致的线程上下文中。
6. 该文件只提供静态成员定义和信号处理函数实现，安全调用的其他流程位于 `seh_env_linux.hpp` 或相关实现文件中。
