...
该头文件定义了 `yggr::seh::seh_darwin`，用于在 Darwin 平台封装安全调用和信号异常处理。

该类将用户处理器转发给 `seh_env_darwin`，并自动绑定异常过滤器、返回值清理器和默认修复器。
...
typedef seh_env_darwin::seh_sys_codes_type seh_sys_codes_type;
```

- `code_type`：异常或信号代码类型，实际为 `u32`；
- `seh_sys_codes_type`：由 Darwin 异常环境提供的系统异常代码集合类型。
...
构造和析构函数用于初始化及释放 Darwin 异常处理对象相关状态，具体实现位于
`seh_darwin.ipp`。
...
发生异常或调用失败时，返回值清理器会生成值初始化后的 `Ret()`。
...
两个重载的返回值均表示底层 `seh_env_darwin::safe_invoke()` 的执行结果。
...
注册指定的 Darwin 信号代码。具体实现通过标准 `signal()` 将信号转发至
`seh_env_darwin::handler_recv_signal`。
...
注销指定的 Darwin 信号代码，具体实现通过 `signal()` 将处理函数恢复为
`SIG_DFL`。
...
void invoke_darwin_safely()
{
    yggr::seh::seh_darwin seh;
    int result = 0;
...
```cpp
void configure_signal_code(yggr::seh::seh_darwin& seh)
{
    const yggr::seh::seh_darwin::code_type code = 1;
...
`seh_darwin` 与平台实现的关系如下：

1. `register_code()` 将信号绑定到 `seh_env_darwin::handler_recv_signal()`；
2. `safe_invoke()` 将处理器和内部回调传入 `seh_env_darwin`；
3. 信号处理器采集调用栈并记录信号代码；
4. `seh_env_darwin` 根据跳转结果执行过滤、清理和修复逻辑。

## 注意事项

1. 该类是 Darwin 平台专用实现。
2. 使用前应包含 `seh.hpp`，以确保 `YGGR_SEH_INCLUDE` 和相关环境配置已建立。
3. `safe_invoke()` 的实际异常捕获和跳转能力由 `seh_env_darwin` 决定。
...
8. `register_code()` 和 `unregister_code()` 的当前实现无条件返回 `true`，不检查底层 `signal()` 的返回值。
9. 构造函数、析构函数及信号代码管理函数的具体实现需链接对应的 Darwin 实现文件。
