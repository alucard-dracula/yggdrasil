...
const char* help_str = "command: \r\n"
						...
						"\t\tcfg_key_unpaked -- the key is paked state\r\n"
						"\t\tcfg_key_real    -- this key is unpaked state\r\n\r\n"
...
std::pair<yggr::u32, yggr::u16> get_id_ver_of_buf(const std::string& key, const std::string& nbuf)
{
	if(key.empty())
	{
		...
		return std::pair<yggr::u32, yggr::u16>(0xeeeeeee0, 0xeee0);
	}
...
std::string parse_wpe_file(const std::string& key, const std::string& fname)
{
	...
	std::string time_path(tm.to_local_string("%Y-%m-%d-%H-%M-%S"));
	std::string full_path(std::string(value_dir) + std::string("/") + time_path);
	...
	return std::string(open_value_dir) + std::string("\\") + time_path; 
	//return full_file;
}
...
void parse_val(void)
{
	...
	if(cfg.val_cfg == cfg_val_wpe)
	{
		...
		system((std::string("explorer ") + open_file).c_str());
		return;
	}
}
