...
void test_parser_mgr(void)
{
	...
	error_type err = mgr.run_command<error_type>(string_type("xy"), 10, string_type("item"), string_type("update name=大血瓶 add_count = 1"));
...
void user_info_mgr_test(void)
{
	...
	user_mgr.add_user(string_type("xy2"), string_type("你好"));
	std::cout << user_mgr.cmp_passwd(string_type("xy"), string_type("123456")) << std::endl;
	std::cout << user_mgr.cmp_passwd(string_type("xy2"), string_type("你好")) << std::endl;
	std::cout << user_mgr.cmp_passwd(string_type("xy"), string_type("你好")) << std::endl;
...
void test_lua_system_gm(void)
{
	...
	boost::shared_ptr<system_gm_type> sys_gm_ptr(new system_gm_type());
...
typedef boost::shared_ptr<sys_gm_cal_type> sys_gm_cal_ptr_type;
