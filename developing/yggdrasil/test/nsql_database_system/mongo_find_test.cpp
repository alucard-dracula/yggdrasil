...
struct sort_test
{
	...
	void serialize(Archive & ar, yggr::u32 version)
	{
		ar & YGGR_SERIALIZE_NAME_NVP("s8", _sort);
	}
...
typedef yggr::time::runtime_speed_test<yggr::time::time> runtime_speed_test_type;
...
void test(void)
{
	...
	accesser.remove_all(std::string("test"), std::string("foo2"));
	...
	{
		runtime_speed_test_type tms_test;
		//accesser.find(saver, std::string("test"), std::string("foo2"), b1, b2);

		//"db.foo2.find().sort({"s8":-1}).limit(1)"
		accesser.find(saver, std::string("test"), std::string("foo2"), b1, b2, 1, 1, 0);
	}

	{
		std::cout << vt.size() << std::endl;
		for(yggr::s32 i = 0, isize = vt.size(); i != isize; ++i)
		{
