...
struct type_test
{
	...
	{
		ar & YGGR_SERIALIZE_NVP(_s8); //这里的序列化不能读取 检查正确性请去数据库中查看
		...
		ar & YGGR_SERIALIZE_NVP(ivt); //注意这个数组的序列化 这个是不带长度的 里面内容换成c_bson 就可以成为 or的条件
...
int main(int argc, char* argv[])
{
	...
	std::cout << "end result please see mongodb test.foo" << std::endl;
