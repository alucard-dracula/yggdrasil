...
void check_last_element_of_cont(set_type::base_type& cont, int check)
{
	//assert(false); // check called
	assert(cont.size() == check);
}
...
void test_element_lookup(void)
{
	...
	assert(cont.count(1) == 2);
	assert(cont.count(2) == 2);
	assert(cont.count(3) == 2);
	assert(cont.count(4) == 0);
	assert(cont.count(5) == 4);
...
void test_modifiers(void)
{
	...
	cont.insert(arr, arr + 10);
	base_cont.insert(arr, arr + 10);

	cont.insert(arr, arr + 10, boost::bind(&check_last_element_of_cont, _1, cont.size() + 10));
	base_cont.insert(arr, arr + 10);
...
void test_thread_safe_of_mutex(void)
{
	...
	typedef yggr::safe_container::safe_unordered_multiset<val_type> container_type;
...
{
	typedef boost::shared_ptr<int> val_type;
	typedef yggr::safe_container::safe_unordered_multiset<val_type, boost::shared_mutex> container_type;
