...
#if USE_STL_MAP
#	include <map>
	typedef yggr::safe_container::safe_map<int, int,
                                            boost::mutex,
                                            std::less<int>,
...
void test_element_access(void)
{
	...
	assert(cont.set_value(5, 100));
	assert(cont.set_value(200, 200));
...
void test_modifiers(void)
{
	...
	cont.set_value(map_type::value_type(200, 200));
	cont.set_value(map_type::value_type(9000, 9000));
	base_cont.insert(map_type::value_type(9000, 9000));
	...
	assert(!cont.replace(200, 500));
	assert(cont.replace(100, 1000));
	assert(cont.replace(map_type::value_type(100, 100)));
...
void test_thread_safe_of_mutex(void)
{
	...
	typedef yggr::safe_container::safe_map<int, val_type> container_type;
...
{
	typedef boost::shared_ptr<int> val_type;
	typedef yggr::safe_container::safe_map<int, val_type, boost::shared_mutex> container_type;
