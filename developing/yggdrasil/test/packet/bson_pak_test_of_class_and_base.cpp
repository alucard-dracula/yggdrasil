...
private :
	...
	void serialize(Archive & ar, yggr::u32 version)
    {
		...
		ar & YGGR_SERIALIZE_NVP(_null);
		ar & YGGR_SERIALIZE_NVP(_undefined);

		ar & YGGR_SERIALIZE_NVP(_regex);
		ar & YGGR_SERIALIZE_NVP(_code);
		ar & YGGR_SERIALIZE_NVP(_symbol);
		ar & YGGR_SERIALIZE_NVP(_wscope);

		ar & YGGR_SERIALIZE_NVP(_timestamp);


        yggr::nsql_database_system::c_bson_u8_binary_buffer bin_buffer(_buf);
        ar & YGGR_SERIALIZE_NVP(bin_buffer);
        yggr::nsql_database_system::c_bson_bpak_binary_buffer bin_pak(_pak_buf);
        ar & YGGR_SERIALIZE_NVP(bin_pak);
...
private:
	...
	void serialize(Archive & ar, yggr::u32 version)
	{
		...
		ar & YGGR_SERIALIZE_NVP(b);
		ar & YGGR_SERIALIZE_NVP(c);
...
std::ostream& operator<<(std::ostream& os, const type_test& val)
{
	...
	os << "regex = " << val._regex.regex() << " " << val._regex.text() << "\n";
...
void test1_1(void)
{
	...
	opak.save(YGGR_SERIALIZE_NAME_NVP("test_ttt", test));
	...
	ipak.load(YGGR_SERIALIZE_NAME_NVP("test_ttt", test2));
...
void test2(void)
{
	...
	t1.b = 20;
	t1.c = 30;
	t1.ptr = boost::shared_ptr<in_val_t>(new in_val_t(100));

	o_bson_packet_type opak;
	opak.save(YGGR_SERIALIZE_NAME_NVP("tt", t1));
	...
	ipak.load(YGGR_SERIALIZE_NAME_NVP("tt", t2));
