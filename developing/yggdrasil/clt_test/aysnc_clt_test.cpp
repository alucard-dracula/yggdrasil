...
// !!!!!!!!!!!!!!!!!!!!!!! please use -s -O2 compile this file in debug test !!!!!!!!!!!!!!!!!!!
...
struct Calculater : public boost::enable_shared_from_this<Calculater>
{
private:
	typedef Calculater this_type;
public:
	ERROR_MAKER_BEGIN("Clt_Calculater")
	...
	void register_cal_object(Action_Table& at, const Recv_Handler& handler)
	{
		...
		at.template register_calculator<test_pak_type, Recv_Handler>(
				handler, boost::bind(&Calculater::cal_test_pak_type<owner_info_type,
																	runner_type,
																	recv_handler_type>,
...
int main(int argc, char* argv[])
{
	...
	boost::shared_ptr<Calculater> my_cal(new Calculater());
