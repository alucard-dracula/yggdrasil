...
#include <yggr/network/network_config/network_balance_tcp_config.hpp> // use tcp config
...
#include <yggr/network/network_mixer_handler.hpp> // this example use_mixer_handler
...
#include <yggr/network/heart/heart_timer_task_mixer_pak.hpp> // this example use heart_timer_task_mixer_pak
											...
											const yggr
													::task_center
														::default_task_data_info_type
															::data_info_type&,
											data_info)
		> proxy_adaper_id_parser_def_type;
...
typedef yggr::server::start_mode::server_active_start_mode<
														...
														> tcp_server_start_mode; // tcp session start mode
...
typedef yggr::network::session<yggr::network::type_traits::tag_server,
							...
							> tcp_proxy_session_type; // tcp proxy session
...
typedef yggr::network::session<yggr::network::type_traits::tag_server,
								...
								first_action_type,
								udp_session_state_fixer_type
							> udp_proxy_session_type; // udp proxy session

typedef yggr::network::session_helper::session_killer<tcp_proxy_session_type> tcp_session_killer_type; // tcp session killer

typedef yggr::network::session_helper::session_killer<udp_proxy_session_type> udp_session_killer_type; // udp session kill
...
typedef yggr::network::session_mgr<tcp_proxy_session_type,
									tcp_server_start_mode,
									yggr::network::session_state::default_session_state_checker_type,
									tcp_session_killer_type
									> tcp_proxy_session_mgr_type; // tcp proxy session mgr

typedef yggr::network::session_mgr<udp_proxy_session_type,
									udp_server_start_mode,
									udp_session_state_checker_type,
									udp_session_killer_type
									> udp_proxy_session_mgr_type; // udp proxy session mgr


//proxy session def end

BOOST_MPL_ASSERT((boost::is_same<tcp_conn_type::service_type, udp_conn_type::service_type>)); // check tcp udp service_type is same
typedef udp_conn_type::service_type network_service_type; // rename service_type to network_service_type (ignore enable)
...
typedef yggr::proxy::proxy_ctrl::basic_proxy_msg_fixer
		<
			...
			net_ipak_type,
			net_opak_type,
			svr_ptr_single_type //注意这里 会有编译out_of_heap问题
...
private:
	...
	void register_dispatchers(Ctrl_Center& cc)
	{
		...
		CTRL_HANDLER_PARAM_1(Tag, cc, yggr::u64, udp_proxy_session_type::close_code_type::value,
