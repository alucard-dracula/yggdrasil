...
{
	RETCODE rc = -1;    			/* Return code for ODBC functions */
	TCHAR szConnStrOut[2048] = {0};  // Microsoft ODBC requires at least 1024
									 // bytes here, otherwise will overwrite
									 // memory
	//SWORD cbConnStrOut = 0;
	//UWORD fDriverCompletion = 0;

	SQLSMALLINT cbConnStrOut = 0;
	SQLUSMALLINT fDriverCompletion = bPrompt? SQL_DRIVER_COMPLETE : SQL_DRIVER_NOPROMPT;

	bool bUseConnectStr = !!szConnStrIn;
...
//#include "merant.h"
	...
	if (bUseConnectStr)
		rc = SQLDriverConnect(
				hdbc, g_AppHwnd, reinterpret_cast<SQLTCHAR *>(const_cast<TCHAR*>(szConnStrIn)),
				static_cast<SQLSMALLINT>(std_tstrlen(const_cast<TCHAR *>(szConnStrIn)) * sizeof(TCHAR)),
				reinterpret_cast<SQLTCHAR *>(szConnStrOut), sizeof(szConnStrOut) - 1,
	...
	if(!RC_SUCCESS(rc))
	{
		...
		errmsg += const_cast<TCHAR*>(szConnStrIn);
		errmsg += _TEXT("! \n\n");
		if(bPrompt)
		{
		...
		DTL_THROW DBException(_TEXT("DBConnection::Connect(const TCHAR *DSN)"), errmsg, this, NULL);
	}
	...
	usedDSN = static_cast<TCHAR *>(szConnStrOut);
	state = CONNECTED;
	...
	void DBConnection::Connect(const tstring &DSN, bool bPrompt, bool auto_commit_setting)
	{
		...
		rc = SQLAllocHandle(SQL_HANDLE_DBC, dbe.GetHENV(), &(tmp.hdbc));
...
//#if defined(WIN32) || defined(_WIN32) || defined(_WINDOWS)
			// may be only win32 bugs, need test
			switch(dbType)
			{
			case DB_UNKNOWN:
			case DB_EXCEL:
...
#else
			rc = SQLDisconnect(hdbc);
#endif // #if defined(WIN32) || defined(_WIN32) || defined(_WINDOWS)
			if (!RC_SUCCESS(rc))
			{
