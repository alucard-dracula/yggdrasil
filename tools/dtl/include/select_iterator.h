				   ...
				   size_t row_size = sizeof(DataObj), SQLINTEGER position = BULK_DEFAULT_POSITION)
	{
		...
		if (this->pRowbuf.get() != pbegin) {
			if (!this->stmt.IsReady()) {
				...
				pBytesFetchedArray = static_cast<SQLLEN *>(this->boundIOs.pBytesFetchedArray.get()) ;
				...
				if (NumDates > 0) {
					...
					pDatesFetchedArray = static_cast<TIMESTAMP_STRUCT *>(this->boundIOs.pDatesFetchedArray.get());
				}
				...
				if (this->stmt.GetConnection().GetDBMSEnum() == DBConnection::DB_SQL_SERVER) {

					this->stmt.ClearStmtAttr(SQL_ATTR_CONCURRENCY);
				...
				this->stmt.SetStmtAttr(SQL_ATTR_ROW_ARRAY_SIZE, (SQLPOINTER) bufferRows, 0);
				this->stmt.SetStmtAttr(SQL_ATTR_ROW_BIND_TYPE, (SQLPOINTER) row_size, 0);

				if (this->boundIOs.HasStrings()  || position != BULK_DEFAULT_POSITION) // if position != -1 require scrollable cursor
				{
				   // We require a STATIC type cursor for random access iterators
				   if (position != BULK_DEFAULT_POSITION)
				   {
					   this->stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, (SQLPOINTER)SQL_CURSOR_STATIC, 0);
				   }
				   else
				   {
					   ...
					   if (this->stmt.GetConnection().GetDBMSEnum() == DBConnection::DB_SQL_SERVER)
					   {
						 // SQL Server requires the use of keyset driven cursors for bulk operations
						   this->stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, (SQLPOINTER)SQL_CURSOR_KEYSET_DRIVEN, 0);
					   }
					   else
					   {
						   this->stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, (SQLPOINTER)SQL_CURSOR_STATIC, 0);
				...
				else
				{
					...
					this->stmt.SetStmtAttr(SQL_ATTR_CURSOR_TYPE, SQL_CURSOR_FORWARD_ONLY, 0);
				}
				...
				this->boundIOs.pRowStatusArray.reset(bufferRows * sizeof(STATUS));
				this->boundIOs.pNumRowsFetched = new SQLULEN(0);

				pRowStatusArray = static_cast<STATUS*>(this->boundIOs.pRowStatusArray.get());

				// SetStmtAttr calls for row wise binding status flags
				this->stmt.SetStmtAttr(SQL_ATTR_ROW_STATUS_PTR, pRowStatusArray, 0);
				this->stmt.SetStmtAttr(SQL_ATTR_ROWS_FETCHED_PTR, this->boundIOs.pNumRowsFetched.get(), 0);

				// Make bindings using pBytesFetchedArray
				// Make bindings must come after SetStmtAttr calls for attributes to be set
				SetWorkingRowAddr((DataObj *)(NULL), this->boundIOs, static_cast<BYTE *>(this->pRowbuf.get()), pDatesFetchedArray);
				...
				const SQLINTEGER bufferRows = this->boundIOs.pBytesFetchedArray.size() / row_size;
				...
				pDatesFetchedArray = static_cast<TIMESTAMP_STRUCT *>(this->boundIOs.pDatesFetchedArray.get()) ;
				SetWorkingRowAddr(static_cast<DataObj *>(NULL), this->boundIOs, static_cast<BYTE *>(this->pRowbuf.get()), pDatesFetchedArray);
				this->MakeColBindings();
				pRowStatusArray = static_cast<STATUS*>(this->boundIOs.pRowStatusArray.get());
				pBytesFetchedArray = static_cast<SQLLEN *>(this->boundIOs.pBytesFetchedArray.get()) ;
		...
		if (position == BULK_DEFAULT_POSITION)
			...
			status = this->stmt.FetchScroll(SQL_FETCH_NEXT, 0);
		else if (position == BULK_END_POSITION)
		{
			...
			if (this->stmt.GetConnection().GetDBMSEnum() == DBConnection::DB_MYSQL)
				status = this->stmt.FetchScroll(SQL_FETCH_ABSOLUTE, -static_cast<SQLINTEGER>((pdata_end - pdata_begin) / row_size));
			else
				status = this->stmt.FetchScroll(SQL_FETCH_LAST, 0);
			...
			else
			{
				...
				try {
					this->stmt.GetStmtAttr(SQL_ATTR_ROW_NUMBER, &endrow, SQL_IS_UINTEGER, &end_bytes);
				}
				...
				if (endrow == 0) // function could not get base row count
				{
					...
					endrow = LONG_MAX - *(this->boundIOs.pNumRowsFetched);
	#if 0
						this->setstate(this->badbit);
						tostringstream errStream;
			...
			status = this->stmt.FetchScroll(SQL_FETCH_ABSOLUTE, position);
		...
		begin = false;
		this->count+=*(this->boundIOs.pNumRowsFetched);
		this->lastCount = *(this->boundIOs.pNumRowsFetched);
      ...
      BYTE *pBytesFetchedBackup = new BYTE[row_size];
		...
		if (this->boundIOs.HasStrings()) {
			...
			for (size_t i = 0; i < *(this->boundIOs.pNumRowsFetched); ++i)
			{
				if (pRowStatusArray[i] != SQL_ROW_ERROR) {
					...
					SetComplexRowAddr(static_cast<DataObj *>(NULL), this->boundIOs, pdata_begin + i * row_size, pDatesFetchedRow);

					try {
						this->stmt.SetPos(i+1, SQL_POSITION, SQL_LOCK_NO_CHANGE);
					}
					catch (RootException &)
					{
						this->setstate(this->badbit);
		...
		if (this->boundIOs.HasStrings() || pDatesFetchedArray != NULL)
		...
		for (size_t i = 0; i < *(this->boundIOs.pNumRowsFetched); ++i)
		{
			...
			try
			{
				...
				this->boundIOs.SelUserValidateAddr(pdata_begin + i * row_size);
			...
			catch (RootException &ex)
			{
				...
				if (error_action == dtl_ios_base::SUPPRESS_ERROR && !dtl_ios_base::bad())
				{
					...
					this->io_handler.RecordFailure(i); // record index of failed row
