...
#    define DTL_NO_CXX98_BINDERS
#  elif defined(_HAS_DEPRECATED_ADAPTOR_TYPEDEFS) && (_HAS_DEPRECATED_ADAPTOR_TYPEDEFS == 0)
#    define DTL_NO_CXX98_BINDERS
#  endif
#endif

#if defined(__clang__) && (__clang__) && (__cplusplus > 201700) && !defined(DTL_NO_CXX98_BINDERS)
#	define DTL_NO_CXX98_BINDERS
#endif // defined(__clang__) && (__clang__) && (__cplusplus > 201700)
...
template<class DataObj>
class lt_functor 
	//: public STD_::binary_function<const DataObj *, const DataObj *, bool>
{
public:
	typedef const DataObj* first_argument_type;
	typedef const DataObj* second_argument_type;
...
public:
	...
	inline bool operator()(const DataObj *key1, const DataObj *key2) const
	{
		return data->lt(key1, key2);
	}
...
template<class DataObj>
class hash_functor
	// : public STD_::unary_function<const DataObj *, size_t>
{
public:
	typedef const DataObj* argument_type;
	typedef size_t result_type;

public:
	inline size_t operator()(const DataObj *key1) const
	{
		return data->hash(key1);
	}
...
#if defined(DTL_NO_CXX98_BINDERS)
					STD_::bind(NonCaseSensitiveEq(), fieldName, std::placeholders::_1))
#else
					STD_::bind1st(NonCaseSensitiveEq(), fieldName))
#endif // DTL_NO_CXX98_BINDERS
						== colNames.end())
					DTL_THROW DBException(_TEXT("DBIndex::DBIndex()"),
					_TEXT("Invalid FieldList \"") +
