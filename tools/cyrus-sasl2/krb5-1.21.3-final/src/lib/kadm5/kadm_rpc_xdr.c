...
bool_t xdr_nulltype(XDR *xdrs, void **objp, xdrproc_t proc)
{
	typedef	bool_t (*xdrproc_t_now)(XDR*, void **);
     ...
     case XDR_DECODE:
	  ...
	  return (*((xdrproc_t_now)proc))(xdrs, objp);
			     ...
			     int v)
{
	...
	r = xdr_array(xdrs, (caddr_t *) &objp->key_data, &n, objp->n_key_data,
		      sizeof(krb5_key_data), (xdrproc_t)&xdr_krb5_key_data_nocontents);
	objp->n_key_data = n;
	if (!r) {
		return (FALSE);
	}
...
xdr_setkey3_arg(XDR *xdrs, setkey3_arg *objp)
{
	...
	if (!xdr_array(xdrs, (caddr_t *) &objp->ks_tuple,
		       (unsigned int *) &objp->n_ks_tuple, ~0,
		       sizeof(krb5_key_salt_tuple), (xdrproc_t)&xdr_krb5_key_salt_tuple)) {
		return (FALSE);
	}
	if (!xdr_array(xdrs, (caddr_t *) &objp->keyblocks,
		       (unsigned int *) &objp->n_keys, ~0,
...
xdr_setkey4_arg(XDR *xdrs, setkey4_arg *objp)
{
	...
	if (!xdr_array(xdrs, (caddr_t *) &objp->key_data,
		       ...
		       sizeof(kadm5_key_data), (xdrproc_t)&xdr_kadm5_key_data)) {
		return FALSE;
	}
	return TRUE;
}
