     ...
     bool_t *no_dispatch)
{
     ...
     if (svc_debug_gssapi) {
	  if (creds.auth_msg && rqst->rq_proc == AUTH_GSSAPI_EXIT) {
	       ...
	       svc_sendreply(rqst->rq_xprt, (xdrproc_t)&xdr_void, NULL);
	       xdr_free((xdrproc_t)&xdr_authgssapi_creds, &creds);
	       cleanup();
	       exit(0);
	  }
     ...
     caddr_t xdr_ptr)
{
	...
	typedef bool_t (*xdr_func_t_now)(XDR*, caddr_t);

     OM_uint32 gssstat, minor_stat;

     if (! SVCAUTH_PRIVATE(auth)->established) {
	  PRINTF(("svc_gssapi_wrap: not established, noop\n"));
	  return (*((xdr_func_t_now)xdr_func))(out_xdrs, xdr_ptr);
...
static bool_t svc_auth_gssapi_unwrap(
     ...
     caddr_t xdr_ptr)
{
	 typedef bool_t (*xdr_func_t_now)(XDR*, auth_gssapi_init_arg*);
     ...
     if (! client_data->established) {
	  ...
	  return (*((xdr_func_t_now)xdr_func))(in_xdrs, (auth_gssapi_init_arg *)(void *) xdr_ptr);
