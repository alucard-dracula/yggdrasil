...
USRDEF_LOCAL_PROJECT_NAME := luabind_ts

l_path := $(LOCAL_PATH)

# output module name
ifeq ($(NDK_DEBUG), 1)
	l_module := luabind09_ts$(USRDEF_APP_COMPILER_VERSION_TAG)-d
else
	l_module := luabind09_ts$(USRDEF_APP_COMPILER_VERSION_TAG)
endif
...
ifeq ($(NDK_DEBUG), 1)
	l_ldflags += \
		-llua$(USRDEF_APP_LUA_VERSION)$(USRDEF_APP_COMPILER_VERSION_TAG)-d \
		$(USRDEF_APP_LDFLAGS_DEBUG) \

else
	l_ldflags += \
		-s \
		-llua$(USRDEF_APP_LUA_VERSION)$(USRDEF_APP_COMPILER_VERSION_TAG) \
