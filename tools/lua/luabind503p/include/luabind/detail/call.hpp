...
/** \brief Interface (with 2 virtual functions)
 *
 * \param
 * \param
 * \return
...
struct LUABIND_API invoke_context
{
     ...
     * \brief Pushes a string describing error on stack.
    **/
    void format_error(lua_State* L, function_object const* overloads) const;

    int best_score;
    function_object const* candidates[10];  //!< max number of overloads ?
    int candidate_index;
};
...
/**
 * \brief will use specialization of invoke0 according to last parameter (is_member_function_pointer true_ or false_)
**/
  ...
  , F const& f, Signature, Policies const& policies)
{
    return invoke0(
        L, self, ctx, f, Signature(), policies
      , boost::is_void<typename mpl::front<Signature>::type>()  //!< Return type
...
/**
* BOOST_PP_ITERATION will generate matching functions with names:
-invoke_member
-invoke_normal
with specialization for each size of signature -1 (Hence the mpl::long_<N>)
**/
template <class F, class Signature, class Policies>
inline int
# ifdef LUABIND_INVOKE_MEMBER
...
# ifndef LUABIND_INVOKE_VOID
    ...
    typename mpl::apply_wrap2<result_policy, result_type, cpp_to_lua>::type result_converter;
...
# endif
        ...
        results = maybe_yield(L, lua_gettop(L) - arguments, static_cast<Policies*>(0));
