...
#	include <inttypes.h>
#else
#	include <mscompatibility/inttypes.h>
#endif // _MSC_VER
...
static int64_t _long(lua_State* L, int pos)
{
	...
	case LUA_TSTRING:
		...
		n = 
			errno == ERANGE?
				(errno = old, luaL_error(L, "integral is too large: %s", str))
				: (errno = old, n);
		break;
	}
...
typedef union _struct_pack_helper_t
{
	int32_t v_s32;
	uint32_t v_u32;
	int64_t v_s64;
...
} struct_pack_helper_t;

static int struct_pack(lua_State *L)
{
	lua_Number value = 0;
	struct_pack_helper_t helper = {0};
    uint8_t format = luaL_checkinteger(L, 2);  
    ...
    switch(format)
    {
        case 'i':
            ...
            lua_settop(L, 1);
            helper.v_s32 = (int32_t)value;
            pack_fixed32(L, (uint8_t*)&helper.v_s32);
            break;
       case 'q':
            {                
                helper.v_s64 = _long(L, 3);
                lua_settop(L, 1);                
                pack_fixed64(L, (uint8_t*)&helper.v_s64);
                break;
            }
        case 'f':
            {
                value = luaL_checknumber(L, 3);
                lua_settop(L, 1);
                helper.v_f32 = (float)value;
                pack_fixed32(L, (uint8_t*)&helper.v_f32);
                break;
            }
        case 'd':
            {
                value = luaL_checknumber(L, 3);
                lua_settop(L, 1);
                helper.v_f64 = (double)value;
                pack_fixed64(L, (uint8_t*)&helper.v_f64);
                break;
            }
        case 'I':
            {
                lua_Number value = luaL_checknumber(L, 3);
                lua_settop(L, 1);
                helper.v_u32 = (uint32_t)value;
                pack_fixed32(L, (uint8_t*)&helper.v_u32);
                break;
            }
        case 'Q':
            {                        
                helper.v_u64 = _ulong(L, 3);
                lua_settop(L, 1);                                
                pack_fixed64(L, (uint8_t*)&helper.v_u64);
                break;
            }
        default:
            luaL_error(L, "Unknown, format");
