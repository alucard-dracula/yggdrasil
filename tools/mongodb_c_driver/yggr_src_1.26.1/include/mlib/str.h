...
static MONGOC_INLINE mstr_mut mstr_new(size_t len) {
...
static MONGOC_INLINE mstr_mut mstr_new(size_t len) {
#ifndef __clang_analyzer__
	mstr_mut ret = {{{(char *)calloc(1, len + 1), len}}};
    assert(len < SIZE_MAX);
#else
    // Clang-analyzer is smart enough to see the calloc(), but not smart enough
    // to link it to the free() in mstr_free()
...
static MONGOC_INLINE mstr_view mstrv_view_data(const char *s, size_t len) {
 ...
 */
static MONGOC_INLINE mstr_view mstrv_view_cstr(const char *s) {
 ...
 */
static MONGOC_INLINE mstr mstr_copy_data(const char *s, size_t len) {
 ...
 */
static MONGOC_INLINE mstr mstr_copy_cstr(const char *s) {
 ...
 */
static MONGOC_INLINE mstr mstr_copy(mstr_view s) {
 ...
 */
static MONGOC_INLINE void mstr_free(mstr s) {
    free((char *)s.data);
}
 ...
 */
static MONGOC_INLINE void mstrm_resize(mstr_mut *s, size_t new_len) {
    if (new_len <= s->len) {
        s->len = new_len;
    } else {
 ...
 * ```
 */
static MONGOC_INLINE void mstr_assign(mstr *s, mstr from) {
    mstr_free(*s);
    *s = from;
}
...
static MONGOC_INLINE int mstr_find(mstr_view given, mstr_view needle) {
...
static MONGOC_INLINE int mstr_find(mstr_view given, mstr_view needle) {
	...
	const char *scan = 0;
	const char *nscan = 0;
	const char *subscan = 0;
	size_t remain = 0;

    for (scan = given.data; scan != scan_end; ++scan) {
        remain = (size_t)(scan_end - scan);
        if (remain < needle.len) {
            break;
        }
		subscan = scan;
        for (nscan = needle.data; nscan != needle_end; ++nscan, ++subscan) {
            if (*nscan == *subscan) {
                continue;
            } else {
                goto skip;
...
static MONGOC_INLINE int mstr_rfind(mstr_view given, mstr_view needle) {
...
static MONGOC_INLINE int mstr_rfind(mstr_view given, mstr_view needle) {
	...
	const char *scan = 0;
	const char *nscan = 0;
	const char *needle_end = 0;
	const char *subscan = 0;

	if (needle.len > given.len) {
        return -1;
    }
    scan = given.data + given.len - needle.len;
    needle_end = needle.data + needle.len;
    for (; scan >= given.data; --scan) {
        subscan = scan;
        for (nscan = needle.data; nscan != needle_end; ++nscan, ++subscan) {
            if (*nscan == *subscan) {
...
static MONGOC_INLINE mstr mstr_splice(mstr_view s, size_t at, size_t del_count, mstr_view insert) {
    assert(at <= s.len);
...
static MONGOC_INLINE mstr mstr_splice(mstr_view s, size_t at, size_t del_count, mstr_view insert) {
    ...
    /* at this point, it is absolutely necessary that del_count <= s.len */
    assert(s.len - del_count <= SIZE_MAX - insert.len);
    new_size = s.len - del_count + insert.len;
    ret = mstr_new(new_size);
    p = ret.data;
    memcpy(p, s.data, at);
    p += at;
    if (insert.data) {
 ...
 */
static MONGOC_INLINE mstr mstr_append(mstr_view s, mstr_view suffix) {
 ...
 */
static MONGOC_INLINE mstr mstr_prepend(mstr_view s, mstr_view prefix) {
 ...
 */
static MONGOC_INLINE mstr mstr_insert(mstr_view s, size_t at, mstr_view infix) {
 ...
 */
static MONGOC_INLINE mstr mstr_erase(mstr_view s, size_t at, size_t count) {
 ...
 */
static MONGOC_INLINE mstr mstr_remove_prefix(mstr_view s, size_t len) {
 ...
 */
static MONGOC_INLINE mstr mstr_remove_suffix(mstr_view s, size_t len) {
    assert(s.len >= len);
...
static MONGOC_INLINE mstr mstr_substr(mstr_view s, size_t at, size_t len) {
    assert(at <= s.len);
    const size_t remain = s.len - at;
    if (len > remain) {
        len = remain;
...
static MONGOC_INLINE mstr_view mstrv_subview(mstr_view s, size_t at, size_t len) {
    assert(at <= s.len);
    const size_t remain = s.len - at;
    if (len > remain) {
        len = remain;
 ...
 * front
 */
static MONGOC_INLINE mstr_view mstrv_remove_prefix(mstr_view s, size_t len) {
 ...
 * end.
 */
static MONGOC_INLINE mstr_view mstrv_remove_suffix(mstr_view s, size_t len) {
    assert(s.len >= len);
 ...
 */
static MONGOC_INLINE mstr mstr_trunc(mstr_view s, size_t new_len) {
...
static MONGOC_INLINE mstr mstr_replace(const mstr_view string, const mstr_view find, const mstr_view subst) {
    if (find.len == 0) {
...
static MONGOC_INLINE mstr mstr_replace(const mstr_view string, const mstr_view find, const mstr_view subst) {
    ...
    // First copy the string
    ret = mstr_copy(string);
    // Keep an index of how far we have processed
    //size_t whence = 0;
    for (;;) {
        // Chop off the front that has already been processed
        tail = mstrv_subview(ret.view, whence, SIZE_MAX);
        // Find where in that tail is the next needle
        pos = mstr_find(tail, find);
        if (pos == -1) {
            // We're done
            break;
        }
 ...
 */
static MONGOC_INLINE bool mstr_eq(mstr_view left, mstr_view right) {
 ...
 */
static MONGOC_INLINE bool mstr_eq_ignore_case(mstr_view left, mstr_view right) {
#ifdef _WIN32
...
static MONGOC_INLINE bool mstr_is_printable(char c) {
    return (c >= ' ' && c <= '~');
}
...
static MONGOC_INLINE void _mstr_write_str_repr_(FILE *out, mstr_view s) {
...
static MONGOC_INLINE void _mstr_write_str_repr_(FILE *out, mstr_view s) {
	const char *it = 0;
    for (it = s.data; it != s.data + s.len; ++it) {
        if (mstr_is_printable(*it)) {
            fputc(*it, out);
        } else {
            fprintf(out, "\\x%.2x", (unsigned)(unsigned char)*it);
...
static MONGOC_INLINE void _mstr_assert_(mstr_view left,
                                 mstr_view right,
 ...
 */
static MONGOC_INLINE bool mstr_contains(mstr_view given, mstr_view needle) {
 ...
 */
static MONGOC_INLINE bool mstr_starts_with(mstr_view given, mstr_view prefix) {
 ...
 */
static MONGOC_INLINE bool mstr_ends_with(mstr_view given, mstr_view suffix) {
...
static MONGOC_INLINE mstr_widen_result mstr_win32_widen(mstr_view str) {
...
static MONGOC_INLINE mstr_widen_result mstr_win32_widen(mstr_view str) {
	...
	mstr_widen_result ret = {0};

    assert(str.len <= INT_MAX);
    length = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data, (int)str.len, NULL, 0);
    if (length == 0 && str.len != 0) {
		ret.error = (int)GetLastError();
		return ret;
    }
    str_ret = (wchar_t *)calloc(length + 1, sizeof(wchar_t));
    assert(length < INT_MAX);
    got_length = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data, (int)str.len, str_ret, length + 1);
    assert(got_length == length);
	ret.wstring = str_ret;
	return ret;
}
...
static MONGOC_INLINE mstr_narrow_result mstr_win32_narrow(const wchar_t *wstring) {
...
static MONGOC_INLINE mstr_narrow_result mstr_win32_narrow(const wchar_t *wstring) {
    ...
    int length =
        WideCharToMultiByte(CP_UTF8, wcflags, wstring, -1 /* wstring is null-terminated */, NULL, 0, NULL, NULL);
	mstr_narrow_result ret = {MSTR_NULL};
	int got_len = 0;
	mstr_mut str_ret = {{{0}}};
    ...
    // Allocate a new string, not including the null terminator
    str_ret = mstr_new((size_t)(length - 1));
    got_len = WideCharToMultiByte(CP_UTF8,
                                      wcflags,
                                      wstring,
                                      -1,
                                      str_ret.data,
...
static MONGOC_INLINE void _mstr_split_iter_next_(struct _mstr_split_iter_ *iter) {
    if (iter->once == 1) {
...
static MONGOC_INLINE void _mstr_split_iter_next_(struct _mstr_split_iter_ *iter) {
    ...
    // Find the next occurence of the token
    pos = mstr_find(iter->remaining, iter->splitter);
    if (pos < 0) {
        // There are no more occurences. yield the remaining string
        iter->part = iter->remaining;
        iter->remaining = mstrv_subview(iter->remaining, iter->remaining.len, 0);
...
static MONGOC_INLINE struct _mstr_split_iter_ _mstr_split_iter_begin_(mstr_view str, mstr_view split) {
...
static MONGOC_INLINE struct _mstr_split_iter_ _mstr_split_iter_begin_(mstr_view str, mstr_view split) {
	struct _mstr_split_iter_ iter = {{0}};
	iter.remaining = str;
	iter.splitter = split;
...
static MONGOC_INLINE bool _mstr_split_iter_done_(struct _mstr_split_iter_ *iter) {
    return iter->state == 2;
}
...
static mlib_constexpr_fn size_t mlib_strnmcopy(char *dst, size_t dst_bufsize, const char *src, size_t src_bufsize) {
    ...
    // The maximum number of characters in the dest is one less than the buffer
    // size, since we need room for the null terminator:
    dstlen = dst_bufsize - 1u;
    // The actual maximum number of characters we can copy is the less of the
    // source length and the dest length:
    minlen = dstlen < src_bufsize ? dstlen : src_bufsize;
    // Track what we copy:
    //size_t ncopied = 0;
    while (ncopied != minlen // Stop if we hit our character limit
           && *src != 0      // Or if we hit the null terminator in the source
