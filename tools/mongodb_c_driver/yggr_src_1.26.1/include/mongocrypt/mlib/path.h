...
static MONGOC_INLINE mstr mpath_current_path(void) {
	...
	wchar_t *wstr = 0;
	DWORD got_len = 0;
	mstr_narrow_result nar = {MSTR_NULL};
    
	while (1) {
        len = GetCurrentDirectoryW(0, NULL);
        wstr = calloc(sizeof(wchar_t), len);
        got_len = GetCurrentDirectoryW(len, wstr);
        if (got_len > len) {
            free(wstr);
            continue;
        }
...
static MONGOC_INLINE mstr mpath_absolute_from(mstr_view path, mstr_view base, mpath_format f) {
	...
	mstr abs_base = MSTR_NULL;
	mstr_view base_rdir = {0};
	mstr_view base_relpath = {0};
	mstr_view relpath = {0};
    mstr ret = MSTR_NULL;

	mstr_view base_rname = {0};
