...
#define TRY(stmt)                                                                                                      \
    ...
    // First operator_ is the non-stub.
    if (!mc_makeRangeFindPlaceholder(&args, &p1, status)) {
        goto fail;
    }

    // Second operator_ (if required) is a stub.
    if (spec->nOps == 2) {
    ...
    if (spec->isAggregateExpression) {
        ...
        */
        ...
        bson_t operator_;
        TRY(BSON_APPEND_ARRAY_BEGIN(&elem, mc_FLE2RangeOperator_to_string(spec->firstOp), &operator_));
        TRY(BSON_APPEND_UTF8(&operator_, "0", spec->field));
        TRY(_mongocrypt_buffer_append(&p1, &operator_, "1", 1));
        TRY(bson_append_array_end(&elem, &operator_));
        TRY(bson_append_document_end(&and, &elem));

        if (spec->nOps == 2) {
            TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "1", &elem));
            TRY(BSON_APPEND_ARRAY_BEGIN(&elem, mc_FLE2RangeOperator_to_string(spec->secondOp), &operator_));
            TRY(BSON_APPEND_UTF8(&operator_, "0", spec->field));
            TRY(_mongocrypt_buffer_append(&p2, &operator_, "1", 1));
            TRY(bson_append_array_end(&elem, &operator_));
            TRY(bson_append_document_end(&and, &elem));
        }
        ...
        */
        ...
        TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "0", &elem));
        bson_t operator_;
        TRY(BSON_APPEND_DOCUMENT_BEGIN(&elem, spec->field, &operator_));
        const char *op_str = mc_FLE2RangeOperator_to_string(spec->firstOp);
        TRY(_mongocrypt_buffer_append(&p1, &operator_, op_str, -1));
        TRY(bson_append_document_end(&elem, &operator_));
        TRY(bson_append_document_end(&and, &elem));
        ...
        if (spec->nOps == 2) {
            ...
            TRY(_mongocrypt_buffer_append(&p2, &operator_, op_str, -1));
            TRY(bson_append_document_end(&elem, &operator_));
            TRY(bson_append_document_end(&and, &elem));
        }
