...
static bool _crypto_aes_256_ctr_encrypt_decrypt_via_ecb(void *ctx,
    ...
    for (/*uint32_t*/ ptr = 0; ptr < args.in->len;) {
        ...
        /* assert rather than return since this should never happen */
        BSON_ASSERT(ctr_bin.len == 0u || ctr_bin.len - 1u <= INT_MAX);
        for (/*int*/ ii = (int)ctr_bin.len - 1; ii >= 0 && carry != 0; --ii) {
            uint8_t *ctr_bin_u8 = ctr_bin.data;
            uint32_t bpp = carry + ctr_bin_u8[ii];
            carry = bpp >> 8;
            ctr_bin_u8[ii] = bpp & 0xFF;
        }
    }
...
static bool _encrypt_step(_mongocrypt_crypto_t *crypto,
	...
	*bytes_written = 0;
	...
	if (mode == MODE_CTR) {
		...
		//													.bytes_written = bytes_written,
		//													.status = status});
		aes_256_arg.key = enc_key;
		aes_256_arg.iv = iv;
		aes_256_arg.in = plaintext;
		aes_256_arg.out = ciphertext;
		aes_256_arg.bytes_written = bytes_written;
		aes_256_arg.status = status;

		return _crypto_aes_256_ctr_encrypt(crypto, aes_256_arg);
	}
	...
	aes_256_arg.key = enc_key;
	aes_256_arg.iv = iv;
	aes_256_arg.in = &to_encrypt;
	aes_256_arg.out = ciphertext;
	aes_256_arg.bytes_written = bytes_written;
	aes_256_arg.status = status;

	ret = _crypto_aes_256_cbc_encrypt(crypto, aes_256_arg);
...
static bool _decrypt_step(_mongocrypt_crypto_t *crypto,
	...
	*bytes_written = 0;
	...
	if (mode == MODE_CBC) {
		...
		aes_256_arg.iv = iv;
		aes_256_arg.key = enc_key;
		aes_256_arg.in = ciphertext;
		aes_256_arg.out = plaintext;
		aes_256_arg.bytes_written = bytes_written;
		aes_256_arg.status = status;

		if(!_crypto_aes_256_cbc_decrypt(crypto, aes_256_arg))
		{
			return false;
		}
		...
		*bytes_written -= padding_byte;
		...
		if (!_crypto_aes_256_ctr_decrypt(crypto, aes_256_arg)) {
			return false;
		}
...
bool _mongocrypt_random_int64(_mongocrypt_crypto_t *crypto,
								...
								int64_t *out,
								mongocrypt_status_t *status) {
	uint64_t u64_exclusive_upper_bound = 0;
	uint64_t u64_out;
