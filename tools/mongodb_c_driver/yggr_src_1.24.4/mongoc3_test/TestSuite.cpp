               ...
               void *arg)
{
	...
	*thread = CreateThread (NULL, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(cb), arg, 0, NULL);
	return 0;
}
#endif
                   ...
                   int (*check) (void)) /* IN */
{
   ...
   test = reinterpret_cast<Test*>(calloc (1, sizeof *test));
...
{
   ParallelInfo *info = reinterpret_cast<ParallelInfo *>(data);
...
TestSuite_RunParallel (TestSuite *suite) /* IN */
{
   ...
   threads = reinterpret_cast<HANDLE*>(calloc (count, sizeof *threads));

   Memory_Barrier ();

   for (test = suite->tests, i = 0; test; test = test->next, i++) {
      info = reinterpret_cast<ParallelInfo*>(calloc (1, sizeof *info));
      info->suite = suite;
      info->test = test;
      info->count = &count;
      info->mutex = &mutex;
