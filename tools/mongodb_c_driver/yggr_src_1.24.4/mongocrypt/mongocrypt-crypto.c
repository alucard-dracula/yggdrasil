...
static bool _decrypt_step(_mongocrypt_crypto_t *crypto,
    ...
    *bytes_written = 0;
    ...
    if (mode == MODE_CBC) {
		...
		aes_256_args.iv = iv;
		aes_256_args.key = enc_key;
		aes_256_args.in = ciphertext;
		aes_256_args.out = plaintext;
		aes_256_args.bytes_written = bytes_written;
		aes_256_args.status = status;

        if (!_crypto_aes_256_cbc_decrypt(crypto, aes_256_args)) {
            return false;
        }
        ...
        *bytes_written -= padding_byte;
        ...
        BSON_ASSERT(mode == MODE_CTR);
        if (!_crypto_aes_256_ctr_decrypt(crypto, aes_256_args)) {
            return false;
        }
        BSON_ASSERT(*bytes_written == plaintext->len);
    }
