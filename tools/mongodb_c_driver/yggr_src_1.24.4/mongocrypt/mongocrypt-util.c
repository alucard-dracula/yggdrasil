...
current_module_result current_module_path(void) {
    mstr ret_str = MSTR_NULL;
    int ret_error = 0;
	current_module_result ret = {MSTR_NULL};
#ifdef _WIN32
    DWORD acc_size = 512;
	wchar_t *path = 0;
	mstr_narrow_result narrow = {MSTR_NULL};
    while (!ret_str.data && !ret_error) {
        // Loop until we allocate a large enough buffer or get an error
        /*wchar_t **/path = calloc(acc_size + 1, sizeof(wchar_t));
        SetLastError(0);
...
bool mc_isinf(double d) {
	static const uint64_t mask = 0x7fffffffffffffffui64;
	static const uint64_t chk_data = 0x7ff0000000000000ui64;
	const uint64_t* p = (uint64_t*)(&d);
	return ((*p) & mask) == chk_data;
}

bool mc_isfinite(double d) {
	static const uint64_t mask = 0x7ff0000000000000ui64;
	const uint64_t* p = (uint64_t*)(&d);
	const uint64_t chk_data = ((*p) & mask);
	return (chk_data) && (chk_data != mask);
}
