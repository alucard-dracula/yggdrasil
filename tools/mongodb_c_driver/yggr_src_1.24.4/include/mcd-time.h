...
static BSON_INLINE const mcd_time_point get_mcd_time_point_max(void)
{
	static const mcd_time_point tmp = {INT64_MAX};
	return tmp;
}

static BSON_INLINE const mcd_time_point get_mcd_time_point_min(void)
{
	static const mcd_time_point tmp = {INT64_MIN};
	return tmp;
}

/// The latest representable future point-in-time
#define MCD_TIME_POINT_MAX get_mcd_time_point_max()
/// The oldest representable past point-in-time
#define MCD_TIME_POINT_MIN get_mcd_time_point_min()
...
static BSON_INLINE const mcd_duration get_mcd_duration_max(void)
{
	static const mcd_duration tmp = {INT64_MAX};
	return tmp;
}

static BSON_INLINE const mcd_duration get_mcd_duration_min(void)
{
	static const mcd_duration tmp = {INT64_MIN};
	return tmp;
}

static BSON_INLINE const mcd_duration get_mcd_duration_zero(void)
{
	static const mcd_duration tmp = {INT64_MIN};
	return tmp;
}
...
/// The maximum representable duration
#define MCD_DURATION_MAX get_mcd_duration_max()
/// The minimal representable (negative) duration
#define MCD_DURATION_MIN get_mcd_duration_min()
/// A duration representing zero amount of time
#define MCD_DURATION_ZERO get_mcd_duration_zero()
...
mcd_now (void)
{
	mcd_time_point tmp = {bson_get_monotonic_time ()};
   // Create a time point representing the current time.
   return tmp;
}
