...
static BSON_INLINE const mc_shared_tpld g_mc_shared_tpld_null(void)
{
	static const mc_shared_tpld tmp = {MONGOC_SHARED_PTR_NULL};
	return tmp;
}

#	define MC_SHARED_TPLD_NULL g_mc_shared_tpld_null()

#endif // #if defined(MONGOC_EX_C99_SUPPORTED)
...
mc_tpld_take_ref (const mongoc_topology_t *tpl)
{
	mc_shared_tpld ret = {0};
	ret._sptr_= mongoc_atomic_shared_ptr_load (&tpl->_shared_descr_._sptr_);
	return ret;
}
...
mc_tpld_unsafe_get_mutable (mongoc_topology_t *tpl)
{
   return reinterpret_cast<mongoc_topology_description_t *>(tpl->_shared_descr_._sptr_.ptr);
}
...
mc_tpld_unsafe_get_const (const mongoc_topology_t *tpl)
{
   return reinterpret_cast<const mongoc_topology_description_t *>(tpl->_shared_descr_._sptr_.ptr);
}
