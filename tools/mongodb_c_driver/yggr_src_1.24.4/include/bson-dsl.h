 ...
 */
 ...
 #if defined(BSON_EX_C99_SUPPORTED)
...
#define bsonParse(Document, ...)										\
	_bsonDSL_begin_part_1()												\
	BSON_MAYBE_UNUSED bool _bvHalt = false;								\
	BSON_MAYBE_UNUSED /*const*/ bool _bvContinue = false;				\
	BSON_MAYBE_UNUSED /*const*/ bool _bvBreak = false;					\
	_bsonDSL_begin_part_2 ("bsonParse(%s)", _bsonDSL_str (Document));	\
	_bsonDSL_disableWarnings ();										\
	bsonParseError = NULL;												\
	_bsonDSL_eval (_bsonParse ((Document), __VA_ARGS__));				\
	_bsonDSL_restoreWarnings ();										\
...
#define bsonVisitEach(Document, ...)										\
	_bsonDSL_begin_part_1()													\
	BSON_MAYBE_UNUSED bool _bvHalt = false;									\
	_bsonDSL_begin_part_2("bsonVisitEach(%s)", _bsonDSL_str (Document));	\
	_bsonDSL_disableWarnings ();											\
	_bsonDSL_eval (_bsonVisitEach ((Document), __VA_ARGS__));				\
	_bsonDSL_restoreWarnings ();											\
	_bsonDSL_end
...
// decompose _bsonDSL_begin
#define _bsonDSL_begin_part_1()			\
   if (true) {                          

#define _bsonDSL_begin_part_2(Str, ...)	\
	_bsonDSLDebug (Str, __VA_ARGS__);	\
	++_bson_dsl_indent
...
#define _bsonDocOperation(Command, _ignore, _count)                   \
   if (!bsonBuildError) {                                             \
      ...
      if (bsonBuildError) {                                           \
         _bsonDSLDebug ("Stopping doc() due to bsonBuildError: [%s]", \
                        bsonBuildError);                              \
      }                                                               \
   }
...
#define _bsonDocOperation_kvl(String, Len, Element)										\
	...
	const char *_bbString = (String);													\
	const uint64_t length = (Len);														\
	_bsonDSL_begin_part_2 ("\"%s\" => [%s]", String, _bsonDSL_strElide (30, Element));	\
	if (bson_in_range_unsigned (int, length)) {											\
		_bbCtx.key = _bbString;															\
		_bbCtx.key_len = (int) length;													\
		_bsonValueOperation (Element);													\
...
#define _bsonValueOperationDeferred_doc(...)                                   \
   ...
   if (!bson_append_document_begin (_bsonBuildAppendArgs, &_bbChildDoc)) {     \
      bsonBuildError =                                                         \
         "Error while initializing child document: " _bsonDSL_str (            \
            __VA_ARGS__);                                                      \
   } else {                                                                    \
      ...
      if (!bsonBuildError) {                                                   \
         if (!bson_append_document_end (bsonBuildContext.doc, &_bbChildDoc)) { \
            bsonBuildError =                                                   \
               "Error while finalizing document: " _bsonDSL_str (__VA_ARGS__); \
         }                                                                     \
      }                                                                        \
   }                                                                           \
   _bsonDSL_end
...
#define _bsonValueOperationDeferred_doc(...)										\
	_bsonDSL_begin_part_1 ()														\
	bson_t _bbChildDoc = BSON_INITIALIZER;											\
	_bsonDSL_begin_part_2 ("doc(%s)", _bsonDSL_strElide (30, __VA_ARGS__));         \
	/* Write to this variable as the child: */										\
	if (!bson_append_document_begin (_bsonBuildAppendArgs, &_bbChildDoc)) {			\
		bsonBuildError =															\
			"Error while initializing child document: " _bsonDSL_str (				\
...
#define _bsonValueOperationDeferred_array(...)                                \
   ...
   if (!bson_append_array_begin (_bsonBuildAppendArgs, &_bbArray)) {          \
      bsonBuildError =                                                        \
         "Error while initializing child array: " _bsonDSL_str (__VA_ARGS__); \
   } else {                                                                   \
      _bsonBuildArray (_bbArray, __VA_ARGS__);                                \
      if (!bsonBuildError) {                                                  \
         if (!bson_append_array_end (bsonBuildContext.doc, &_bbArray)) {      \
            bsonBuildError =                                                  \
               "Error while finalizing child array: " _bsonDSL_str (          \
                  __VA_ARGS__);                                               \
         }                                                                    \
      } else {                                                                \
...
#define _bsonValueOperationDeferred_array(...)										\
	_bsonDSL_begin_part_1 ()														\
	bson_t _bbArray = BSON_INITIALIZER;												\
	_bsonDSL_begin_part_2 ("array(%s)", _bsonDSL_strElide (30, __VA_ARGS__));		\
	/* Write to this variable as the child array: */								\
	if (!bson_append_array_begin (_bsonBuildAppendArgs, &_bbArray)) {				\
		bsonBuildError =															\
			"Error while initializing child array: " _bsonDSL_str (__VA_ARGS__);	\
...
#define _bsonValueOperation_iterValue(Iter)                          \
   if (!bson_append_iter (_bsonBuildAppendArgs, &(Iter))) {          \
      bsonBuildError =                                               \
         "Error while appending iterValue(" _bsonDSL_str (Iter) ")"; \
   } else                                                            \
      ((void) 0)
...
#define _bsonValueOperation_bson(Doc)                                       \
   if (!bson_append_document (_bsonBuildAppendArgs, &(Doc))) {              \
      bsonBuildError =                                                      \
         "Error while appending subdocument: bson(" _bsonDSL_str (Doc) ")"; \
   } else                                                                   \
      ((void) 0)
...
#define _bsonValueOperation_value(Value)                          \
   ...
   if (!bson_append_value (_bsonBuildAppendArgs, &(Value))) {     \
      bsonBuildError =                                            \
         "Error while appending value(" _bsonDSL_str (Value) ")"; \
   }                                                              \
   _bsonDSL_end
...
#define _bsonDocOperation_insert(OtherBSON, Pred)										\
	_bsonDSL_begin_part_1()																\
	bool _bvHalt = false; /* Required for _bsonVisitEach() */							\
	_bsonDSL_begin_part_2 ("Insert other document: [%s]", _bsonDSL_str (OtherBSON));	\
	_bsonVisitEach (																	\
		OtherBSON,																		\
		if (Pred, then (do (_bsonDocOperation_iterElement (bsonVisitIter)))));			\
	_bsonDSL_end
...
#define _bsonDocOperation_insertFromIter(Iter, Pred)						\
	_bsonDSL_begin_part_1()													\
	bson_t _bbDocFromIter = _bson_dsl_iter_as_doc (&(Iter));				\
	_bsonDSL_begin_part_2 ("Insert document from iterator: [%s]",			\
					_bsonDSL_str (Iter));									\
	if (_bbDocFromIter.len == 0) {											\
		_bsonDSLDebug (														\
			"NOTE: Skipping insert of non-document value from iterator");	\
...
#define _bsonDocOperation_iterElement(Iter)                \
   _bsonDSL_begin ("Insert element from bson_iter_t [%s]", \
                   _bsonDSL_str (Iter));                   \
   bson_iter_t _bbIter = (Iter);                           \
   _bsonDocOperation_kvl (bson_iter_key (&_bbIter),        \
                          bson_iter_key_len (&_bbIter),    \
                          iterValue (_bbIter));            \
   _bsonDSL_end
...
#define _bsonDocOperation_iterElement(Iter)							\
	_bsonDSL_begin_part_1()											\
	bson_iter_t _bbIter = (Iter);									\
	_bsonDSL_begin_part_2 ("Insert element from bson_iter_t [%s]",	\
					_bsonDSL_str (Iter));							\
	_bsonDocOperation_kvl (bson_iter_key (&_bbIter),				\
							bson_iter_key_len (&_bbIter),			\
							iterValue (_bbIter));					\
...
#define _bsonVisitEach(Doc, ...)												\
	...
	do {																		\
		...
		BSON_MAYBE_UNUSED bool _bvContinue = false;								\
		/* Reset the context */													\
		struct _bsonVisitContext_t _bvCtx = {0};								\
		_bvCtx.doc = &(Doc);													\
		_bvCtx.parent = _bsonVisitContextThreadLocalPtr;						\
		_bvCtx.index = 0;														\
		_bsonVisitContextThreadLocalPtr = &_bvCtx;								\
		bsonParseError = NULL;													\
		/* Iterate over each element of the document */							\
...
#if defined(MONGOC_EX_C99_SUPPORTED)
...
#define _bsonPredicate_Condition_iKeyWithType(Key, Type)            \
   (_bsonPredicate_Condition_allOf _bsonDSL_nothing () (iKey (Key), \
                                                        type (Type)))
...
#define _bsonPredicate_Condition_iStrEqual(S) \
   (_bson_dsl_test_strequal (S, false))

#define _bsonPredicate_Condition_eq(Type, Value) \
   (_bsonPredicate_Condition_type (Type) && bsonAs (Type) == Value)
...
#define _bsonParseOperation_append \
   _bsonParseOperationDeferred_append _bsonDSL_nothing ()
...
#define _bsonBuildArray(BSON, ...)							\
	do {													\
		struct _bsonBuildContext_t _bbCtx = {0};			\
		_bsonDSL_disableWarnings ();						\
		_bbCtx.doc = &(BSON);                               \
		_bbCtx.parent = _bsonBuildContextThreadLocalPtr;    \
		_bbCtx.index = 0;                                   \
		_bsonBuildContextThreadLocalPtr = &_bbCtx;			\
		_bsonBuildArrayWithCurrentContext (__VA_ARGS__);	\
		_bsonBuildContextThreadLocalPtr = _bbCtx.parent;	\
...
#define _bsonBuildAppend(BSON, ...)												\
	...
	_bbCtx.parent = _bsonBuildContextThreadLocalPtr;							\
																				\
	_bsonDSL_begin_part_2 ("Appending to document '%s'", _bsonDSL_str (BSON));	\
	_bsonDSL_disableWarnings ();												\
	/* Reset the context */														\
	_bsonBuildContextThreadLocalPtr = &_bbCtx;									\
	bsonBuildError = NULL;														\
...
#define bsonBuild(BSON, ...)														\
	_bsonDSL_begin_part_1()															\
	bson_t *_bbDest = 0;															\
	_bsonDSL_begin_part_2 ("Build a new document for '%s'", _bsonDSL_str (BSON));	\
	/*bson_t **/_bbDest = &(BSON);													\
	bson_init (_bbDest);															\
	bsonBuildAppend (*_bbDest, __VA_ARGS__);										\
	_bsonDSL_end
...
{
	char *acc = 0;
	const struct _bsonVisitContext_t *ctx = 0;
	char *prev = 0;
...
#define _bsonDSL_strElide(MaxLen, ...)             \
   (strlen (_bsonDSL_str (__VA_ARGS__)) > (MaxLen) \
       ? "[...]"                                   \
