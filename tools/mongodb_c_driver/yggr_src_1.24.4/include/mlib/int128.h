...
//	//ret.r.hi = high
//	//return ret;
//	return MLIB_INT128_FROM_PARTS(low, high);
//}
//
//#endif // YGGR_EX_C99_SUPPORTED
...
static mlib_constexpr_fn struct mlib_int128_divmod_result _mlibDivide_u128_by_u64(const mlib_int128 numer,
	...
	mlib_int128_divmod_result ret = {{{0}}};

    mlib_int128 adjusted;
    int d = 0;
    ...
    if (d >= 32) {
		...
		ret.quotient = MLIB_INIT(mlib_int128) MLIB_INT128_FROM_PARTS(q1_q0, q3_q2);
		ret.remainder = MLIB_INIT(mlib_int128) MLIB_INT128_FROM_PARTS(rem, 0);
	...
	ret.quotient = MLIB_INIT(mlib_int128) MLIB_INT128_FROM_PARTS(quo, numer.r.hi / denom);
	ret.remainder = MLIB_INIT(mlib_int128) MLIB_INT128_FROM_PARTS(rem, 0);
...
static mlib_constexpr_fn mlib_int128_divmod_result mlib_int128_divmod(mlib_int128 numer, mlib_int128 denom) {
    ...
    if (dhi > nhi) {
        ...
        if (nhi == 0) {
            // Both high words are zero, so this is just a division of two 64bit
            // numbers
			ret.quotient = MLIB_INT128_CAST(nlo / dlo);
			ret.remainder = MLIB_INT128_CAST(nlo % dlo);
			return ret;
        } else if (nlo > dlo) {
            // The numerator is larger than the denom and the high word on the
            // denom is non-zero, so this cannot divide to anything greater than 1.
			ret.quotient = MLIB_INT128(1);
			ret.remainder = mlib_int128_sub(numer, denom);
			return ret;
        } else if (nlo < dlo) {
            // numer.r.lo < denom.r.lo and denom.r.hi > denom.r.lo, so the
            // integer division becomes zero
			ret.quotient =  MLIB_INT128(0);
			ret.remainder = numer;
			return ret;
        } else {
            // N / N is one
			ret.quotient =  MLIB_INT128(1);
			ret.remainder = MLIB_INT128(0);
			return ret;
        }
    } else if (dhi == 0) {
        // No high in denominator. We can use a u128/u64
       ...
       // uint32_t q[2] = {0};
		...
		ret.quotient =  MLIB_INT128_CAST(q[0] | (uint64_t)q[1] << 32);
...
static mlib_constexpr_fn mlib_int128 mlib_int128_pow10(uint8_t nth) {
	mlib_int128 zehn = MLIB_INT128(10);
    mlib_int128 r = MLIB_INT128(1);
    while (nth-- > 0) {
        r = mlib_int128_mul(r, zehn);
    }
    return r;
}
...
static mlib_constexpr_fn mlib_int128_charbuf mlib_int128_format(mlib_int128 i) {
	...
	mlib_int128_divmod_result dm = {{0}};
	uint64_t v = 0;
	char d = 0;
