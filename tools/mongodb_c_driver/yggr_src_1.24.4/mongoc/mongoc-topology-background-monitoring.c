									...
									mongoc_set_t *server_descriptions)
{
	size_t i = 0u;
	uint32_t i2 = 0u;
	mongoc_server_monitor_t *server_monitor = 0;
	uint32_t id = 0;
	...
	*/
	...
	/* Remove freed server monitors that have completed shutdown. */
	for (/*uint32_t*/ i2 = 0u; i2 < n_server_monitor_ids_to_remove; i2++) {
		mongoc_set_rm (server_monitors, server_monitor_ids_to_remove[i2]);
	}
	bson_free (server_monitor_ids_to_remove);
}
...
_mongoc_topology_background_monitoring_stop (mongoc_topology_t *topology)
{
	size_t n_srv_monitors = 0;
	size_t n_rtt_monitors = 0;
	size_t i = 0u;
