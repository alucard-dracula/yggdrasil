...
bool
_mongoc_cluster_time_greater (const bson_t *var_new, const bson_t *old)
{
   uint32_t new_t, new_i, old_t, old_i;

   if (!_mongoc_parse_cluster_time (var_new, &new_t, &new_i) ||
   ...
   const mongoc_client_session_t *session)
{
   ...
   assert(false);
   return (mongoc_transaction_state_t)(-1);
}
