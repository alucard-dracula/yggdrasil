							...
							char *str)                    /* OUT */
{
	...
	*/
	if (scientific_exponent < -6 || exponent > 0) {
		...
		*(str_out++) = 'E';
		// Truncation is OK.
		req = bson_snprintf (str_out, 6, "%+d", scientific_exponent);
		BSON_ASSERT (req > 0);
	} else {
		/* Regular format with no decimal place */
		if (exponent >= 0) {
			...
			*str_out = '\0';
			...
			if (radix_position > 0) { /* non-zero digits before radix */
				{
					int32_t i;
					for (i = 0; i < radix_position && (str_out - str) < available_bytes; i++) {
						*(str_out++) = *(significand_read++) + '0';
					}
				}
			} else { /* leading zero before radix point */
			...
			*(str_out++) = '0';
			...
			{
				uint32_t i;
				for (i = 0; mcommon_cmp_greater_us (significand_digits - i, BSON_MAX (radix_position - 1, 0)) &&
									(str_out - str) < available_bytes;
					i++) {
				*(str_out++) = *(significand_read++) + '0';
				}
...
#define SSCANF sscanf
      ...
      int64_t temp_exponent = 0;
      //int read_exponent = SSCANF (++str_read, "%" SCNd64 "%n", &temp_exponent, &nread);
	  int read_exponent = SSCANF (++str_read, "%d%n", &temp_exponent, &nread);
      str_read += nread;
