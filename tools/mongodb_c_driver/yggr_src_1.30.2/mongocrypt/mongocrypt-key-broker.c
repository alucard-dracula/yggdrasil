...
mongocrypt_kms_ctx_t *_mongocrypt_key_broker_next_kms(_mongocrypt_key_broker_t *kb) {
    ...
    if (kb->state == KB_AUTHENTICATING) {
		...
		{
			size_t i = 0;
			for (i = 0; i < mc_mapof_kmsid_to_authrequest_len(kb->auth_requests); i++) {
	...
	do
	{
		key_returned_t *ptr = 0;
		for (ptr = kb->keys_returned; ptr != NULL; ptr = ptr->next) {
