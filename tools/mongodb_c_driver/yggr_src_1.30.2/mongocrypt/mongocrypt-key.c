...
_mongocrypt_key_doc_t *_mongocrypt_key_new(void) {
	...
	memset(key_doc, 0, sizeof(_mongocrypt_key_doc_t));
    bson_init(&key_doc->bson);
...
_mongocrypt_key_alt_name_t *_mongocrypt_key_alt_name_new(const bson_value_t *value) {
	_mongocrypt_key_alt_name_t *name = 0;

    BSON_ASSERT_PARAM(value);

    name = BSON_ALIGNED_ALLOC(_mongocrypt_key_alt_name_t);
	memset(name, 0, sizeof(_mongocrypt_key_alt_name_t));
    BSON_ASSERT(name);
