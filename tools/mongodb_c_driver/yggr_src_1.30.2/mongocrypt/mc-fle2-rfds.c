...
parse_aggregate_expression(const bson_t *orig, bson_iter_t *in, operator_value_t *out, mongocrypt_status_t *status) {
	...
	do
	{
		operator_value_t args = {0};

		args.field = field;
		args.op_type_str = op_type_str;
		args.op_type = get_operator_type(op_type_str);
		args.value = value;
...
//        ERR_WITH_BSON(orig, "unsupported var_operator: %s", op_type_str);
//        goto fail;
//    }
//    value = document;
//
...
//        default:
//            ERR_WITH_BSON(in, "unsupported var_operator type %s", op.op_type_str);
//            goto fail;
//            break;
//        }
//
...
bool mc_FLE2RangeFindDriverSpec_parse(mc_FLE2RangeFindDriverSpec_t *spec,
	...
	memset(spec, 0, sizeof(mc_FLE2RangeFindDriverSpec_t));
...
//        bson_t var_operator;
//        TRY(BSON_APPEND_ARRAY_BEGIN(&elem, mc_FLE2RangeOperator_to_string(spec->firstOp), &var_operator));
//        TRY(BSON_APPEND_UTF8(&var_operator, "0", spec->field));
//        TRY(_mongocrypt_buffer_append(&p1, &var_operator, "1", 1));
//        TRY(bson_append_array_end(&elem, &var_operator));
//        TRY(bson_append_document_end(&and, &elem));
//
//        if (spec->nOps == 2) {
//            TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "1", &elem));
//            TRY(BSON_APPEND_ARRAY_BEGIN(&elem, mc_FLE2RangeOperator_to_string(spec->secondOp), &var_operator));
//            TRY(BSON_APPEND_UTF8(&var_operator, "0", spec->field));
//            TRY(_mongocrypt_buffer_append(&p2, &var_operator, "1", 1));
//            TRY(bson_append_array_end(&elem, &var_operator));
//            TRY(bson_append_document_end(&and, &elem));
//        }
//
...
//        TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "0", &elem));
//        bson_t var_operator;
//        TRY(BSON_APPEND_DOCUMENT_BEGIN(&elem, spec->field, &var_operator));
//        const char *op_str = mc_FLE2RangeOperator_to_string(spec->firstOp);
//        TRY(_mongocrypt_buffer_append(&p1, &var_operator, op_str, -1));
//        TRY(bson_append_document_end(&elem, &var_operator));
//        TRY(bson_append_document_end(&and, &elem));
//
//        if (spec->nOps == 2) {
//            TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "1", &elem));
...
//            TRY(_mongocrypt_buffer_append(&p2, &var_operator, op_str, -1));
//            TRY(bson_append_document_end(&elem, &var_operator));
//            TRY(bson_append_document_end(&and, &elem));
//        }
//
...
bool mc_FLE2RangeFindDriverSpec_to_placeholders(mc_FLE2RangeFindDriverSpec_t *spec,
	...
	mc_makeRangeFindPlaceholder_args_t args = {0};
...
#define TRY(stmt)                                                                                                      \
	...
	args.isStub = false;
    args.user_key_id = user_key_id;
    args.index_key_id = index_key_id;
    args.lowerBound = spec->lower.set ? spec->lower.value : negInf;
    args.lbIncluded = spec->lower.set ? spec->lower.included : true;
    args.upperBound = spec->upper.set ? spec->upper.value : posInf;
    args.ubIncluded = spec->upper.set ? spec->upper.included : true;
    args.payloadId = payloadId;
    args.firstOp = spec->firstOp;
    args.secondOp = spec->secondOp;
    args.indexMin = indexMin;
    args.indexMax = indexMax;
    args.precision = range_opts->precision;
    args.maxContentionFactor = maxContentionFactor;
    args.sparsity = range_opts->sparsity;
    args.trimFactor = range_opts->trimFactor;
    ...
    if (spec->isAggregateExpression) {
        ...
        */
        ...
        TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "0", &elem));
        TRY(BSON_APPEND_DOCUMENT_BEGIN(&elem, spec->field, &var_operator));
        op_str = mc_FLE2RangeOperator_to_string(spec->firstOp);
        ...
        if (spec->nOps == 2) {
            ...
            TRY(BSON_APPEND_DOCUMENT_BEGIN(&and, "1", &elem));
            TRY(BSON_APPEND_DOCUMENT_BEGIN(&elem, spec->field, &var_operator));
            op_str = mc_FLE2RangeOperator_to_string(spec->secondOp);
            TRY(_mongocrypt_buffer_append(&p2, &var_operator, op_str, -1));
            TRY(bson_append_document_end(&elem, &var_operator));
            TRY(bson_append_document_end(&and, &elem));
        }
...
void mc_reset_payloadId_for_testing(void) {
	int onle_once = 0;
    mlib_call_once(&payloadId_init_flag, payloadId_init_mutex);
    MONGOCRYPT_WITH_MUTEX(payloadId_mutex, onle_once) {
        payloadId = 0;
    }
}
...
int32_t mc_getNextPayloadId(void) {
    ...
    mlib_call_once(&payloadId_init_flag, payloadId_init_mutex);
    MONGOCRYPT_WITH_MUTEX(payloadId_mutex, only_once) {
        ret = payloadId;
        payloadId = payloadId < INT32_MAX ? payloadId + 1 : 0;
    }
    return ret;
