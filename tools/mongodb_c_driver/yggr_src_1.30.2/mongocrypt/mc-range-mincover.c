...
void mc_mincover_destroy(mc_mincover_t *mincover) {
    ...
    for (i = 0; i < mincover->mincover.len; i++) {
        char *val = _mc_array_index(&mincover->mincover, char *, i);
        bson_free(val);
    }
    _mc_array_destroy(&mincover->mincover);
...
mc_mincover_t *mc_getMincoverInt32(mc_getMincoverInt32_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_OSTType_Int32 a, b;
	MinCoverGenerator_u32 *mcg = 0;
	mc_mincover_t *mc = 0;

	mc_getTypeInfo32_args_t args_low = {0};
	mc_getTypeInfo32_args_t args_upp = {0};

    BSON_ASSERT_PARAM(status);
    CHECK_BOUNDS(args, PRId32, IDENTITY, LESSTHAN);
    //mc_OSTType_Int32 a, b;

	args_low.value = args.lowerBound;
	args_low.min = args.min;
	args_low.max = args.max;

	args_upp.value = args.upperBound;
	args_upp.min = args.min;
	args_upp.max = args.max;

    if (!mc_getTypeInfo32(args_low, &a, status)) {
        return NULL;
    }

    if (!mc_getTypeInfo32(args_upp, &b, status)) {
        return NULL;
    }
    ...
    mc = MinCoverGenerator_minCover_u32(mcg);
    mc->usedTrimFactor = MinCoverGenerator_usedTrimFactor_u32(mcg);
    MinCoverGenerator_destroy_u32(mcg);
    return mc;
}
...
mc_mincover_t *mc_getMincoverInt64(mc_getMincoverInt64_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_OSTType_Int64 a, b;
	MinCoverGenerator_u64 *mcg = 0;
	mc_mincover_t *mc = 0;

	mc_getTypeInfo64_args_t args_low = {0};
	mc_getTypeInfo64_args_t args_upp = {0};
	...
	if (!mc_getTypeInfo64(args_low, &a, status)) {
        return NULL;
    }
    if (!mc_getTypeInfo64(args_upp, &b, status)) {
        return NULL;
    }
    ...
    mc = MinCoverGenerator_minCover_u64(mcg);
    mc->usedTrimFactor = MinCoverGenerator_usedTrimFactor_u64(mcg);
    MinCoverGenerator_destroy_u64(mcg);
    return mc;
}
...
mc_mincover_t *mc_getMincoverDouble(mc_getMincoverDouble_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_getTypeInfoDouble_args_t args_low = {0};
	mc_getTypeInfoDouble_args_t args_upp = {0};
	...
	args_low.min = args.min;
	args_low.max = args.max;
	args_low.precision = args.precision;

	args_upp.value = args.upperBound;
	args_upp.min = args.min;
	args_upp.max = args.max;
	args_upp.precision = args.precision;


    if (!mc_getTypeInfoDouble(args_low, &a, status, use_range_v2)) {
        return NULL;
    }
    if (!mc_getTypeInfoDouble(args_upp, &b, status, use_range_v2)) {
        return NULL;
    }
...
mc_getMincoverDecimal128(mc_getMincoverDecimal128_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_OSTType_Decimal128 a, b;
	MinCoverGenerator_u128 *mcg = 0;
	mc_mincover_t *mc = 0;

	mc_getTypeInfoDecimal128_args_t args_low = {0};
	mc_getTypeInfoDecimal128_args_t args_upp = {0};
...
#define ToString(Dec) (mc_dec128_to_string(Dec).str)
    ...
    //mc_OSTType_Decimal128 a, b;
    if (!mc_getTypeInfoDecimal128(args_low, &a, status, use_range_v2)) {
        return NULL;
    }
    if (!mc_getTypeInfoDecimal128(args_upp, &b, status, use_range_v2)) {
        return NULL;
    }
    ...
    mc = MinCoverGenerator_minCover_u128(mcg);
    mc->usedTrimFactor = MinCoverGenerator_usedTrimFactor_u128(mcg);
    MinCoverGenerator_destroy_u128(mcg);
    return mc;
}
