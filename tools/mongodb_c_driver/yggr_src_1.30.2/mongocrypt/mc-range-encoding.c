...
bool mc_getTypeInfo32(mc_getTypeInfo32_args_t args, mc_OSTType_Int32 *out, mongocrypt_status_t *status) {
    ...
    // Convert to unbiased uint32. Then subtract the min value.
    v_u32 = encodeInt32(args.value);
    min_u32 = encodeInt32(args.min.value);
    max_u32 = encodeInt32(args.max.value);
...
bool mc_getTypeInfo64(mc_getTypeInfo64_args_t args, mc_OSTType_Int64 *out, mongocrypt_status_t *status) {
    ...
    // Convert to unbiased uint64. Then subtract the min value.
    v_u64 = encodeInt64(args.value);
    min_u64 = encodeInt64(args.min.value);
    max_u64 = encodeInt64(args.max.value);
...
bool ceil_log2_double(uint64_t i, uint32_t *maxBitsOut, mongocrypt_status_t *status) {
    ...
    clz = (uint32_t)_mlibCountLeadingZeros_u64(i);
    //uint32_t bits;
    if ((i & (i - 1)) == 0) {
        bits = 64 - clz - 1;
    } else {
...
{
	return mc_isfinite(d)? (double)((int64_t)d) : d;
}
...
bool mc_canUsePrecisionModeDouble(double min,
    ...
    scaled_prc = exp10Double(precision);

    scaled_max = max * scaled_prc;
    scaled_min = min * scaled_prc;
    ...
    t_1 = scaled_max - scaled_min;
    t_4 = (double)UINT64_MAX - t_1;
    ...
    i_1 = (int64_t)(scaled_max);
    i_2 = (int64_t)(scaled_min);

    range = subtract_int64_t(i_1, i_2);
...
bool mc_getTypeInfoDouble(mc_getTypeInfoDouble_args_t args,
    ...
    if (args.precision.set) {
        ...
        scaled = exp10Double(args.precision.value);
        if (!mc_isfinite(scaled)) {
            CLIENT_ERR("Precision is too large and cannot be used to calculate the scaled range bounds");
            return false;
        }
    ...
    if (use_precision_mode) {
        ...
        ret = (uint64_t)v_prime2;

        // Adjust maximum value to be the max bit range. This will be used by
        // getEdges/minCover to trim bits.
        max_value = (UINT64_C(1) << bits_range) - 1;
        BSON_ASSERT(ret <= max_value);
...
static mlib_int128 dec128_to_uint128(mc_dec128 dec) {
    ...
    ret = mc_dec128_coeff(dec);

    // Scale the resulting number by a power of ten matching the exponent of the
    // Decimal128:
    exp = ((int32_t)mc_dec128_get_biased_exp(dec)) - MC_DEC128_EXPONENT_BIAS;
    // We will scale up/down based on whether it is negative:
    BSON_ASSERT(abs(exp) <= UINT8_MAX);
    e1 = mlib_int128_pow10((uint8_t)abs(exp));
    if (exp < 0) {
        ret = mlib_int128_div(ret, e1);
    } else {
        ret = mlib_int128_mul(ret, e1);
...
bool ceil_log2_int128(mlib_int128 i, uint32_t *maxBitsOut, mongocrypt_status_t *status) {
    ...
    clz = (uint32_t)_mlibCountLeadingZeros_u128(i);
    //uint32_t bits;
...
bool mc_canUsePrecisionModeDecimal(mc_dec128 min,
	...
	mc_dec128 scaled_max;
    mc_dec128 scaled_min;

    mc_dec128 scaled_max_trunc;
    mc_dec128 scaled_min_trunc;
    ...
    scaled_max = mc_dec128_scale(max, precision);
    scaled_min = mc_dec128_scale(min, precision);

    scaled_max_trunc = mc_dec128_round_integral_ex(scaled_max, MC_DEC128_ROUND_TOWARD_ZERO, NULL);
    scaled_min_trunc = mc_dec128_round_integral_ex(scaled_min, MC_DEC128_ROUND_TOWARD_ZERO, NULL);
    ...
    t_1 = mc_dec128_sub(scaled_max, scaled_min);
    t_4 = mc_dec128_sub(UINT_128_MAX_AS_DECIMAL, t_1);

    // t_5 = floor(log10(t_4)) - 1;
    t_5 = mc_dec128_sub(mc_dec128_round_integral_ex(mc_dec128_log10(t_4), MC_DEC128_ROUND_TOWARD_ZERO, NULL),
                                  MC_DEC128(1));

    // We convert precision to a double so we can avoid warning C4146 on Windows.
    prc_dec = mc_dec128_from_double((double)precision);
    ...
    i_1 = dec128_to_int128(scaled_max);
    i_2 = dec128_to_int128(scaled_min);

    // Because we have guaranteed earlier that max is greater than min, we can
    // subtract these values and guarantee that taking their unsigned
    // representation will yield the actual range result.
    range128 = mlib_int128_sub(i_1, i_2);
    ...
    i_3 = mlib_int128_add(range128, mlib_int128_pow10((uint8_t)precision));
    if (!ceil_log2_int128(i_3, maxBitsOut, status)) {
        return false;
    }
...
bool mc_getTypeInfoDecimal128(mc_getTypeInfoDecimal128_args_t args,
    ...
    if (args.precision.set) {
        ...
        scaled = mc_dec128_scale(MC_DEC128(1), args.precision.value);
        if (!mc_dec128_is_finite(scaled)) {
            CLIENT_ERR("Precision is too large and cannot be used to calculate the scaled range bounds");
            return false;
        }
    ...
    if (use_precision_mode) {
        ...
        // Shift the number up
        // Returns: 3141.9
        valScaled = mc_dec128_scale(args.value, args.precision.value);

        // Round the number down
        // Returns 3141.0
        valTrunc = mc_dec128_round_integral_ex(valScaled, MC_DEC128_ROUND_TOWARD_ZERO, NULL);

        // Shift the number down
        // Returns: 31.41
        v_prime = mc_dec128_scale(valTrunc, -(int32_t)args.precision.value);

        // Adjust the number by the lower bound
        // Make it an integer by scaling the number
        //
        // Returns 3141.0
        v_prime2 = mc_dec128_scale(mc_dec128_sub(v_prime, args.min.value), args.precision.value);
        // Round the number down again. min may have a fractional value with more
        // decimal places than the precision (e.g. .001). Subtracting min may have
        // resulted in v_prime2 with a non-zero fraction. v_prime2 is expected to
        // have no fractional value when converting to int128.
        ...
        BSON_ASSERT(bits_range < 128);
        // Resulting OST maximum
        ost_max = mlib_int128_sub(mlib_int128_pow2((uint8_t)bits_range), i128_one);
        ...
        if (bits_range < 64) {
            ...
            if (flags.bits == 0) {
                // No error. It fits
				out->value = MLIB_INT128_CAST(as64);
				out->min = i128_zero;
				out->max = ost_max;
                return true;
            } else {
        ...
        u_ret = dec128_to_int128(v_prime2);
    ...
    // The coefficient of the number, without exponent/sign
    coeff = mc_dec128_coeff(args.value);

    if (mlib_int128_eq(coeff, i128_zero)) {
        // If the coefficient is zero, the result is encoded as the midpoint
        // between zero and 2^128-1
		out->value = i128_2pow127;
		out->min = i128_zero;
		out->max = MLIB_INT128_UMAX;
        return true;
    }

    // Coefficient is an unsigned value. We'll later scale our answer based on
    // the sign of the actual Decimal128
    isNegative = mc_dec128_is_negative(args.value);
    ...
    // The biased exponent from the decimal number. The paper refers to the
    // expression (e - e_min), which is the value of the biased exponent.
    exp_biased = mc_dec128_get_biased_exp(args.value);
