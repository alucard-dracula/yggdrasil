...
static mc_edges_t *mc_edges_new(const char *leaf,
	...
	trimFactor = trimFactorDefault(leaf_len, opt_trimFactor, use_range_v2);
	if (trimFactor != 0 && mc_cmp_greater_equal_su(trimFactor, leaf_len)) {
		// We append a total of leaf_len + 1 (for the root) - trimFactor edges. When this number is equal to 1, we
		// degenerate into equality, which is not desired, so trimFactor must be less than leaf_len.
		CLIENT_ERR("trimFactor must be less than the number of bits (%ld) used to represent an element of the domain, "
	...
	edges = bson_malloc0(sizeof(mc_edges_t));
	edges->usedTrimFactor = trimFactor;
	edges->sparsity = sparsity;
	_mc_array_init(&edges->edges, sizeof(char *));
	edges->leaf = bson_strdup(leaf);
	...
	leaf_copy = bson_strdup(leaf);
	_mc_array_append_val(&edges->edges, leaf_copy);

	// Start loop at max(trimFactor, 1). The full leaf is unconditionally appended after loop.
	BSON_ASSERT(mc_in_range_size_t_signed(trimFactor));
	trimFactor_sz = (size_t)trimFactor;
	startLevel = trimFactor > 0 ? trimFactor_sz : 1;

	do
	{
		size_t i = 0;
		for (i = startLevel; i < leaf_len; i++) {
			if (i % sparsity == 0) {
				char *edge = bson_malloc(i + 1);
				bson_strncpy(edge, leaf, i + 1);
				_mc_array_append_val(&edges->edges, edge);
...
mc_edges_t *mc_getEdgesInt32(mc_getEdgesInt32_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
    ...
    mc_OSTType_Int32 got;
	mc_getTypeInfo32_args_t args_in = {0};

	args_in.value = args.value;
	args_in.min = args.min;
	args_in.max = args.max;

    if (!mc_getTypeInfo32(args_in, &got, status)) {
        return NULL;
    }
    ...
    valueBin = mc_convert_to_bitstring_u32(got.value);
    offset = mc_count_leading_zeros_u32(got.max);
    leaf = valueBin.str + offset;
    ret = mc_edges_new(leaf, args.sparsity, args.trimFactor, status, use_range_v2);
    return ret;
}
...
mc_edges_t *mc_getEdgesInt64(mc_getEdgesInt64_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_getTypeInfo64_args_t args_in = {0};
	args_in.value = args.value;
	args_in.min = args.min;
	args_in.max = args.max;

    
    if (!mc_getTypeInfo64(args_in, &got, status)) {
        return NULL;
    }
    ...
    valueBin = mc_convert_to_bitstring_u64(got.value);
    offset = mc_count_leading_zeros_u64(got.max);
    leaf = valueBin.str + offset;
    ret = mc_edges_new(leaf, args.sparsity, args.trimFactor, status, use_range_v2);
    return ret;
}
...
mc_edges_t *mc_getEdgesDouble(mc_getEdgesDouble_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_getTypeInfoDouble_args_t args_in = {0};
	args_in.value = args.value;
    args_in.min = args.min;
    args_in.max = args.max;
    args_in.precision = args.precision;

    if (!mc_getTypeInfoDouble(args_in, &got, status, use_range_v2)) {
        return NULL;
    }
...
mc_edges_t *mc_getEdgesDecimal128(mc_getEdgesDecimal128_args_t args, mongocrypt_status_t *status, bool use_range_v2) {
	...
	mc_getTypeInfoDecimal128_args_t args_in = {0};
	args_in.value = args.value;
    args_in.min = args.min;
    args_in.max = args.max;
    args_in.precision = args.precision;

    if (!mc_getTypeInfoDecimal128(args_in, &got, status, use_range_v2)) {
        return NULL;
    }

    BSON_ASSERT(mlib_int128_eq(got.min, MLIB_INT128(0)));

    bits = mc_convert_to_bitstring_u128(got.value);
    offset = mc_count_leading_zeros_u128(got.max);
    leaf = bits.str + offset;
    ret = mc_edges_new(leaf, args.sparsity, args.trimFactor, status, use_range_v2);
    return ret;
}
