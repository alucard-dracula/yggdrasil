...
#define CHECK_AND_GOTO(x)                                                                                              \
	...
	subtype = (uint8_t)MC_SUBTYPE_FLE2IndexedEqualityEncryptedValue;
	...
	bson_type = (uint8_t)original_bson_type;
...
#define CHECK_AND_GOTO(x)                                                                                              \
	...
	const _mongocrypt_buffer_t *token_buf = 0;
    ...
    ciphertext_len = fle2alg->get_ciphertext_len(expected_buf_size, status);
    ...
    token_buf = mc_ServerDataEncryptionLevel1Token_get(token);
...
bool mc_FLE2IndexedEncryptedValue_add_S_Key(_mongocrypt_crypto_t *crypto,
	...
	ret = mc_FLE2IndexedEncryptedValue_decrypt(crypto, iev, token, NULL, status);
...
static bool mc_FLE2IndexedEncryptedValue_decrypt(_mongocrypt_crypto_t *crypto,
    ...
    /* Read ClientEncryptedValue. */
    expected_length = mc_reader_get_consumed_length(&reader) + length - 16;
    if (length > iev->Inner.len || expected_length > iev->Inner.len) {
        CLIENT_ERR("mc_FLE2IndexedEncryptedValue_decrypt expected "
                   "byte length >= %" PRIu64 " got: %" PRIu32,
                   expected_length,
