...
#define DERIVE_TOKEN_IMPL(Name)																							\
	static bool _fle2_derive_##Name##_token(_mongocrypt_crypto_t *crypto,												\
											...
											int64_t contentionFactor,													\
											mongocrypt_status_t *status) {												\
		mc_##Name##Token_t *token = 0;																					\
		mc_##Name##DerivedFromDataToken_t *fromDataToken = 0;															\
		mc_##Name##DerivedFromDataTokenAndContentionFactor_t *fromTokenAndContentionFactor = 0;							\
		BSON_ASSERT_PARAM(crypto);																						\
		BSON_ASSERT_PARAM(out);																							\
		BSON_ASSERT_PARAM(level1Token);																					\
		BSON_ASSERT_PARAM(value);																						\
...
#define DERIVE_TEXT_SEARCH_TOKEN_IMPL(Name, Type)																		\
	static bool _fle2_derive_##Name##Text##Type##_token(_mongocrypt_crypto_t *crypto,									\
														...
														mongocrypt_status_t *status) {									\
		mc_##Name##Token_t *token = 0;																					\
		mc_##Name##Text##Type##Token_t *textToken = 0;																	\
		mc_##Name##Text##Type##DerivedFromDataTokenAndContentionFactorToken_t *fromDataAndContentionFactor = 0;			\
																														\
		BSON_ASSERT_PARAM(crypto);																						\
		BSON_ASSERT_PARAM(out);																							\
		BSON_ASSERT_PARAM(level1Token);																					\
		...
		_mongocrypt_buffer_init(out);																					\
																														\
		token = mc_##Name##Token_new(crypto, level1Token, status);														\
		if (!token) {																									\
			return false;																								\
		}																												\
		textToken = mc_##Name##Text##Type##Token_new(crypto, token, status);											\
		mc_##Name##Token_destroy(token);																				\
		if (!textToken) {																								\
			return false;																								\
		}																												\
		fromDataAndContentionFactor =																					\
			mc_##Name##Text##Type##DerivedFromDataTokenAndContentionFactorToken_new(crypto,								\
																					textToken,							\
																					value,								\
																					(uint64_t)contentionFactor,			\
...
#define DERIVE_TEXT_SEARCH_SERVER_DERIVED_FROM_DATA_TOKEN_IMPL(Type)													\
	static bool _fle2_derive_serverText##Type##DerivedFromDataToken(													\
		...
		mongocrypt_status_t *status) {																					\
																														\
		mc_ServerText##Type##Token_t *token = 0;																		\
		mc_ServerText##Type##DerivedFromDataToken_t *dataToken = 0;														\
																														\
		BSON_ASSERT_PARAM(crypto);																						\
		BSON_ASSERT_PARAM(out);																							\
		BSON_ASSERT_PARAM(level1Token);																					\
		...
		BSON_ASSERT_PARAM(status);																						\
																														\
		_mongocrypt_buffer_init(out);																					\
		token = mc_ServerText##Type##Token_new(crypto, level1Token, status);											\
		if (!token) {																									\
			return false;																								\
		}																												\
		dataToken =																										\
...
static bool _fle2_derive_serverDerivedFromDataToken(_mongocrypt_crypto_t *crypto,
                                                    ...
                                                    const _mongocrypt_buffer_t *value,
                                                    mongocrypt_status_t *status) {
	mc_ServerDerivedFromDataToken_t *token = 0;
	...
	token = mc_ServerDerivedFromDataToken_new(crypto, level1Token, value, status);
    if (!token) {
        return false;
    }
...
static bool _fle2_placeholder_aes_ctr_encrypt(_mongocrypt_crypto_t *crypto,
	...
	//_mongocrypt_buffer_t iv;
	cipherlen = fle2alg->get_ciphertext_len(in->len, status);
	if (cipherlen == 0) {
		return false;
	}
	//uint32_t written = 0;
...
static bool _fle2_placeholder_aes_aead_encrypt(_mongocrypt_key_broker_t *kb,
	...
	crypto = kb->crypt->crypto;
	//_mongocrypt_buffer_t iv, key;
	cipherlen = algorithm->get_ciphertext_len(in->len, status);
	if (cipherlen == 0) {
		return false;
	}
	//uint32_t written = 0;
...
static bool _fle2_derive_encrypted_token(_mongocrypt_crypto_t *crypto,
	...
	if (!eccDerivedToken) {
		// FLE2v2
		if (use_range_v2 && is_leaf.set) {
			_mongocrypt_buffer_t tokens[2] = {{0}};
			// Range V2; concat isLeaf
			_mongocrypt_buffer_t isLeafBuf;
			const uint8_t* is_leaf_val_buf = (const uint8_t*)&is_leaf.value;
			//if (!_mongocrypt_buffer_copy_from_data_and_size(&isLeafBuf, (uint8_t[]){is_leaf.value}, 1)) {
			if (!_mongocrypt_buffer_copy_from_data_and_size(&isLeafBuf, is_leaf_val_buf, 1)) {
				CLIENT_ERR("failed to create is_leaf buffer");
				goto fail;
			}

			tokens[0] = *escDerivedToken;
			tokens[1] = isLeafBuf;
...
static void _FLE2EncryptedPayloadCommon_cleanup(_FLE2EncryptedPayloadCommon_t *common) {
	...
	_mongocrypt_buffer_cleanup(&common->serverDerivedFromDataToken);
	// Zero out memory so `_FLE2EncryptedPayloadCommon_cleanup` is safe to call twice.
	memset(common, 0, sizeof(_FLE2EncryptedPayloadCommon_t));
}
...
static bool _mongocrypt_fle2_placeholder_common(_mongocrypt_key_broker_t *kb,
	...
	crypto = kb->crypt->crypto;
    //*ret = (_FLE2EncryptedPayloadCommon_t){{0}};
	memset(ret, 0, sizeof(_FLE2EncryptedPayloadCommon_t));
...
static bool _mongocrypt_fle2_placeholder_to_insert_update_common(_mongocrypt_key_broker_t *kb,
    ...
    // Or in Range V2, when using range: p := EncryptCTR(ECOCToken, ESCDerivedFromDataTokenAndContentionFactor || 0x00)
	do {
		const mc_optional_bool_t ob_false = OPT_BOOL(false);
		const mc_optional_bool_t ob_failed = {0};
		if (!_fle2_derive_encrypted_token(
				crypto,
				&out->encryptedTokens,
				kb->crypt->opts.use_range_v2,
				...
				NULL, // unused in v2
				// If this is a range insert, we append isLeaf to the encryptedTokens. Otherwise, we don't.
				placeholder->algorithm == MONGOCRYPT_FLE2_ALGORITHM_RANGE ? ob_false : ob_failed,
				status)) {
			goto fail;
		}
	} while(0);
    ...
    // v := UserKeyId + EncryptCBCAEAD(UserKey, value)
    {
		...
		v[0] = placeholder->user_key_id;
		v[1] = ciphertext;

		ok = _mongocrypt_buffer_concat(&out->value, v, 2);
...
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertext(_mongocrypt_key_broker_t *kb,
	...
	mc_FLE2EncryptionPlaceholder_t *placeholder = 0;
    _FLE2EncryptedPayloadCommon_t common = {{0}};
    mc_FLE2InsertUpdatePayloadV2_t payload;
    bool res = false;
...
get_edges(mc_FLE2RangeInsertSpec_t *insertSpec, size_t sparsity, mongocrypt_status_t *status, bool use_range_v2) {
    ...
    value_type = bson_iter_type(&insertSpec->v);
	...
	case BSON_TYPE_INT32:
		{
			mc_getEdgesInt32_args_t args = {
				bson_iter_int32(&insertSpec->v),
				OPT_I32(bson_iter_int32(&insertSpec->min)),
				OPT_I32(bson_iter_int32(&insertSpec->max)),
				sparsity,
				{ insertSpec->trimFactor.set, insertSpec->trimFactor.value} };

			return mc_getEdgesInt32(args, status, use_range_v2);
		}
	case BSON_TYPE_INT64:
		{
			mc_getEdgesInt64_args_t args = {
				bson_iter_int64(&insertSpec->v),
				OPT_I64(bson_iter_int64(&insertSpec->min)),
				OPT_I64(bson_iter_int64(&insertSpec->max)),
				sparsity,
				{ insertSpec->trimFactor.set, insertSpec->trimFactor.value } };

			return mc_getEdgesInt64(args, status, use_range_v2);
		}
	case BSON_TYPE_DATE_TIME:
		{
			mc_getEdgesInt64_args_t args = {
				bson_iter_date_time(&insertSpec->v),
				OPT_I64(bson_iter_int64(&insertSpec->min)),
				OPT_I64(bson_iter_int64(&insertSpec->max)),
				sparsity,
				{ insertSpec->trimFactor.set, insertSpec->trimFactor.value } };
	...
	case BSON_TYPE_DOUBLE:
		{
			...
			if (insertSpec->precision.set) 
			{
				...
				// or all three must be unset in mc_getTypeInfoDouble.
				args.min.set = true;
				args.min.value = bson_iter_double(&insertSpec->min);
				args.max.set = true;
				args.max.value = bson_iter_double(&insertSpec->max);
				args.precision = insertSpec->precision;
			}
...
#	if MONGOCRYPT_HAVE_DECIMAL128_SUPPORT
			const mc_dec128 value = mc_dec128_from_bson_iter(&insertSpec->v);
			mc_getEdgesDecimal128_args_t args;
			memset(&args, 0, sizeof(mc_getEdgesDecimal128_args_t));
			args.value = value;
			args.sparsity = sparsity;
			args.trimFactor = insertSpec->trimFactor;
...
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertextForRange(_mongocrypt_key_broker_t *kb,
	...
	use_range_v2 = kb->crypt->opts.use_range_v2;
	...
	// g:= array<EdgeTokenSetV2>
	{
		...
		{
			size_t i = 0;
			for (i = 0; i < mc_edges_len(edges); ++i) {
				// Create an EdgeTokenSet from each edge.
				bool loop_ok = false;
				const char *edge = mc_edges_get(edges, i);
				bool is_leaf = mc_edges_is_leaf(edges, edge);
				...
				_mongocrypt_buffer_t encryptedTokens = {0};
				mc_EdgeTokenSetV2_t etc = {{0}};
				mc_optional_bool_t is_leaf_arg = OPT_BOOL(is_leaf);
	...
	// Include "range" payload fields introduced in SERVER-91889.
	payload.sparsity.set = true;
	payload.sparsity.value = placeholder->sparsity;
	payload.precision = insertSpec.precision;
	payload.trimFactor.set = true;
	payload.trimFactor.value = mc_edges_get_used_trimFactor(edges);
	bson_value_copy(bson_iter_value(&insertSpec.min), &payload.indexMin);
	bson_value_copy(bson_iter_value(&insertSpec.max), &payload.indexMax);
...
#define GENERATE_TEXT_SEARCH_TOKEN_SET_FOR_TYPE_IMPL(Type)																\
	static bool _fle2_generate_Text##Type##TokenSet(_mongocrypt_key_broker_t *kb,										\
													...
													const mc_ServerTokenDerivationLevel1Token_t *serverLevel1Token,		\
													mongocrypt_status_t *status) {										\
		const mc_optional_bool_t opt_bool_failed = {0};																	\
																														\
		BSON_ASSERT_PARAM(kb);																							\
		BSON_ASSERT_PARAM(kb->crypt);																					\
		BSON_ASSERT_PARAM(out);																							\
...
static bool _fle2_generate_TextSearchTokenSets(_mongocrypt_key_broker_t *kb,
	...
	_mongocrypt_crypto_t *crypto = 0;
	mc_TextSearchTokenSets_t *tsts = 0;
	_FLE2EncryptedPayloadCommon_t common = {{0}};
	bool res = false;

	mc_str_encode_sets_t *encodeSets = 0;
	...
	crypto = kb->crypt->crypto;
	tsts = &payload->textSearchTokenSets.tsts;
	//_FLE2EncryptedPayloadCommon_t common = {{0}};
	//bool res = false;

	encodeSets = mc_text_search_str_encode(spec, status);
	if (!encodeSets) {
		goto fail;
	}
...
static bool _mongocrypt_fle2_placeholder_to_insert_update_ciphertextForTextSearch(_mongocrypt_key_broker_t *kb,
	...
	mc_optional_bool_t opt_bool_failed = {0};
	...
	do {
		...
		do {
			bool ok = false;
			_mongocrypt_buffer_t v[2];
			v[0] = placeholder->user_key_id;
			v[1] = ciphertext;

			ok = _mongocrypt_buffer_concat(&payload.value, v, 2);
			_mongocrypt_buffer_cleanup(&ciphertext);
			if (!ok) {
				goto fail;
			}
...
mc_mincover_t *mc_get_mincover_from_FLE2RangeFindSpec(mc_FLE2RangeFindSpec_t *findSpec,
	...
	bson_type_t bsonType = BSON_TYPE_EOD;
	...
	bsonType = bson_iter_type(&findSpec->edgesInfo.value.indexMin);
	...
	lowerBound = findSpec->edgesInfo.value.lowerBound;
	upperBound = findSpec->edgesInfo.value.upperBound;
	includeLowerBound = findSpec->edgesInfo.value.lbIncluded;
	includeUpperBound = findSpec->edgesInfo.value.ubIncluded;
	...
	case BSON_TYPE_INT32:
		{
			mc_getMincoverInt32_args_t args = {0};
			...
			args.lowerBound = bson_iter_int32(&lowerBound);
			args.includeLowerBound = includeLowerBound;
			args.upperBound = bson_iter_int32(&upperBound);
			args.includeUpperBound = includeUpperBound;
			args.min.set = true;
			args.min.value = bson_iter_int32(&findSpec->edgesInfo.value.indexMin);
			args.max.set = true;
			args.max.value = bson_iter_int32(&findSpec->edgesInfo.value.indexMax);
			args.sparsity = sparsity;
			args.trimFactor = findSpec->edgesInfo.value.trimFactor;

			return mc_getMincoverInt32(args, status, use_range_v2);
		}

	case BSON_TYPE_INT64:
		{
			mc_getMincoverInt64_args_t args = {0};
			...
			args.lowerBound = bson_iter_int64(&lowerBound);
			args.includeLowerBound = includeLowerBound;
			args.upperBound = bson_iter_int64(&upperBound);
			args.includeUpperBound = includeUpperBound;
			args.min.set = true;
			args.min.value = bson_iter_int64(&findSpec->edgesInfo.value.indexMin);
			args.max.set = true;
			args.max.value = bson_iter_int64(&findSpec->edgesInfo.value.indexMax);
			args.sparsity = sparsity;
			args.trimFactor = findSpec->edgesInfo.value.trimFactor;

			return mc_getMincoverInt64(args, status, use_range_v2);
		}
	case BSON_TYPE_DATE_TIME:
		{
			mc_getMincoverInt64_args_t args = {0};
			...
			args.lowerBound = bson_iter_date_time(&lowerBound);
			args.includeLowerBound = includeLowerBound;
			args.upperBound = bson_iter_date_time(&upperBound);
			args.includeUpperBound = includeUpperBound;
			args.min.set = true;
			args.min.value = bson_iter_date_time(&findSpec->edgesInfo.value.indexMin);
			args.max.set = true;
			args.max.value = bson_iter_date_time(&findSpec->edgesInfo.value.indexMax);
			args.sparsity = sparsity;
			args.trimFactor = findSpec->edgesInfo.value.trimFactor;
	...
	case BSON_TYPE_DOUBLE: 
		{
			mc_getMincoverDouble_args_t args = {0};
			...
			args.lowerBound = bson_iter_double(&lowerBound);
			args.includeLowerBound = includeLowerBound;
			args.upperBound = bson_iter_double(&upperBound);
			args.includeUpperBound = includeUpperBound;
			args.sparsity = sparsity;
			args.trimFactor = findSpec->edgesInfo.value.trimFactor;
			...
			if (findSpec->edgesInfo.value.precision.set) 
			{
				...
				// or all three must be unset in mc_getTypeInfoDouble.
				args.min.set = true;
				args.min.value = bson_iter_double(&findSpec->edgesInfo.value.indexMin);
				args.max.set = true;
				args.max.value = bson_iter_double(&findSpec->edgesInfo.value.indexMax);
				args.precision = findSpec->edgesInfo.value.precision;
			}
...
#	if MONGOCRYPT_HAVE_DECIMAL128_SUPPORT
		...
		mc_getMincoverDecimal128_args_t args = {0};
		...
		args.lowerBound = mc_dec128_from_bson_iter(&lowerBound);
		args.includeLowerBound = includeLowerBound;
		args.upperBound = mc_dec128_from_bson_iter(&upperBound);
		args.includeUpperBound = includeUpperBound;
		args.sparsity = sparsity;
		args.trimFactor = findSpec->edgesInfo.value.trimFactor;
		...
		{
			args.min.set = true;
			args.min.value = mc_dec128_from_bson_iter(&findSpec->edgesInfo.value.indexMin);
			args.max.set = true;
			args.max.value = mc_dec128_from_bson_iter(&findSpec->edgesInfo.value.indexMax);
			args.precision = findSpec->edgesInfo.value.precision;
		}
...
static bool _mongocrypt_fle2_placeholder_to_find_ciphertextForRange(_mongocrypt_key_broker_t *kb,
	...
	if (findSpec.edgesInfo.set) {
		...
		// g:= array<EdgeFindTokenSet>
		{
			...
			do {
				size_t i = 0;
				for (i = 0; i < mc_mincover_len(mincover); i++) {
					// Create a EdgeFindTokenSet from each edge.
					bool loop_ok = false;
					const char *edge = mc_mincover_get(mincover, i);
					_mongocrypt_buffer_t edge_buf = {0};
		...
		if (use_range_v2) {
			...
			payload.precision = findSpec.edgesInfo.value.precision;
			payload.trimFactor.set = true;
			payload.trimFactor.value = mc_mincover_get_used_trimFactor(mincover);
			bson_value_copy(bson_iter_value(&findSpec.edgesInfo.value.indexMin), &payload.indexMin);
			bson_value_copy(bson_iter_value(&findSpec.edgesInfo.value.indexMax), &payload.indexMax);
		}
	}
