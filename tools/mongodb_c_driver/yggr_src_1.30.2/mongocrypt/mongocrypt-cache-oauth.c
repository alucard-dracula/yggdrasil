...
char *mc_mapof_kmsid_to_token_get_token(mc_mapof_kmsid_to_token_t *k2t, const char *kmsid) {
    ...
    for (i = 0; i < k2t->entries.len; i++) {
        ...
        if (0 == strcmp(k2te.kmsid, kmsid)) {
            ...
            access_token = bson_strdup(k2te.access_token);
            _mongocrypt_mutex_unlock(&k2t->mutex);
            return access_token;
        }
    }
...
bool mc_mapof_kmsid_to_token_add_response(mc_mapof_kmsid_to_token_t *k2t,
	...
	int64_t expiration_time_us;
	mc_mapof_kmsid_to_token_entry_t to_put = {0};
	...
	to_put.kmsid = bson_strdup(kmsid);
    to_put.access_token = bson_strdup(access_token);
    to_put.expiration_time_us = expiration_time_us;
