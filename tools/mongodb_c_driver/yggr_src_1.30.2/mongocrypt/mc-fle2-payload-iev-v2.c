...
bool mc_FLE2IndexedEncryptedValueV2_add_S_Key(_mongocrypt_crypto_t *crypto,
	...
	_mongocrypt_buffer_t TokenKey;
	mc_ServerDataEncryptionLevel1Token_t *token = 0;

	bool ret = false;
    const _mongocrypt_value_encryption_algorithm_t *fle2alg = 0;
    uint32_t DecryptedServerEncryptedValueLen = 0;
    ...
    /* Use TokenKey to create ServerDataEncryptionLevel1Token and decrypt
     * ServerEncryptedValue into ClientEncryptedValue */
    token = mc_ServerDataEncryptionLevel1Token_new(crypto, &TokenKey, status);
    if (!token) {
        return false;
    }

    //bool ret = false;
    fle2alg = _mcFLE2Algorithm();
    DecryptedServerEncryptedValueLen = fle2alg->get_plaintext_len(iev->ServerEncryptedValue.len, status);
    if (!mongocrypt_status_ok(status)) {
        goto fail;
    }
    if (DecryptedServerEncryptedValueLen <= UUID_LEN) {
...
bool mc_FLE2IndexedEncryptedValueV2_add_K_Key(_mongocrypt_crypto_t *crypto,
    ...
    /* Attempt to decrypt ClientEncryptedValue */
    ClientValueLen = fle2v2aead->get_plaintext_len(iev->ClientEncryptedValue.len, status);
    if (!mongocrypt_status_ok(status)) {
        return false;
    }
...
bool mc_FLE2IndexedEncryptedValueV2_parse(mc_FLE2IndexedEncryptedValueV2_t *iev,
	...
	uint64_t min_required_len = 0;
	uint64_t SEV_and_metadata_len = 0;
	uint64_t SEV_len = 0;
	...
	// Maximum edge_count(4294967295) times kMetadataLen(96) fits easily without
	// overflow.
	metadata_len = iev->edge_count * kMetadataLen;

	/* Read ServerEncryptedValue. */
	min_required_len = kMinServerEncryptedValueLen + metadata_len;
	SEV_and_metadata_len = mc_reader_get_remaining_length(&reader);
	if (SEV_and_metadata_len < min_required_len) {
		CLIENT_ERR("Invalid payload size %" PRIu64 ", smaller than minimum length %" PRIu64,
					SEV_and_metadata_len,
					min_required_len);
	...
	SEV_len = SEV_and_metadata_len - metadata_len;
	CHECK_AND_RETURN(mc_reader_read_buffer(&reader, &iev->ServerEncryptedValue, SEV_len, status));
...
static MONGOC_INLINE uint32_t mc_FLE2IndexedEncryptedValueV2_serialized_length(const mc_FLE2IndexedEncryptedValueV2_t *iev) {
...
bool mc_FLE2IndexedEncryptedValueV2_serialize(const mc_FLE2IndexedEncryptedValueV2_t *iev,
	...
	expected_len = mc_FLE2IndexedEncryptedValueV2_serialized_length(iev);
	//mc_writer_t writer;
	_mongocrypt_buffer_resize(buf, expected_len);
	mc_writer_init_from_buffer(&writer, buf, __FUNCTION__);
