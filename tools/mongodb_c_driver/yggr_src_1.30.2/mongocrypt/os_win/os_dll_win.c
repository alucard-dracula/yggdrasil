...
mcr_dll mcr_dll_open(const char *filepath_) {
	...
	wide = mstr_win32_widen(filepath.view);
	mstr_free(filepath);
	if (wide.error) {
		ret.error_string = merror_system_error_string(wide.error);
		return ret;
	}
	lib = LoadLibraryW(wide.wstring);
	if (lib == NULL) {
		ret.error_string = merror_system_error_string(GetLastError());
		return ret;
	}
	free(wide.wstring);

	ret._native_handle = lib;
	return ret;
}
...
mcr_dll_path_result mcr_dll_path(mcr_dll dll) {
	...
	int ret_error = 0;
	DWORD acc_size = 512;
	mcr_dll_path_result ret = {MSTR_NULL};
	...
	ret.path = ret_str;
	ret.error_string = merror_system_error_string(ret_error);
