...
bool
_mongoc_cluster_time_greater (const bson_t *new_, const bson_t *old)
{
   uint32_t new_t, new_i, old_t, old_i;

   if (!_mongoc_parse_cluster_time (new_, &new_t, &new_i) || !_mongoc_parse_cluster_time (old, &old_t, &old_i)) {
      return false;
   }
                                         ...
                                         bson_error_t *error)
{
   mongoc_server_stream_t *server_stream = NULL;
   bool ret;
   const mongoc_ss_log_context_t ss_log_context = {"mongoc_client_session_start_transaction"};
...
mongoc_client_session_get_transaction_state (const mongoc_client_session_t *session)
{
	...
	RETURN (MONGOC_TRANSACTION_ABORTED); // fix warning c4715
}
