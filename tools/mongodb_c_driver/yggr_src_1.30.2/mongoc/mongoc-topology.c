...
mongoc_topology_reconcile (const mongoc_topology_t *topology, mongoc_topology_description_t *td)
{
	...
	{
		size_t i = 0u;
		for (i = 0u; i < servers->items_len; i++) {
			sd = mongoc_set_get_item (servers, i);
			_mongoc_topology_reconcile_add_nodes (sd, topology->scanner);
		}
	} while(0);
...
_server_session_should_prune (const void *session_vp, void *topo_vp)
{
	const mongoc_server_session_t *session = 0;
	mongoc_topology_t *topo = 0;
...
_mongoc_apply_srv_max_hosts (const mongoc_host_list_t *hl, size_t max_hosts, size_t *hl_array_size)
{
	...
	hl_size = _mongoc_host_list_length (hl);
...
mongoc_topology_new (const mongoc_uri_t *uri, bool single_threaded)
{
   ...
   int32_t heartbeat_default = 0;
   int32_t heartbeat = 0;
   size_t hl_array_size = 0u;

   const mongoc_host_list_t **hl_array = 0;
   ...
   heartbeat_default = single_threaded ? MONGOC_TOPOLOGY_HEARTBEAT_FREQUENCY_MS_SINGLE_THREADED
                                                     : MONGOC_TOPOLOGY_HEARTBEAT_FREQUENCY_MS_MULTI_THREADED;

   heartbeat = mongoc_uri_get_option_as_int32 (uri, MONGOC_URI_HEARTBEATFREQUENCYMS, heartbeat_default);
   ...
   if (srv_hostname) {
      ...
      topology->srv_polling_last_scan_ms = bson_get_monotonic_time () / 1000;
      /* TODO (CDRIVER-4047) use BSON_MIN */
      new_iv = BSON_MAX (rr_data.min_ttl * 1000, MONGOC_TOPOLOGY_MIN_RESCAN_SRV_INTERVAL_MS);
      _mongoc_topology_set_srv_polling_rescan_interval_ms (topology, new_iv);
   ...
   BSON_ASSERT (mcommon_in_range_signed (size_t, td->max_hosts));
   hl_array = _mongoc_apply_srv_max_hosts (hl, (size_t) td->max_hosts, &hl_array_size);
...
//   {
//      /* Prefer mc_tpld_unsafe_get_const to mc_tpld_take_ref/drop_ref here: no other references remain, and bypassing
//       * the shared pointer lock has the side-effect of revealing problems in TSAN. */
//      const mongoc_topology_description_t *td = mc_tpld_unsafe_get_const (topology);
//
...
//         _mongoc_topology_description_monitor_server_closed (td, &topology->log_and_monitor, sd);
//      }
//
	...
	* all servers and transition to an unknown topology. */
	{
		...
		* the shared pointer lock has the side-effect of revealing problems in TSAN. */
		...
		mongoc_topology_description_t next_td;
		size_t i = 0u;
		for (i = 0u; i < mc_tpld_servers_const (td)->items_len; i++) {
			const mongoc_server_description_t *sd = mongoc_set_get_item_const (mc_tpld_servers_const (td), i);
			_mongoc_topology_description_monitor_server_closed (td, &topology->log_and_monitor, sd);
		}
...
mongoc_topology_compatible (const mongoc_topology_description_t *td,
							...
							bson_error_t *error)
{
	int64_t max_staleness_seconds = 0;
