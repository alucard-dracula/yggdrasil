...
static const struct {
   const char *name;
   mongoc_structured_log_level_t level;
} gStructuredLogLevelAliases[] = {{"off", (mongoc_structured_log_level_t) 0},
                                  {"warn", MONGOC_STRUCTURED_LOG_LEVEL_WARNING},
                                  {"info", MONGOC_STRUCTURED_LOG_LEVEL_INFO}};
...
{
	bson_t *bson = 0;
	const mongoc_structured_log_builder_stage_t *stage = 0;
	const mongoc_structured_log_opts_t *opts = 0;

	BSON_ASSERT_PARAM (entry);
	bson = bson_new ();
	BSON_APPEND_UTF8 (bson, "message", entry->envelope.message);
	stage = entry->builder;
	opts = &entry->envelope.instance->opts;
	while (stage->func) {
		stage = stage->func (bson, stage, opts);
	}
	return bson;
																...
																mongoc_structured_log_level_t level)
{
	...
	{
		int component = 0;
		for (component = 0; component < STRUCTURED_LOG_COMPONENT_TABLE_SIZE; component++) {
			if (!mongoc_structured_log_opts_set_max_level_for_component (
					opts, (mongoc_structured_log_component_t) component, level)) {
				// Fine to stop on the first error, always means 'level' is wrong and none of these will succeed.
				return false;
												...
												int volatile *err_flag_atomic)
{
	...
	result = mongoc_structured_log_get_named_level (level, out);
	if (!result) {
		// Only log the first instance of each error per process
		if (0 == mcommon_atomic_int_compare_exchange_strong (err_flag_atomic, 0, 1, mcommon_memory_order_seq_cst)) {
			MONGOC_WARNING ("Invalid log level '%s' read from environment variable %s. Ignoring it.", level, variable);
...
mongoc_structured_log_get_named_component (const char *name, mongoc_structured_log_component_t *out)
{
	...
	table_size = sizeof gStructuredLogComponentNames / sizeof gStructuredLogComponentNames[0];
	for (table_index = 0; table_index < table_size; table_index++) {
		if (!strcasecmp (name, gStructuredLogComponentNames[table_index])) {
			*out = (mongoc_structured_log_component_t) table_index;
			return true;
...
{
	const char *variable = "MONGODB_LOG_MAX_DOCUMENT_LENGTH";
	char *max_length_str = 0;
	bool result = false;

	BSON_ASSERT_PARAM (opts);

	//const char *variable = "MONGODB_LOG_MAX_DOCUMENT_LENGTH";
	max_length_str = _mongoc_getenv (variable);
...
_mongoc_structured_log_default_handler (const mongoc_structured_log_entry_t *entry, void *user_data)
{
	mongoc_structured_log_instance_t *instance = 0;
	bson_t *bson_message = 0;
	char *json_message = 0;
	...
	instance = entry->envelope.instance;

	// We can serialize the message before taking the default_handler_shared mutex
	bson_message = mongoc_structured_log_entry_message_as_bson (entry);
	json_message = bson_as_relaxed_extended_json (bson_message, NULL);
	bson_destroy (bson_message);

	level_name = mongoc_structured_log_get_level_name (mongoc_structured_log_entry_get_level (entry));
	component_name =
		mongoc_structured_log_get_component_name (mongoc_structured_log_entry_get_component (entry));
																...
																const mongoc_structured_log_opts_t *opts)
{
	...
	do
	{
		...
		for (i = 0; i < cmd->payloads_count; i++) {
			const uint8_t *doc_begin = 0;
			const uint8_t *doc_end = 0;
			const uint8_t *doc_ptr = 0;
			int32_t doc_len = 0;
			...
			doc_begin = cmd->payloads[i].documents;
			BSON_ASSERT (doc_begin);
			doc_end = doc_begin + cmd->payloads[i].size;
			BSON_ASSERT (doc_begin != doc_end);
...
_mongoc_structured_log_append_utf8 (bson_t *bson,
									...
									const mongoc_structured_log_opts_t *opts)
{
	const char *key_or_null = 0;
	BSON_UNUSED (opts);
	key_or_null = stage->arg1.utf8;
	if (key_or_null) {
		bson_append_utf8 (bson, key_or_null, -1, stage->arg2.utf8, -1);
	}
	return stage + 1;
												...
												const mongoc_structured_log_opts_t *opts)
{
	...
	BSON_UNUSED (opts);
	BSON_ASSERT (stage[1].func == _mongoc_structured_log_append_utf8_n_stage1);
	key_or_null = stage[0].arg1.utf8;
	key_len = stage[0].arg2.int32;
	value = stage[1].arg1.utf8;
	value_len = stage[1].arg2.int32;
	if (key_or_null) {
		bson_append_utf8 (bson, key_or_null, key_len, value, value_len);
	}
	return stage + 2;
...
{
	const char *key_or_null = 0;
	const bson_oid_t *oid_or_null = 0;

	BSON_UNUSED (opts);
	key_or_null = stage->arg1.utf8;
	oid_or_null = stage->arg2.oid;
	if (key_or_null) {
		if (oid_or_null) {
			bson_append_oid (bson, key_or_null, -1, oid_or_null);
		} else {
											...
											const mongoc_structured_log_opts_t *opts)
{
	...
	is_sensitive = mongoc_apm_is_sensitive_command_message (cmd->command_name, cmd->command) ||
						mongoc_apm_is_sensitive_command_message (cmd->command_name, reply);
	_mongoc_structured_log_append_redacted_cmd_reply (bson, is_sensitive, reply, opts);
	return stage + 1;
}
												...
												const mongoc_structured_log_opts_t *opts)
{
	...
	is_sensitive = mongoc_apm_is_sensitive_command_message (cmd_name, reply);
	_mongoc_structured_log_append_redacted_cmd_reply (bson, is_sensitive, reply, opts);
	return stage + 1;
}
...
_mongoc_structured_log_append_cmd_failure_stage0 (bson_t *bson,
													...
													const mongoc_structured_log_opts_t *opts)
{
	const mongoc_cmd_t *cmd = 0;
	const bson_t *reply = 0;
	const bson_error_t *error = 0;
	bool is_sensitive = false;

	BSON_UNUSED (opts);
	BSON_ASSERT (stage[1].func == _mongoc_structured_log_append_cmd_failure_stage1);
	cmd = stage[0].arg1.cmd;
	reply = stage[0].arg2.bson;
	error = stage[1].arg1.error;
...
_mongoc_structured_log_append_server_description (bson_t *bson,
													...
													const mongoc_structured_log_opts_t *opts)
{
	const mongoc_server_description_t *sd = 0;
	mongoc_server_description_content_flags_t flags = 0;

	BSON_UNUSED (opts);
	sd = stage->arg1.server_description;
														...
														const mongoc_structured_log_opts_t *opts)
{
	...
	bson_t doc = BSON_INITIALIZER;
	mcommon_string_t *result = 0;
	mongoc_topology_description_append_contents_to_bson (td, &doc, td_flags, server_flags);
	result = _mongoc_structured_log_document_as_truncated_json (&doc, opts);
	bson_destroy (&doc);
	return result;
}
...
{
	const char *key_or_null = 0;
	const mongoc_read_prefs_t *read_prefs = 0;

	BSON_UNUSED (opts);
	key_or_null = stage->arg1.utf8;
	read_prefs = stage->arg2.read_prefs;
	if (key_or_null) {
		bson_t child;
		if (BSON_APPEND_DOCUMENT_BEGIN (bson, key_or_null, &child)) {
			mongoc_read_prefs_append_contents_to_bson (
