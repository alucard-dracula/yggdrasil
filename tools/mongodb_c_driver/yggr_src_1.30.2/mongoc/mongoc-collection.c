											...
											mongoc_write_result_t *result)
{
	...
	ss_log_context.operation = _mongoc_write_command_get_name (command);
	ss_log_context.has_operation_id = true;
	ss_log_context.operation_id = command->operation_id;
											...
											bson_error_t *error)
{
	...
	// Create and execute a "count" command
	bsonBuildDecl (cmd, kv ("count", cstr (coll->collection)));
	command_ok = _mongoc_client_command_with_opts (coll->client,
													coll->db,
													&cmd,
													MONGOC_CMD_READ,
													opts,
											...
											bson_error_t *error)
{
	...
	const mongoc_ss_log_context_t ss_log_context = {"createIndexes"};
   ...
   mongoc_collection_t *collection, const bson_t *selector, const bson_t *opts, bson_t *reply, bson_error_t *error)
{
   ...
   ret = _mongoc_delete_one_or_many (
      collection, false /* multi */, selector, &delete_one_opts.delete_, &delete_one_opts.extra, reply, error);
   ...
   mongoc_collection_t *collection, const bson_t *selector, const bson_t *opts, bson_t *reply, bson_error_t *error)
{
   ...
   ret = _mongoc_delete_one_or_many (
      collection, true /* multi */, selector, &delete_many_opts.delete_, &delete_many_opts.extra, reply, error);
												...
												bson_error_t *error)
{
	...
	const mongoc_ss_log_context_t ss_log_context = {"findAndModify"};
...
mongoc_index_model_new (const bson_t *keys, const bson_t *opts)
{
	mongoc_index_model_t *im = 0;
	BSON_ASSERT_PARAM (keys);
	// `opts` may be NULL.

	im = bson_malloc (sizeof (mongoc_index_model_t));
	im->keys = bson_copy (keys);
	im->opts = opts ? bson_copy (opts) : NULL;
	return im;
}
