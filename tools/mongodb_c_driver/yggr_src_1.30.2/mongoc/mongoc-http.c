...
_mongoc_http_send (const mongoc_http_request_t *req,
					...
					bson_error_t *error)
{
	static const char *proto_leader_10 = "HTTP/1.0 ";
	static const char *proto_leader_11 = "HTTP/1.1 ";

	const char *resp_end_ptr = 0;
	ssize_t remain = 0;

	char status_buf[4] = {0};
	char *status_endptr = 0;
	...
	http_response_str = (char *) http_response_buf.data;
	resp_end_ptr = http_response_str + http_response_buf.len;
	...
	headers_len = (size_t) (ptr - http_response_str);
	BSON_ASSERT (mcommon_in_range_unsigned (int, headers_len));

	body_len = http_response_buf.len - headers_len - strlen (header_delimiter);
	BSON_ASSERT (mcommon_in_range_unsigned (int, body_len));
