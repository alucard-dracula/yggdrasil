								...
								mcd_rpc_message *rpc)
{
	...
	n_return = (flags & MONGOC_OP_QUERY_FLAG_TAILABLE_CURSOR) != 0 ? 0 : _mongoc_n_return (cursor);
...
_mongoc_cursor_op_getmore (mongoc_cursor_t *cursor, mongoc_cursor_response_legacy_t *response)
{
	...
	ss_log_context.operation = "getMore";
	ss_log_context.has_operation_id = true;
	ss_log_context.operation_id = cursor->operation_id;
	...
	op_code = mcd_rpc_header_get_op_code (response->rpc);
	if (op_code != MONGOC_OP_CODE_REPLY) {
		bson_set_error (&cursor->error,
						MONGOC_ERROR_PROTOCOL,
						MONGOC_ERROR_PROTOCOL_INVALID_REPLY,
	...
	response_to = mcd_rpc_header_get_response_to (response->rpc);
	if (response_to != request_id) {
		bson_set_error (&cursor->error,
						MONGOC_ERROR_PROTOCOL,
						MONGOC_ERROR_PROTOCOL_INVALID_REPLY,
	...
	documents = mcd_rpc_op_reply_get_documents (response->rpc);
	if (documents == NULL) {
		// Use a non-NULL pointer to satisfy precondition of
		// `bson_reader_new_from_data`:
		documents = "";
