...
_bcrypt_derive_key_pbkdf2 (BCRYPT_ALG_HANDLE prf,
							...
							unsigned char *output)
{
	unsigned char *password_copy = 0;
	unsigned char *salt_copy = 0;
	NTSTATUS status;
	...
	// Make non-const versions of password and salt.
	password_copy = bson_malloc (password_len);
	memcpy (password_copy, password, password_len);
	salt_copy = bson_malloc (salt_len);
	memcpy (salt_copy, salt, salt_len);

	status = BCryptDeriveKeyPBKDF2 (prf,
									password_copy,
									(ULONG) password_len,
									salt_copy,
									(ULONG) salt_len,
