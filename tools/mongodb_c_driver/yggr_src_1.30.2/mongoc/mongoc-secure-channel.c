...
mongoc_secure_channel_setup_ca (mongoc_stream_tls_secure_channel_t *secure_channel, mongoc_ssl_opt_t *opt)
{
	...
	pem_key_buf = (char *) bson_malloc0 (length);
	pem_key = pem_key_buf;
	fread ((void *) pem_key, 1, length, file);
	fclose (file);
...
fail:
	if(pem_key_buf)
	{
		SecureZeroMemory(pem_key_buf, length);
		bson_free(pem_key_buf);
	}
...
mongoc_secure_channel_read (mongoc_stream_tls_t *tls, void *data, size_t data_length)
{
	...
	*bytes)
	**/
	length = mongoc_stream_read (tls->base_stream, data, data_length, 0, (int32_t) tls->timeout_msec);
