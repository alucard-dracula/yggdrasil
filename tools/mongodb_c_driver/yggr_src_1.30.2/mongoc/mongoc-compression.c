					...
					size_t *uncompressed_len)
{
	...
	case MONGOC_COMPRESSOR_ZLIB_ID: {
#ifdef MONGOC_ENABLE_COMPRESSION_ZLIB
		uLong actual_uncompressed_len = 0;
		int res = 0;
		// Malformed message: unrepresentable.
		if (BSON_UNLIKELY (!mcommon_in_range_unsigned (unsigned_long, compressed_len))) {
			return false;
		...
		actual_uncompressed_len = (uLong) *uncompressed_len;

		res = uncompress (uncompressed, &actual_uncompressed_len, (const Bytef *) compressed, (uLong) compressed_len);
