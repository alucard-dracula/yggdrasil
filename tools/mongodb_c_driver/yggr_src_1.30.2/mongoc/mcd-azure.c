...
mcd_azure_imds_request_destroy (mcd_azure_imds_request *req)
{
	...
	bson_free (req->_owned_headers);
	//*req = MCD_AZURE_IMDS_REQUEST_INIT;
	memset(req, 0, sizeof(mcd_azure_imds_request));
}
												...
												bson_error_t *error)
{
	...
	// Zero the output
	//*out = (mcd_azure_access_token){0};
	memset(out, 0, sizeof(mcd_azure_access_token));
	...
	//bson_iter_t iter;
	// access_token
	found = bson_iter_init_find (&iter, &bson, "access_token");
	access_token = !found ? NULL : bson_iter_utf8 (&iter, NULL);
	// resource
	found = bson_iter_init_find (&iter, &bson, "resource");
	resource = !found ? NULL : bson_iter_utf8 (&iter, NULL);
	// token_type
	found = bson_iter_init_find (&iter, &bson, "token_type");
	token_type = !found ? NULL : bson_iter_utf8 (&iter, NULL);
	// expires_in
	found = bson_iter_init_find (&iter, &bson, "expires_in");
	expires_in_len = 0;
	expires_in_str = !found ? NULL : bson_iter_utf8 (&iter, &expires_in_len);
	...
	if (!(access_token && resource && token_type && expires_in_str)) {
		...
		memset(out, 0, sizeof(mcd_azure_access_token));
		out->access_token = bson_strdup (access_token);
		out->resource = bson_strdup (resource);
		out->token_type = bson_strdup (token_type);

		// "expires_in" encodes the number of seconds since the issue time for
		// which the token will be valid. strtoll() will saturate on range errors
		// and return zero on parse errors.
		//char *parse_end;
		s = strtol (expires_in_str, &parse_end, 0);
		if (parse_end != expires_in_str + expires_in_len) {
			// Did not parse the entire string. Bad
			bson_set_error (error,
							MONGOC_ERROR_AZURE,
