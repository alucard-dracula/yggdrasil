...
_mongoc_cluster_buffer_iovec (mongoc_iovec_t *iov, size_t iovcnt, int skip, char *buffer)
{
	...
	for (n = 0u; n < iovcnt; n++) {
		...
		* beyond the skip, we need to figure out the portion of the iovec
		* we should skip passed */
		remaining = total_iov_len - iov_len;
		if (remaining < skip) {
			difference = (size_t) (skip - remaining);
		} else {
			difference = 0u;
	...
	mongoc_cluster_t *cluster, const mongoc_cmd_t *cmd, int32_t compressor_id, mcd_rpc_message *rpc, bson_error_t *error)
{
	...
	ns = bson_strdup_printf ("%s.$cmd", cmd->db_name);
	...
	// set. Clear bits except secondaryOk; leave secondaryOk set only if it is
	// already.
	flags = (int32_t) cmd->query_flags & MONGOC_OP_QUERY_FLAG_SECONDARY_OK;
	...
	mongoc_cluster_t *cluster, const mongoc_cmd_t *cmd, mcd_rpc_message *rpc, bson_t *reply, bson_error_t *error)
{
	...
	message_length = _int32_from_le (buffer.data);
	...
	remaining_bytes = (size_t) message_length - sizeof (int32_t);
...
_in_sharded_or_loadbalanced_txn (const mongoc_client_session_t *session)
{
   ...
   type = _mongoc_topology_get_type (session->client->topology);
...
mongoc_cluster_run_command_monitored (mongoc_cluster_t *cluster, mongoc_cmd_t *cmd, bson_t *reply, bson_error_t *error)
{
	...
	const mongoc_log_and_monitor_instance_t *log_and_monitor = 0;

	server_stream = cmd->server_stream;
	server_id = server_stream->sd->id;

	log_and_monitor = &cluster->client->topology->log_and_monitor;
					...
					bson_error_t *error)
{
	...
	int64_t start = 0;
	mongoc_server_description_t empty_sd;
	mongoc_server_stream_t *server_stream = 0;

	mongoc_query_flags_t query_flags = MONGOC_QUERY_NONE;

	mongoc_cmd_t hello_cmd = {0};
	...
	* API version; the correct hello_cmd has already been selected: */
	...
	hello_cmd.query_flags = query_flags;
	...
	if (!mongoc_cluster_run_command_private (cluster, &hello_cmd, &reply, error)) {
		...
		/* Note: This call will render our copy of the topology description to be
		* stale */
		update_okay = _mongoc_topology_update_from_handshake (cluster->client->topology, ret_handshake_sd);
		if (!update_okay) {
			mongoc_server_description_reset (ret_handshake_sd);
			bson_set_error (&ret_handshake_sd->error,
							MONGOC_ERROR_STREAM,
									...
									bson_error_t *error)
{
	...
	// Important: authentication errors are also considered retryable even if
	// they not considered a network error.
	retryable_error = _mongoc_error_is_network (&first_error) || _mongoc_error_is_auth (&first_error);
											...
											bson_error_t *error)
{
	...
	message_length = mcd_rpc_header_get_message_length (rpc);
							...
							bson_error_t *error)
{
	...
	message_length = _int32_from_le (buffer->data + offset);
	...
	mongoc_cluster_t *cluster, const mongoc_cmd_t *cmd, mcd_rpc_message *rpc, bson_t *reply, bson_error_t *error)
{
	...
	flags = (cmd->is_acknowledged ? MONGOC_OP_MSG_FLAG_NONE : MONGOC_OP_MSG_FLAG_MORE_TO_COME) |
							(cmd->op_msg_is_exhaust ? MONGOC_OP_MSG_FLAG_EXHAUST_ALLOWED : MONGOC_OP_MSG_FLAG_NONE);

	{
		...
		{
			size_t i = 0;
			for (i = 0; i < cmd->payloads_count; i++) {
				const mongoc_cmd_payload_t payload = cmd->payloads[i];
				...
				section_length = sizeof (int32_t) + strlen (payload.identifier) + 1u + (size_t) payload.size;
				BSON_ASSERT (mcommon_in_range_unsigned (int32_t, section_length));
							...
							bson_error_t *error)
{
	...
	int32_t original_message_length = 0;
	int32_t message_header_length = 0;
	...
	original_message_length = mcd_rpc_header_get_message_length (rpc);

	// msgHeader consists of four int32 fields.
	message_header_length = 4u * sizeof (int32_t);

	// compressedMessage does not include msgHeader fields.
	BSON_ASSERT (original_message_length >= message_header_length);
	uncompressed_size = (size_t) (original_message_length - message_header_length);
	BSON_ASSERT (mcommon_in_range_unsigned (int32_t, uncompressed_size));

	estimated_compressed_size = mongoc_compressor_max_compressed_length (compressor_id, uncompressed_size);
	...
	// Store values before they are converted to little endian.
	request_id = mcd_rpc_header_get_request_id (rpc);
	response_to = mcd_rpc_header_get_response_to (rpc);
	op_code = mcd_rpc_header_get_op_code (rpc);
...
mcd_rpc_message_decompress (mcd_rpc_message *rpc, void **data, size_t *data_len)
{
	size_t message_header_length = 0;

	int32_t uncompressed_size_raw = 0;
	...
	uncompressed_size_raw = mcd_rpc_op_compressed_get_uncompressed_size (rpc);
	...
	uncompressed_size = (size_t) uncompressed_size_raw;
	...
	// uncompressedSize does not include msgHeader fields.
	original_message_length = message_header_length + uncompressed_size;
	ptr = bson_malloc (original_message_length);

	message_length = original_message_length;
	request_id = mcd_rpc_header_get_request_id (rpc);
	response_to = mcd_rpc_header_get_response_to (rpc);
...
retry:
	...
	if (can_retry && _mongoc_write_error_get_type (reply) == MONGOC_WRITE_ERR_RETRY) {
		...
		// Select a server.
		{
			mongoc_ss_log_context_t ss_log_context = {0};
			mongoc_deprioritized_servers_t *const ds = mongoc_deprioritized_servers_new ();
			...
			//const mongoc_ss_log_context_t ss_log_context = {
			//.operation = cmd->command_name, .has_operation_id = true, .operation_id = cmd->operation_id};
			ss_log_context.operation = cmd->command_name;
			ss_log_context.has_operation_id = true;
			ss_log_context.operation_id = cmd->operation_id;
