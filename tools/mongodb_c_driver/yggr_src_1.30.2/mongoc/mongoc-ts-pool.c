...
_pool_node_data_offset (const mongoc_ts_pool *pool)
{
	...
	alignment = pool->params.element_alignment;
...
_pool_node_new (const mongoc_ts_pool *pool)
{
	...
	alignment = pool->params.element_alignment;
	size = pool->params.element_size;
	minimum_size = sizeof (pool_node) + _pool_node_data_offset (pool) + size;
	...
	// aligned_alloc requires allocation size to be a multiple of the alignment.
	required_size = minimum_size + (alignment - (minimum_size % alignment));
