...
#if (defined(__ANDROID__) || defined(ANDROID)) && (__ANDROID_MIN_SDK_VERSION__ < 22)
...
#endif // #if (defined(__ANDROID__) || defined(ANDROID)) && (__ANDROID_MIN_SDK_VERSION__ < 22)
...
#if defined (MONGOC_HAVE_RES_NSEARCH) || defined(MONGOC_HAVE_RES_NCLOSE)
   struct __res_state state = {0};
#endif // #if defined (MONGOC_HAVE_RES_NSEARCH) || defined(MONGOC_HAVE_RES_NCLOSE)
...
mongoc_client_connect_tcp (int32_t connecttimeoutms, const mongoc_host_list_t *host, bson_error_t *error)
{
   ...
   // Expect no truncation.
   req = bson_snprintf (portstr, sizeof portstr, "%hu", host->port);
   BSON_ASSERT (mcommon_cmp_less_su (req, sizeof portstr));
...
mongoc_client_start_session (mongoc_client_t *client, const mongoc_session_opt_t *opts, bson_error_t *error)
{
	...
	const mongoc_ss_log_context_t ss_log_context = {"startSession"};
	...
	* original error to be reported. */
	if (is_retryable && _mongoc_read_error_get_type (ret, error, reply) == MONGOC_READ_ERR_RETRY) {
...

		{
			...
			ss_log_context.operation = parts->assembled.command_name;
			ss_log_context.has_operation_id = true;
			ss_log_context.operation_id = parts->assembled.operation_id;
								...
								bson_error_t *error)
{
	...
	*/
	//const mongoc_ss_log_context_t ss_log_context = {.operation = _mongoc_get_command_name (command)};
	ss_log_context.operation =  _mongoc_get_command_name (command);
	server_stream = mongoc_cluster_stream_for_reads (cluster, &ss_log_context, read_prefs, NULL, NULL, reply, error);
									...
									bson_error_t *error)
{
	...
	ss_log_context.operation = command_name;
										...
										const char *collection)
{
	...
	log_and_monitor = &client->topology->log_and_monitor;
								...
								const char *collection)
{
	...
	//bson_error_t error;
	res = mongoc_cluster_legacy_rpc_sendv_to_server (cluster, rpc, server_stream, &error);
...
mongoc_client_kill_cursor (mongoc_client_t *client, int64_t cursor_id)
{
	...
	const mongoc_server_description_t *selected_server = 0;
	mongoc_read_prefs_t *read_prefs = 0;
	bson_error_t error;
	uint32_t server_id = 0;
	mc_shared_tpld td = {{0}};;
	...
	topology = BSON_ASSERT_PTR_INLINE (client)->topology;
	//mongoc_server_description_t const *selected_server;
	//mongoc_read_prefs_t *read_prefs;
	//bson_error_t error;
	//uint32_t server_id = 0;
...
mongoc_client_get_server_description (mongoc_client_t *client, uint32_t server_id)
{
	mongoc_server_description_t *ret = 0;
	mc_shared_tpld td = {{0}};
	...
	sd = mongoc_topology_description_server_by_id_const (td.ptr, server_id, NULL /* <- the error info isn't useful */);
	ret = mongoc_server_description_new_copy (sd);
	mc_tpld_drop_ref (&td);
	return ret;
}
...
{
	mc_shared_tpld td = {{0}};
	mongoc_server_description_t **sds = 0;

	BSON_ASSERT_PARAM (client);

	td = mc_tpld_take_ref (BSON_ASSERT_PTR_INLINE (client)->topology);
	sds = mongoc_topology_description_get_servers (td.ptr, BSON_ASSERT_PTR_INLINE (n));
	mc_tpld_drop_ref (&td);
	return sds;
}
...
{
	mongoc_ss_optype_t optype = for_writes ? MONGOC_SS_WRITE : MONGOC_SS_READ;
	mongoc_server_description_t *sd = 0;

	const mongoc_ss_log_context_t ss_log_context = {"mongoc_client_select_server"};
...
_mongoc_client_end_sessions (mongoc_client_t *client)
{
	...
	while (!mongoc_server_session_pool_is_empty (t->session_pool)) {
		...
		ss_log_context.operation = "endSessions";
		ss_log_context.has_operation_id = true;
		ss_log_context.operation_id = 1 + cluster->operation_id;
