...
mcommon_string_append_bytes (mcommon_string_append_t *append, const char *str, uint32_t len)
{
	...
	buffer = string->str;
	alloc = (uint64_t) string->alloc;
	old_len = (uint64_t) string->len;
	max_len = (uint64_t) append->_max_len;
	new_len = old_len + (uint64_t) len;
	new_len_with_nul = new_len + 1;
