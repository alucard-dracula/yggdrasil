...
#define MC_DEC128_C_INIT(N) mc_dec128 _mcDec128Const_INIT(((N) < 0 ? -(N) : (N)), ((N) < 0 ? 1 : 0))
#else
#define MC_DEC128_C(N) _mcDec128Const(((N) < 0 ? -(N) : (N)), ((N) < 0 ? 1 : 0))
#define MC_DEC128_C_INIT(N) _mcDec128Const_INIT(((N) < 0 ? -(N) : (N)), ((N) < 0 ? 1 : 0))
#endif
...
#define _mcDec128Const_INIT(N, Negate) _mcDec128ConstFromParts_INIT(N, (_mcDec128ZeroExpCombo | ((uint64_t)(Negate) << 63)))
...
#define _mcDec128ConstFromParts_INIT(CoeffLow, HighWord)                                                               \
    {                                                                                                                  \
        {                                                                                                              \
            MLIB_IS_LITTLE_ENDIAN ? (uint64_t)(CoeffLow) : (uint64_t)(HighWord),                                       \
            MLIB_IS_LITTLE_ENDIAN ? (uint64_t)(HighWord) : (uint64_t)(CoeffLow),                                       \
...
#	define _mcDec128ConstFromParts _mcDec128ConstFromParts_INIT

#else

static MONGOC_INLINE mc_dec128 make_mc_dec128(uint64_t coeff_low, uint64_t high_word)
{
	mc_dec128 ret = {																									\
		{																												\
			MLIB_IS_LITTLE_ENDIAN ? (uint64_t)(coeff_low) : (uint64_t)(high_word),										\
			MLIB_IS_LITTLE_ENDIAN ? (uint64_t)(high_word) : (uint64_t)(coeff_low),										\
		}																												\
	};
	return ret;
}
...
#	define _mcDec128ConstFromParts(CoeffLow, HighWord)                                                              \
		(make_mc_dec128(CoeffLow, HighWord))

#endif // #if defined(YGGR_EX_C99_SUPPORTED)

static const mc_dec128 MC_DEC128_ZERO = MC_DEC128_C_INIT(0);
static const mc_dec128 MC_DEC128_ONE = MC_DEC128_C_INIT(1);
static const mc_dec128 MC_DEC128_MINUSONE = MC_DEC128_C_INIT(-1);
...
static MONGOC_INLINE BID_UINT128 _mc_to_bid128(mc_dec128 d) {
    BID_UINT128 r;
    memcpy(&r, &d, sizeof d);
    return r;
}
...
static MONGOC_INLINE mc_dec128 _bid128_to_mc(BID_UINT128 d) {
    mc_dec128 r;
    memcpy(&r, &d, sizeof d);
    return r;
}
 ...
 */
static MONGOC_INLINE mc_dec128 mc_dec128_from_double_ex(double d, mc_dec128_rounding_mode rnd, mc_dec128_flagset *flags) {
 ...
 * nearest Decimal128 value
 */
static MONGOC_INLINE mc_dec128 mc_dec128_from_double(double d) {
 ...
 * @param flags Out param for exception/error flags (Optional)
 */
static MONGOC_INLINE mc_dec128 mc_dec128_from_string_ex(const char *s, mc_dec128_rounding_mode rnd, mc_dec128_flagset *flags) {
    mc_dec128_flagset zero_flags = {0};
 ...
 * Decimal128 value
 */
static MONGOC_INLINE mc_dec128 mc_dec128_from_string(const char *s) {
 ...
 */
static MONGOC_INLINE mc_dec128_string mc_dec128_to_string_ex(mc_dec128 d, mc_dec128_flagset *flags) {
    mc_dec128_flagset zero_flags = {0};
 ...
 * @brief Render a Decimal128 value as a string (in engineering notation)
 */
static MONGOC_INLINE mc_dec128_string mc_dec128_to_string(mc_dec128 d) {
    return mc_dec128_to_string_ex(d, NULL);
}

/// Compare two dec128 numbers
#define DECL_IDF_COMPARE_1(Oper)                                                                                       \
    static MONGOC_INLINE bool mc_dec128_##Oper##_ex(mc_dec128 left, mc_dec128 right, mc_dec128_flagset *flags) {              \
        mc_dec128_flagset zero_flags = {0};                                                                            \
        return 0                                                                                                       \
            != bid128_quiet_##Oper(_mc_to_bid128(left),                                                                \
                                   ...
                                   flags ? &flags->bits : &zero_flags.bits);                                           \
                                                                                                                       ...
                                                                                                                       \
    static MONGOC_INLINE bool mc_dec128_##Oper(mc_dec128 left, mc_dec128 right) {                                             \
...
#define DECL_PREDICATE(Name, BIDName)                                                                                  \
    static MONGOC_INLINE bool mc_dec128_##Name(mc_dec128 d) { return 0 != bid128_##BIDName(_mc_to_bid128(d)); }
...
#define DECL_IDF_BINOP_WRAPPER(Oper)                                                                                   \
    static MONGOC_INLINE mc_dec128 mc_dec128_##Oper##_ex(mc_dec128 left,                                                      \
                                                  mc_dec128 right,                                                     \
        ...
        return _bid128_to_mc(                                                                                          \
            bid128_##Oper(_mc_to_bid128(left), _mc_to_bid128(right), mode, flags ? &flags->bits : &zero_flags.bits));  \
                                                                                                                       ...
                                                                                                                       \
    static MONGOC_INLINE mc_dec128 mc_dec128_##Oper(mc_dec128 left, mc_dec128 right) {                                        \
...
mc_dec128_round_integral_ex(mc_dec128 value, mc_dec128_rounding_mode direction, mc_dec128_flagset *flags) {
	...
	return failed_ret; // fix C4715
}

static MONGOC_INLINE mc_dec128 mc_dec128_negate(mc_dec128 operand) {
    return _bid128_to_mc(bid128_negate(_mc_to_bid128(operand)));
}

static MONGOC_INLINE mc_dec128 mc_dec128_abs(mc_dec128 operand) {
 ...
 * @return mc_dec128 The product
 */
static MONGOC_INLINE mc_dec128 mc_dec128_scale(mc_dec128 fac, long int exp) {
 ...
 */
static MONGOC_INLINE mc_dec128_modf_result mc_dec128_modf_ex(mc_dec128 d, mc_dec128_flagset *flags) {
    mc_dec128_flagset zero_flags = {0};
 ...
 * @param d The value to inspect
 */
static MONGOC_INLINE mc_dec128_modf_result mc_dec128_modf(mc_dec128 d) {
 ...
 */
static MONGOC_INLINE mc_dec128 mc_dec128_fmod_ex(mc_dec128 numer, mc_dec128 denom, mc_dec128_flagset *flags) {
    mc_dec128_flagset zero_flags = {0};
    return _bid128_to_mc(
 ...
 * @param denom The divisor of the modulus
 */
static MONGOC_INLINE mc_dec128 mc_dec128_fmod(mc_dec128 numer, mc_dec128 denom) {
 ...
 * @param flags Control/status flags
 */
static MONGOC_INLINE int64_t mc_dec128_to_int64_ex(mc_dec128 d, mc_dec128_flagset *flags) {
    mc_dec128_flagset zero_flags = {0};
 ...
 * @param d The value to inspect
 */
static MONGOC_INLINE int64_t mc_dec128_to_int64(mc_dec128 d) {
...
static MONGOC_INLINE uint32_t mc_dec128_combination(mc_dec128 d) {
 ...
 */
static MONGOC_INLINE uint64_t mc_dec128_coeff_high(mc_dec128 d) {
 ...
 */
static MONGOC_INLINE uint64_t mc_dec128_coeff_low(mc_dec128 d) {
    uint32_t combo = mc_dec128_combination(d);
    if (combo < MC_DEC128_COMBO_NONCANONICAL) {
 ...
 * 128-bit integer.
 */
static MONGOC_INLINE mlib_int128 mc_dec128_coeff(mc_dec128 d) {
    // Hi bits
 ...
 */
static MONGOC_INLINE uint32_t mc_dec128_get_biased_exp(mc_dec128 d) {
    uint32_t combo = mc_dec128_combination(d);
    if (combo < MC_DEC128_COMBO_NONCANONICAL) {
        return combo >> 3;
    }
...
//static MONGOC_INLINE char *mc_dec128_to_new_decimal_string(mc_dec128 d) {
...
static MONGOC_INLINE char *mc_dec128_to_new_decimal_string(mc_dec128 d) {
	...
	if (mc_dec128_is_negative(d)) {
		...
		// Negate the result, return a string with a '-' prefix
		d = mc_dec128_negate(d);
		s = mc_dec128_to_new_decimal_string(d);
		if (!s) {
			return NULL;
		}
		s1 = (char *)calloc(strlen(s) + 2, 1);
		if (s1) {
			s1[0] = '-';
			strcpy(s1 + 1, s);
		}
	...
	if (mc_dec128_is_zero(modf.frac)) {
		...
		// 'd' is now a whole number
		part = mc_dec128_to_new_decimal_string(d);
		if (!part) {
			return NULL;
		}
		buf = (char *)calloc(strlen(part) + 3, 1);
		if (buf) {
			buf[0] = '0';
			buf[1] = '.';
			strcpy(buf + 2, part);
		...
		frac = mc_dec128_to_new_decimal_string(modf.frac);
		if (!frac) {
			free(whole);
			return NULL;
		}
		ret = (char *)calloc(strlen(whole) + strlen(frac) + 1, 1);
		if (ret) {
			char *out = ret;
			strcpy(out, whole);
			out += strlen(whole);
...
//static MONGOC_INLINE mc_dec128 mc_dec128_from_bson_iter(const bson_iter_t *it) {
//    bson_decimal128_t b;
...
static MONGOC_INLINE bson_decimal128_t mc_dec128_to_bson_decimal128(mc_dec128 v) {
