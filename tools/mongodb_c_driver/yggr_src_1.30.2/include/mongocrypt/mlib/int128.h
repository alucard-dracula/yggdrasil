...
static MONGOC_INLINE mlib_int128 make_mlib_int128(uint64_t low, uint64_t high)
{
	mlib_int128 ret = {{low, high}};
	return ret;
}

#	define MLIB_INT128_FROM_PARTS(LowWord_u64, HighWord_u64)                                                              \
		(make_mlib_int128(LowWord_u64, HighWord_u64))
...
static mlib_constexpr_fn int _mlibCountLeadingZeros_u128(mlib_int128 r) {
    ...
    clz_l = _mlibCountLeadingZeros_u64(r.r.hi);
    if (clz_l != 64) {
        return clz_l;
    }

    clz_r = _mlibCountLeadingZeros_u64(r.r.lo);
    return clz_l + clz_r;
}
...
static mlib_constexpr_fn struct mlib_int128_divmod_result _mlibDivide_u128_by_u64(const mlib_int128 numer,
	...
	mlib_int128_divmod_result result = {{0}};

	int d = 0;
    mlib_int128 adjusted = numer;
    adjusted.r.hi %= denom;
    d = _mlibCountLeadingZeros_u64(denom);
    ...
    if (d >= 32) {
		...
		result.quotient = MLIB_INT128_FROM_PARTS(q1_q0, q3_q2);
		result.remainder = MLIB_INT128_FROM_PARTS(rem, 0);
	...
	u[0] = (u32)(adjusted.r.lo << d);
	u[1] = (u32)(adjusted.r.lo >> (32 - d));
	u[2] = (u32)(adjusted.r.hi << d);
	u[3] = (u32)(adjusted.r.hi >> (32 - d));
    ...
    rem = ((u64)u[1] << (32 - d)) | (u[0] >> d);
    quo = ((u64)qparts[1] << 32) | qparts[0];

	result.quotient = MLIB_INT128_FROM_PARTS(quo, numer.r.hi / denom);
	result.remainder = MLIB_INT128_FROM_PARTS(rem, 0);
	return result;
}
...
static mlib_constexpr_fn mlib_int128_divmod_result mlib_int128_divmod(mlib_int128 numer, mlib_int128 denom) {
    ...
    if (dhi > nhi) {
		...
		mlib_int128_divmod_result result = {{0}};
		result.quotient = MLIB_INT128_C(0);
		result.remainder = numer;
		return result;
    } else if (dhi == nhi) {
        // High words are equal
        if (nhi == 0) {
            ...
            // numbers
			mlib_int128_divmod_result result = {{0}};
			result.quotient = MLIB_INT128_CAST(nlo / dlo);
			result.remainder = MLIB_INT128_CAST(nlo % dlo);
			return result;
        } else if (nlo > dlo) {
            // The numerator is larger than the denom and the high word on the
            // denom is non-zero, so this cannot divide to anything greater than 1.
			mlib_int128_divmod_result result = {{0}};
			result.quotient = MLIB_INT128(1);
			result.remainder = mlib_int128_sub(numer, denom);
			return result;
        } else if (nlo < dlo) {
            // numer.r.lo < denom.r.lo and denom.r.hi > denom.r.lo, so the
            // integer division becomes zero
			mlib_int128_divmod_result result = {{0}};
			result.quotient = MLIB_INT128(0);
			result.remainder = numer;
			return result;
        } else {
            // N / N is one
			mlib_int128_divmod_result result = {{0}};
			result.quotient = MLIB_INT128(1);
			result.remainder = MLIB_INT128(0);
			return result;
        }
    } else if (dhi == 0) {
        // No high in denominator. We can use a u128/u64
		...
		u[0] = (uint32_t)(numer.r.lo << d);
		u[1] = (uint32_t)(numer.r.lo >> (32 - d));
		u[2] = (uint32_t)(numer.r.hi << d);
		u[3] = (uint32_t)(numer.r.hi >> (32 - d));
		...
		v[0] = (uint32_t)(denom.r.lo << d);
		v[1] = (uint32_t)(denom.r.lo >> (32 - d));
		v[2] = (uint32_t)(denom.r.hi << d);
		v[3] = (uint32_t)(denom.r.hi >> (32 - d));
        ...
        remainder = MLIB_INT128_FROM_PARTS(((uint64_t)u[1] << 32) | u[0], ((uint64_t)u[3] << 32) | u[2]);
        remainder = mlib_int128_rshift(remainder, d);

		result.quotient = MLIB_INT128_CAST(q[0] | (uint64_t)q[1] << 32);
