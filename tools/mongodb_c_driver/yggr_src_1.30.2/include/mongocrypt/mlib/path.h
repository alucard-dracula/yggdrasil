 ...
 */
static MONGOC_INLINE bool mpath_is_sep(char c, mpath_format f) {
    if (f == MPATH_WIN32) {
        return c == '/' || c == '\\';
    } else {
        return c == '/';
 ...
 */
static MONGOC_INLINE char mpath_preferred_sep(mpath_format f) {
    if (f == MPATH_WIN32) {
        return '\\';
    } else {
        return '/';
 ...
 */
//static MONGOC_INLINE mstr mpath_current_path(void) {
//#if _WIN32
//    while (1) {
 ...
 */
static MONGOC_INLINE bool mpath_has_trailing_sep(mstr_view path, mpath_format f) {
 ...
 */
static MONGOC_INLINE mstr_view mpath_parent(mstr_view path, mpath_format f) {
 ...
 */
//static MONGOC_INLINE mstr_view mpath_filename(mstr_view path, mpath_format f) {
//    if (!path.len) {
//        return path;
//    }
...
static MONGOC_INLINE mstr_view mpath_filename(mstr_view path, mpath_format f) {
    ...
    it = path.data + path.len;
    while (it != path.data && !mpath_is_sep(it[-1], f)) {
        --it;
    }
    off = (size_t)(it - path.data);
    fname = mstrv_subview(path, off, path.len);
    if (fname.len == 0) {
        return mstrv_lit(".");
    }
    return fname;
 ...
 */
//static MONGOC_INLINE mstr mpath_join(mstr_view base, mstr_view suffix, mpath_format f) {
//    if (!base.len) {
...
static MONGOC_INLINE mstr mpath_join(mstr_view base, mstr_view suffix, mpath_format f) {
    ...
    // We must insert a path separator between the two strings
    assert(base.len <= SIZE_MAX - suffix.len - 1u);
    r = mstr_new(base.len + suffix.len + 1);
    p = r.data;
    memcpy(p, base.data, base.len);
    p += base.len;
    *p++ = mpath_preferred_sep(f);
 ...
 */
static MONGOC_INLINE mstr_view mpath_root_name(mstr_view path, mpath_format f) {
 ...
 */
static MONGOC_INLINE mstr_view mpath_root_directory(mstr_view path, mpath_format f) {
 ...
 */
static MONGOC_INLINE mstr_view mpath_root_path(mstr_view path, mpath_format f) {
    mstr_view rname = mpath_root_name(path, f);
 ...
 */
static MONGOC_INLINE bool mpath_is_absolute(mstr_view path, mpath_format f) {
    if (f == MPATH_WIN32) {
 ...
 */
static MONGOC_INLINE mstr_view mpath_relative_path(mstr_view path, mpath_format f) {
 ...
 */
static MONGOC_INLINE mstr mpath_to_format(mpath_format from, mstr_view path, mpath_format to) {
 ...
 */
static MONGOC_INLINE bool mpath_is_relative(mstr_view path, mpath_format f) {
 ...
 * @note The return value must be freed with mstr_free()
 */
static MONGOC_INLINE mstr mpath_absolute(mstr_view path, mpath_format f);
 ...
 */
//static MONGOC_INLINE mstr mpath_absolute_from(mstr_view path, mstr_view base, mpath_format f) {
//    mstr_view rname = mpath_root_name(path, f);
//    mstr_view rdir = mpath_root_directory(path, f);
//    if (rname.len) {
//        if (rdir.len) {
...
static MONGOC_INLINE mstr mpath_absolute_from(mstr_view path, mstr_view base, mpath_format f) {
    ...
    if (rname.len) {
        ...
        if (rdir.len) {
            ...
            abs_base = mpath_absolute(base, f);
            base_rname = mpath_root_name(abs_base.view, f);
            ret = mpath_join(base_rname, path, f);
            mstr_free(abs_base);
            return ret;
        } else {
            mstr abs_base = mpath_absolute(base, f);
...
static MONGOC_INLINE mstr mpath_absolute(mstr_view path, mpath_format f) {
    ...
    cur = mpath_current_path();
    ret = mpath_absolute_from(path, cur.view, MPATH_NATIVE);
    mstr_assign(&ret, mpath_to_format(MPATH_NATIVE, ret.view, f));
    mstr_free(cur);
    return ret;
}
