...
#ifndef UINT_LSHIFT
static MONGOC_INLINE UINT_T DECORATE_NAME(_mc_default_lshift)(UINT_T lhs, int off) {
    if (off < 0) {
        return lhs >> -off;
    } else {
        return lhs << off;
...
static MONGOC_INLINE int DECORATE_NAME(_mc_compare)(UINT_T lhs, UINT_T rhs) {
...
//static MONGOC_INLINE DECORATE_NAME(MinCoverGenerator)
...
static MONGOC_INLINE DECORATE_NAME(MinCoverGenerator)
    * DECORATE_NAME(MinCoverGenerator_new)(UINT_T rangeMin,
	...
	size_t maxlen = 0;
	int32_t trimFactor = 0;
	DECORATE_NAME(MinCoverGenerator) *mcg = 0;
    ...
    maxlen = (size_t)BITS - DECORATE_NAME(mc_count_leading_zeros)(max);
    trimFactor = trimFactorDefault(maxlen, opt_trimFactor, use_range_v2);
    if (trimFactor != 0 && mc_cmp_greater_equal_su(trimFactor, maxlen)) {
        CLIENT_ERR("Trim factor must be less than the number of bits (%ld) used to represent an element of the domain, "
                   "but got %" PRId32,
                   maxlen,
    ...
    mcg = bson_malloc0(sizeof(DECORATE_NAME(MinCoverGenerator)));
    mcg->_rangeMin = rangeMin;
    mcg->_rangeMax = rangeMax;
    mcg->_maxlen = (size_t)BITS - DECORATE_NAME(mc_count_leading_zeros)(max);
    mcg->_sparsity = sparsity;
...
static MONGOC_INLINE void DECORATE_NAME(MinCoverGenerator_destroy)(DECORATE_NAME(MinCoverGenerator) * mcg) {
    bson_free(mcg);
}

// applyMask applies a mask of 1 bits starting from the right.
// Bits 0 to bit-1 are replaced with 1. Other bits are left as-is.
//static MONGOC_INLINE UINT_T DECORATE_NAME(applyMask)(UINT_T value, size_t maskedBits) {
...
static MONGOC_INLINE UINT_T DECORATE_NAME(applyMask)(UINT_T value, size_t maskedBits) {
    ...
    shift = ((size_t)BITS - maskedBits);
    mask = UINT_LSHIFT(ones, -(int)shift);
    return UINT_BITOR(value, mask);
}
...
//static MONGOC_INLINE bool DECORATE_NAME(MinCoverGenerator_isLevelStored)(DECORATE_NAME(MinCoverGenerator) * mcg,
//                                                                  size_t maskedBits) {
...
static MONGOC_INLINE bool DECORATE_NAME(MinCoverGenerator_isLevelStored)(DECORATE_NAME(MinCoverGenerator) * mcg,
                                                                  size_t maskedBits) {
	size_t level = 0;
	size_t trimFactor_sz = 0;

    BSON_ASSERT_PARAM(mcg);
    level = mcg->_maxlen - maskedBits;
    BSON_ASSERT(mc_in_range_size_t_signed(mcg->_trimFactor));
    trimFactor_sz = (size_t)mcg->_trimFactor;
    return 0 == maskedBits || (level >= trimFactor_sz && 0 == (level % mcg->_sparsity));
}
...
DECORATE_NAME(MinCoverGenerator_toString)(DECORATE_NAME(MinCoverGenerator) * mcg, UINT_T start, size_t maskedBits) {
    ...
    shifted = UINT_LSHIFT(start, -(int)maskedBits);
    valueBin = DECORATE_NAME(mc_convert_to_bitstring)(shifted);
    ret = bson_strndup(valueBin.str + ((size_t)BITS - mcg->_maxlen + maskedBits), mcg->_maxlen + maskedBits);
    return ret;
}

#endif // #if defined(YGGR_EX_C99_SUPPORTED)

//static MONGOC_INLINE void DECORATE_NAME(MinCoverGenerator_minCoverRec)(DECORATE_NAME(MinCoverGenerator) * mcg,
//                                                                mc_array_t *c,
//                                                                UINT_T blockStart,
//                                                                size_t maskedBits) {
//    BSON_ASSERT_PARAM(mcg);
...
static MONGOC_INLINE void DECORATE_NAME(MinCoverGenerator_minCoverRec)(DECORATE_NAME(MinCoverGenerator) * mcg,
    ...
    BSON_ASSERT_PARAM(mcg);
    BSON_ASSERT_PARAM(c);
    blockEnd = DECORATE_NAME(applyMask)(blockStart, maskedBits);
...
//static MONGOC_INLINE mc_mincover_t *DECORATE_NAME(MinCoverGenerator_minCover)(DECORATE_NAME(MinCoverGenerator) * mcg) {
//    BSON_ASSERT_PARAM(mcg);
...
static MONGOC_INLINE int32_t DECORATE_NAME(MinCoverGenerator_usedTrimFactor)(DECORATE_NAME(MinCoverGenerator) * mcg) {
    BSON_ASSERT_PARAM(mcg);
