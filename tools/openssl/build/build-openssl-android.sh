...
prefix_dir=${prefix_root_dir}/openssl
prefix_tmp_dir_ssl=${prefix_tmp_dir}/ssl

prefix_dir_include=${prefix_dir}/include
prefix_dir_ssl=${prefix_dir}/ssl
...
do
    ...
    sh build-openssl-android_tpl.sh \
        ${prefix_tmp_dir} \
        ${prefix_tmp_dir_ssl} \
        ${AndroidNDK} \
        ${var_target} \
        ${USED_AndroidAPI} \
        ${AndroidToolChainPlatfrom} 

    if [ ! -d ${prefix_dir_include} ]; then
        cp -fr ${prefix_tmp_dir}/include ${prefix_dir}/
    fi 

    if [ ! -d ${prefix_dir_ssl} ]; then
        cp -fr ${prefix_tmp_dir}/ssl ${prefix_dir}/
    fi 
