...
#if defined(KECCAK_COMPLEMENTING_TRANSFORM)
static void round_trans_transform_0(uint64_t R[5][5], uint64_t C[5], size_t iotas_i)
{
    R[0][0] = C[0] ^ ( C[1] | C[2]) ^ iotas_i;
    R[0][1] = C[1] ^ (~C[2] | C[3]);
    R[0][2] = C[2] ^ ( C[3] & C[4]);
    R[0][3] = C[3] ^ ( C[4] | C[0]);
    R[0][4] = C[4] ^ ( C[0] & C[1]);
...
static void round_trans_transform_1(uint64_t R[5][5], uint64_t C[5])
{
   R[1][0] = C[0] ^ (C[1] |  C[2]);
    R[1][1] = C[1] ^ (C[2] &  C[3]);
    R[1][2] = C[2] ^ (C[3] | ~C[4]);
...
#endif //KECCAK_COMPLEMENTING_TRANSFORM
...
//     R[0][4] = C[4] ^ (~C[0] & C[1]);
// #endif
    round_trans_transform_0(R, C, iotas[i]);
