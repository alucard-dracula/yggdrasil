...
namespace objects
{
  ...
  BOOST_PYTHON_DECL PyObject* static_data()
  {
      if (static_data_object.tp_dict == 0)
      {
		  ...
		  (reinterpret_cast<PyObject*>(&static_data_object))->ob_type = &PyType_Type;
#else
		  Py_TYPE(&static_data_object) = &PyType_Type;
#endif  // 
...
#if defined(_MSC_VER) /*&& !(_MSC_VER < 1900)*/
		  (reinterpret_cast<PyObject*>(&class_metatype_object))->ob_type = &PyType_Type;
#else
		  Py_TYPE(&class_metatype_object) = &PyType_Type;
...
#if defined(_MSC_VER) /*&& !(_MSC_VER < 1900)*/
		  (reinterpret_cast<PyObject*>(&class_type_object))->ob_type = incref(class_metatype().get());
#else
		  Py_TYPE(&class_type_object) = incref(class_metatype().get());
#endif // #if defined(_MSC_VER) /*&& !(_MSC_VER < 1900)*/
