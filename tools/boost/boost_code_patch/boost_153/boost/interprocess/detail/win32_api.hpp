...
inline bool get_wmi_class_attribute( std::wstring& strValue, const wchar_t *wmi_class, const wchar_t *wmi_class_var)
{
   ...
   if( 0 == sec_init_ret || RPC_E_TOO_LATE_BIPC == sec_init_ret)
   {
...
#else
	  //wchar_variant vwchar;
	  struct wchar_variant_wrap
	  {
		  union
		  {
			  wchar_variant vwchar;
			  char vwchar_buf[sizeof(wchar_variant) + 8];
		  };
	  };


	  wchar_variant_wrap vwchar_wrap;
	  memset(&vwchar_wrap, 0, sizeof(wchar_variant_wrap));
      ...
      {
         com_releaser<IWbemClassObject_BIPC> IWbemClassObject_releaser(pClassObject);
         if ( 0 == pClassObject->Get( L"LastBootUpTime", 0, &(vwchar_wrap.vwchar), 0, 0 ) ){
            bRet = true;
            strValue = vwchar_wrap.vwchar.value.pbstrVal;
            VariantClear(&(vwchar_wrap.vwchar));
            break;
         }
      }
