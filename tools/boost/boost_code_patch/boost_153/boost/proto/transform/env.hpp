...
namespace boost
{
    ...
    #define BOOST_PROTO_DEFINE_ENV_VAR(TAG, NAME)                                                   \
          ...
          : proto::transform<_env_var<Key> >
        {
            ...
            {
			private:
				typedef transform_impl<Expr, State, Data> base_type;
			public:
                typedef typename base_type::data::template lookup<Key>::type result_type;
                BOOST_MPL_ASSERT_NOT((is_same<result_type, key_not_found>)); // lookup failed

                BOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(result_type, typename base_type::data::template lookup<Key>::const_reference)
                operator ()(
                    typename base_type::expr_param
                  , typename base_type::state_param
                  , typename base_type::data_param d
                ) const
                {
                    return d[Key()];
                }
