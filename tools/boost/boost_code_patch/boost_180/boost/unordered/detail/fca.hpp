  ...
  namespace unordered {
    namespace detail {
		...
		template<typename Alloc,
					bool chk =  boost::allocator_propagate_on_container_swap<Alloc>::type::value>
		struct allocator_swap_helper
		{
		public:
			typedef Alloc allocator_type;
		public:
			inline void operator()(const allocator_type&, const allocator_type&) const
			{
			}
		};

		template<typename Alloc>
		struct allocator_swap_helper<Alloc, true>
		{
		public:
			typedef Alloc allocator_type;
		public:
			inline void operator()(allocator_type& l, allocator_type& r) const
			{
				::boost::swap(l, r);
			}
		};
...
#else
		  patch::allocator_swap_helper<allocator_type> helper;
		  helper(get_node_allocator(), other.get_node_allocator());
#endif // 0, 1
        }
