...
#include <boost/container/detail/patch_aligned_storage.hpp>
   ...
   :  public intrusive_tree_hook<VoidPointer, tree_type_value, OptimizeSize>::type
{
   ...
   BOOST_CONTAINER_FORCEINLINE T &get_data()
   {  return *(patch::aligned_storage_value_pointer<T>(this->m_storage));   }

   BOOST_CONTAINER_FORCEINLINE const T &get_data() const
   {   return *(patch::aligned_storage_value_pointer<const T>(this->m_storage));  }
   ...
   //{  return reinterpret_cast<internal_type*>(this->m_storage.data);  }
    ...
    BOOST_CONTAINER_FORCEINLINE internal_type &get_real_data()
   {  return *(patch::aligned_storage_value_pointer<internal_type>(this->m_storage));   }

   BOOST_CONTAINER_FORCEINLINE const internal_type &get_real_data() const
   {  return *(patch::aligned_storage_value_pointer<const internal_type>(this->m_storage));  }
   ...
   BOOST_CONTAINER_FORCEINLINE ~tree_node()
   {  (patch::aligned_storage_value_pointer<internal_type>(this->m_storage))->~internal_type();  }
...
struct has_trivial_destructor_after_move
   ...
   >
{
   typedef typename ::boost::container::allocator_traits<typename ::boost::container::dtl::tree<T, KeyOfValue, Compare, Allocator, Options>::allocator_type>::pointer pointer;
   static const bool value = ::boost::has_trivial_destructor_after_move<typename ::boost::container::dtl::tree<T, KeyOfValue, Compare, Allocator, Options>::allocator_type>::value &&
                             ::boost::has_trivial_destructor_after_move<pointer>::value &&
                             ::boost::has_trivial_destructor_after_move<Compare>::value;
};
