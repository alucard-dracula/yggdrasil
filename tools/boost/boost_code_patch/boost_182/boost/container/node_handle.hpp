...
{
public:
	template<typename T1, typename T2> BOOST_CONTAINER_FORCEINLINE
	void operator()(T1& l, BOOST_FWD_REF(T2) r) const
	{
		l = boost::move(r);
	}
};
...
public:
	template<typename T1, typename T2> BOOST_CONTAINER_FORCEINLINE
	void operator()(T1&, BOOST_FWD_REF(T2)) const
	{
	}
};
   ...
   node_handle & operator=(BOOST_RV_REF(node_handle) nh) BOOST_NOEXCEPT
   {
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            ...
            //}
			patch_detail::node_handle_inner_alloc_move_setter<NodeAllocator> setter;
			setter(this->node_alloc(), ::boost::move(nh.node_alloc()));
         }
         else{
      ...
      BOOST_NOEXCEPT_IF(nator_traits::propagate_on_container_swap::value || nator_traits::is_always_equal::value)
   {
      ...
      if(was_nh_non_null){
         if(was_this_non_null){
            ...
            //}
			patch_detail::node_handle_inner_alloc_swapper<NodeAllocator> swapper;
			swapper(this->node_alloc(), nh.node_alloc());
         }
         else{
            this->move_construct_alloc(nh.node_alloc());
            nh.destroy_alloc();
