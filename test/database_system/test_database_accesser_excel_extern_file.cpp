...
#   define XLSX_DSN "DRIVER={Microsoft Excel Driver (*.xls, *.xlsx, *.xlsm, *.xlsb)};READONLY=FALSE;FIRSTROWHASNAMES;DBQ=.\\database_system\\excel_org.xls;"
#elif defined(YGGR_AT_DARWIN)
#   define XLSX_DSN "DRIVER={Actual Access};READONLY=FALSE;FIRSTROWHASNAMES;DBQ=/Users/alucard/yggdrasil_macos_project/yggdrasil/test/database_system/excel_org.xls;"
#	define EXCEL_ODBC_DIRVER_SUPPORT_EXTERN_FILE 1
#else
...
void select_test(void)
{
	...
	{
		tester_type test;
		bool bsuccess = db_accesser_type::s_select_records_of_view(client, "test3", rows, "", err); // failed
		if(!bsuccess)
		{
			std::cout << err.what() << std::endl;
		}

#if EXCEL_ODBC_DIRVER_SUPPORT_EXTERN_FILE
        std::cout << "now excel odbc dirver support extend file" << std::endl;
		yggr_test_assert(bsuccess);
#else
        std::cout << "now excel odbc dirver not support extend file" << std::endl;
		yggr_test_assert(!bsuccess);
#endif // EXCEL_ODBC_DIRVER_SUPPORT_EXTERN_FILE
	}
...
void select_test3(void)
{
	...
	std::cout << "--------select_test3 end------------" << std::endl;
}
