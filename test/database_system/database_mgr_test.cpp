...
void test_database_mgr_of_connection(void)
{
	...
	typedef yggr::database_system::database_mgr<std::string, db_connection_type, db_accesser_type> db_mgr_type;
...
#	endif // USE_AUTO_COMMIT()
	...
	db_mgr.execute("clean");

	db_mgr.execute("insert_test");
	db_mgr.execute("select_test");

	db_mgr.execute("update_test");
	db_mgr.execute("select_test");

	db_mgr.execute("insert_test_1_2");
	db_mgr.execute("select_test");

	db_mgr.execute("select_test2");

	db_mgr.execute("delete_test");
	db_mgr.execute("select_test");
