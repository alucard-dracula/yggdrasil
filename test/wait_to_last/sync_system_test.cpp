...
public:
	...
	{
		num = helper.result().get_value<int>("num");
		str = helper.result().get_value<std::string>("str");
		return true;
	}
...
public:
	...
	bool operator<<(const helper_type& helper) // helper to var
	{
		num = helper.result().get_value<std::string>("num");
		str = helper.result().get_value<std::string>("str");
		return true;
	}
...
public:
	...
	public:
		...
		bool operator<<(const helper_type& helper)
		{
			vt = helper.result().get_value<std::vector<int> >("int_vt");
			return true;
		}
...
public:
	...
	bool operator<<(const helper_type& helper) // helper to var
	{
		int cdt = helper.condition().get_value<int>("cdt");

		if(cdt == 1)
		{
			vt.push_back(helper.result().get_value<int>("num"));
		}

		if(cdt == 2)
		{
			//vt.push_back(helper.result().get_value<std::vectot>)
			const std::vector<int>& tvt =helper.result().get_value<std::vector<int> >("int_vt");
			int size = vt.size();
