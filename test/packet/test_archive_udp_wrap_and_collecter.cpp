...
void test1(void)
{
	...
	ipak_wrap_collecter_type collecter1;
	ipak_wrap_collecter_type collecter2;

	for(int i = 0; i < buf_vt.size(); ++i)
	{
		udp_pak_id_type id;
		id.from_buffer(buf_vt[i]);
		collecter1.collect(id, buf_vt[i]);

		collecter2.collect(boost::move(buf_vt[i]));
	}
