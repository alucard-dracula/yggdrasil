...
void test_log_sql_db_op_caller(void)
{
	...
	{
		error_type err;
		caller.call_append(log_op, ts1);
	}

	{
		error_type err;
		caller.call_append(log_op, ts2);
	}

	{
		boost::array<t_s, 1> rows = {ts3};
		error_type err;
		caller.call_append_any(log_op, rows);
	}
