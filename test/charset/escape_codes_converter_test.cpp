...
void test_escape_codes_converter(void)
{
	...
	{
		wchar_t escape_str[12 + 1] = {0};
		wchar_t data[2] = { L' ', L'<' };

		

		std::wcout << conv_type::s_escape_bytes(escape_str, reinterpret_cast<const yggr::u8*>(data), 4, conv_type::SEPUrl<wchar_t>()) << std::endl;
		...
		std::wcout << conv_type::s_escape_bytes(escape_str, reinterpret_cast<const yggr::u8*>(data), 4, conv_type::SEPStringLiteral<wchar_t>()) << std::endl;
		...
		{
			yggr::wstring data2;
			std::size_t unescape_size = conv_type::s_unescape_bytes(data2, escape_str, 12, conv_type::SEPStringLiteral<wchar_t>());
			yggr_test_assert(unescape_size == 4);
...
void test_unescape_codes_converter(void)
{
	...
	{
		yggr::utf8_string escape_str = "aaa%20%3ccccÄãºÃ";
		...
		yggr_test_assert(hex_buf == "aaa <cccÄãºÃ");
	}
