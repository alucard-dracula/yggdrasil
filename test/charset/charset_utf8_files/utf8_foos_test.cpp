...
void test_default_charset_config(void)
{
    ...
    yggr_test_assert(yggr::string("utf-8") == yggr::charset::default_charset_config::get_default_string_charset_name<char>());
#elif defined(YGGR_AT_DARWIN)
    yggr_test_assert(yggr::string("utf-8") == yggr::charset::default_charset_config::get_default_string_charset_name<char>());
#else
#   error "append now platfrom code!!!"
...
#elif defined(YGGR_AT_LINUX)
    yggr_test_assert(yggr::charset::default_charset_config::get_str_default_string_charset_name<char>() == yggr::string("utf-8"));
#elif defined(YGGR_AT_DARWIN)
    yggr_test_assert(yggr::charset::default_charset_config::get_str_default_string_charset_name<char>() == yggr::string("utf-8"));
#else
#   error "append now platfrom code!!!"
...
void test_charset_converter(void)
{
    ...
    yggr::utf8_string std_str_utf8("abc你好", YGGR_STR_UTF8_STRING_CHARSET_NAME());

    std::string std_str = std_str_utf8.str<std::string>();
    std::wstring std_wstr;
    conv(std_str, std_wstr);
    std::wcout << std_wstr << std::endl;
    yggr_test_assert(std_wstr == L"abc你好");


	boost::container::string boost_str = std_str_utf8.str<boost::container::string>();
	boost::container::wstring boost_wstr;
	conv(boost_str, boost_wstr);
	yggr_test_assert(boost_wstr == L"abc你好");
...
void test_charset_foo(void)
{
	// char to char same
	{
	    yggr::utf8_string str_utf8("abc1你好", YGGR_STR_UTF8_STRING_CHARSET_NAME());
		std::string str = str_utf8.str<std::string>();
	...
	// char to wchar diff
	{
        yggr::utf8_string str_utf8("abc3你好", YGGR_STR_UTF8_STRING_CHARSET_NAME());
		std::string str = str_utf8.str<std::string>();
	...
	{
    yggr::utf8_string str_utf8("abc4 你好", YGGR_STR_UTF8_STRING_CHARSET_NAME());
		std::string str = str_utf8.str<std::string>();
...
{
	{
	  yggr::utf8_string str_utf8("abc你cd好", YGGR_STR_UTF8_STRING_CHARSET_NAME());
		std::string str = str_utf8.str<std::string>();
	...
	{
	    yggr::utf8_string str_utf8("abc2你cd好", YGGR_STR_UTF8_STRING_CHARSET_NAME());
		yggr::string str = str_utf8.str<yggr::string>();
...
{
	// '严' unicode = 0x4e25 : 2, utf8 = 0xe4b8a5 : 3
	// charlen
	{
		yggr_test_assert(yggr::charset::utf8_foo::charlen('a') == 1);
		yggr_test_assert(yggr::charset::utf8_foo::charlen("a") == 1);
		yggr_test_assert(yggr::charset::utf8_foo::charlen("严") == 3);

		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::string("a")) == 1);
		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::string("严")) == 3);

		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::string("a").begin()) == 1);
		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::string("严").begin()) == 3);

		{
			char str[] = {static_cast<char>(0xe4), static_cast<char>(0xb8), static_cast<char>(0xa5)};
			yggr_test_assert(yggr::charset::utf8_foo::charlen(str) == 3); // !!!!invalid use less length, but right!!!!
			yggr_test_assert(yggr::charset::utf8_foo::charlen(str, 4) == 3); // !!!!right use !!!!
		}

		{
			char str[] = { static_cast<char>(0x4e), static_cast<char>(static_cast<char>(0x25)) };
			yggr_test_assert(str);
			//yggr_test_assert(yggr::charset::utf8_foo::charlen(str) != 0); // !!!!invalid use less length !!!!
#ifndef _DEBUG
			yggr_test_assert(yggr::charset::utf8_foo::charlen(str, 2) == 0); // !!!!right use !!!!
#endif // _DEBUG
		}
...
#	ifndef _DEBUG
		yggr_test_assert(yggr::charset::utf8_foo::charlen(L'你', 2) == 0); // debug assert(false), release == 0
		yggr_test_assert(yggr::charset::utf8_foo::charlen(L"你", 2) == 0); // debug assert(false), release == 0


		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::wstring(L"a", 2)) == 0); // debug assert(false), release == 0
		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::wstring(L"你", 2)) == 0); // debug assert(false), release == 0

		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::wstring(L"a").begin(), 2) == 0); // debug assert(false), release == 0
		yggr_test_assert(yggr::charset::utf8_foo::charlen(std::wstring(L"你").begin(), 2) == 0); // debug assert(false), release == 0
...
#	endif // _DEBUG
	...
	// is_utf8_string
	{
		...
		{
		    // now source file charset is utf8
			char str[] = "你好";
			yggr_test_assert(yggr::charset::utf8_foo::is_utf8_string("你好") == true);
			yggr_test_assert(yggr::charset::utf8_foo::is_utf8_string(std::string("你好")) == true);
			yggr_test_assert(yggr::charset::utf8_foo::is_utf8_string(boost::container::string("你好")) == true);
