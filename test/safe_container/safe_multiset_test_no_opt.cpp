...
template<typename T>
struct begin_end_foo_t
{
	typedef T cont_type;
	typedef typename cont_type::iterator iter_type;
	typedef typename cont_type::const_iterator citer_type;

	typedef iter_type (cont_type::*begin_end_foo_type)(void);
	typedef citer_type (cont_type::*cbegin_end_foo_type)(void) const;
};

#if defined(__GNUC__)

template<typename T, typename C, typename A>
struct begin_end_foo_t< std::set<T, C, A> >
{
	typedef std::set<T, C, A> cont_type;
	typedef typename cont_type::iterator iter_type;
	typedef typename cont_type::const_iterator citer_type;

	typedef iter_type (cont_type::*begin_end_foo_type)(void) const;
	typedef begin_end_foo_type cbegin_end_foo_type;
};

template<typename T, typename C, typename A>
struct begin_end_foo_t< std::multiset<T, C, A> >
{
	typedef std::multiset<T, C, A> cont_type;
	typedef typename cont_type::iterator iter_type;
	typedef typename cont_type::const_iterator citer_type;
...
void test_constructor(void)
{
	...
	yggr_test_assert(cont7.size() == 10);
	...
	yggr_test_assert(cont9.size() == 10);

	typename set_type::base_type base_cont10(arr + 0, arr + 10);
	set_type cont10(base_cont10);
	yggr_test_assert(cont10.size() == 10 && base_cont10.size() == 10);
	...
	yggr_test_assert(cont12.size() == 10 && base_cont12.size() == 10);
...
#else
template<typename Cont>
bool handler_erase(typename Cont::base_type& base, const std::pair<typename Cont::const_iterator, bool>& rst)
{
	return rst.second;
}
...
void test_modifiers(void)
{
	...
	typedef typename test_detail::begin_end_foo_t<base_set_type>::begin_end_foo_type begin_end_foo_type;
	typedef typename test_detail::begin_end_foo_t<base_set_type>::cbegin_end_foo_type cbegin_end_foo_type;
...
void push_handler(typename Container::base_type&,
					const std::pair<typename Container::iterator, bool>& pos_pair,
					int& count)
{
	++count;
}
...
void write_op(Container& cont, hvt_type& hvt)
{
	...
	typedef typename test_detail::begin_end_foo_t<container_base_type>::begin_end_foo_type begin_end_foo_type;
	typedef typename test_detail::begin_end_foo_t<container_base_type>::cbegin_end_foo_type cbegin_end_foo_type;
...
int main(int argc, char* argv[])
{
	...
	// mutex
	{
		...
		typedef yggr::safe_container::safe_multiset<val_type> set_type;
	...
	{
		typedef yggr::shared_ptr<int> val_type;
		typedef yggr::safe_container::safe_multiset<val_type, boost::shared_mutex> set_type;
