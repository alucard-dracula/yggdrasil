...
struct foos_of_hash
{
	...
	static void handler_check(const typename mi_cont_type::base_type& l, const typename mi_cont_type::base_type& r)
	{
		...
		{
			yggr_test_assert(rcref.find(i->prm1) != rcref.end());
		}
	}
	...
	static void handler_insert_of_ret_helper(mi_cont_type& cont, const typename mi_cont_type::base_type& base_cont, const Handler& ret)
	{
		...
		cont.using_handler(boost::bind(&this_type::handler_insert_of_ret<handler_type>, _1, boost::cref(base_cont), _2), ret);
	}
	...
	{
		typedef Handler handler_type;
		cont.using_handler(boost::bind(&this_type::handler_check_of_ret<handler_type>, _1, boost::cref(base_cont), _2), ret);
	}
...
void test_use_handler_of_hash(void)
{
	...
	ref_cont.using_handler(boost::bind(&foos_of_hash<mi_cont_type>::handler_insert, _1, boost::cref(base_cont)));
	cref_cont.using_handler(boost::bind(&foos_of_hash<mi_cont_type>::handler_check, _1, boost::cref(base_cont)));
...
void test_use_handler_of_ordered(void)
{
	...
	ref_cont.using_handler(boost::bind(&foos_of_ordered<mi_cont_type>::handler_insert, _1, boost::cref(base_cont)));
	cref_cont.using_handler(boost::bind(&foos_of_ordered<mi_cont_type>::handler_check, _1, boost::cref(base_cont)));
...
template<typename Container>
struct foos_of_ranked
{
	typedef Container mi_cont_type;
	typedef foos_of_ranked this_type;
...
void test_use_handler_of_ranked(void)
{
	typedef yggr::safe_container::safe_multi_index_container<
			A,
			boost::multi_index::indexed_by<
	...
	ref_cont.using_handler(boost::bind(&foos_of_ranked<mi_cont_type>::handler_insert, _1, boost::cref(base_cont)));
	cref_cont.using_handler(boost::bind(&foos_of_ranked<mi_cont_type>::handler_check, _1, boost::cref(base_cont)));

	int n = 0;
	foos_of_ranked<mi_cont_type>::handler_insert_of_ret_helper(cont, base_cont,
																boost::bind(&foos_of_ranked<mi_cont_type>::handler_of_ret, boost::ref(n)));
	yggr_test_assert(n == 1);

	n = 0;

	foos_of_ranked<mi_cont_type>::handler_check_of_ret_helper(cont, base_cont,
																boost::bind(&foos_of_ranked<mi_cont_type>::handler_of_ret, boost::ref(n)));
	yggr_test_assert(n == 1);


	std::cout << "-----------------test_use_handler_of_ranked end---------------" << std::endl;
}
...
struct foos_of_sequenced
{
	...
	static void handler_check(const typename mi_cont_type::base_type& l, const typename mi_cont_type::base_type& r)
	{
		...
		for(citer_type i = lcref.begin(), isize = lcref.end(); i != isize; ++i)
		{
			yggr_test_assert(std::find(rcref.begin(), rcref.end(), i->prm1) != rcref.end());
		}
	}
...
void test_use_handler_of_sequenced(void)
{
	...
	ref_cont.using_handler(boost::bind(&foos_of_sequenced<mi_cont_type>::handler_insert, _1, boost::cref(base_cont)));
	cref_cont.using_handler(boost::bind(&foos_of_sequenced<mi_cont_type>::handler_check, _1, boost::cref(base_cont)));
...
void test_use_handler_of_random_access(void)
{
	...
	ref_cont.using_handler(boost::bind(&foos_of_random_access<mi_cont_type>::handler_insert, _1, boost::cref(base_cont)));
	cref_cont.using_handler(boost::bind(&foos_of_random_access<mi_cont_type>::handler_check, _1, boost::cref(base_cont)));
