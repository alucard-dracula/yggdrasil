...
void test_safe_other(void)
{
	...
	std::size_t size = cont.replace(0, 100);

	yggr_test_assert(size == 5);
	...
	yggr_test_assert(beg_val == 1);
	yggr_test_assert(rbeg_val == 100);
...
{
	{
		typedef yggr::safe_container::safe_options_multiset<int> set_type;
		debug_test<set_type>();
	}
	...
	// mutex
	{
		...
		typedef yggr::safe_container::safe_options_multiset<val_type> set_type;
	...
	{
		typedef yggr::shared_ptr<int> val_type;
		typedef yggr::safe_container::safe_options_multiset<val_type, boost::shared_mutex> set_type;
