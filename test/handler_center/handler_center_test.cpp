...
void test_reg(void)
{
	...
	ptr->register_handler(YGGR_PP_HANDLER_NAME(get_eins), boost::bind(&get_eins, _1));
	ptr->register_handler(YGGR_PP_HANDLER_NAME(get_zwei), boost::bind(&get_zwei, _1, _2));

	// compile failed please use boost::reference_wrapper
	ptr->register_handler(YGGR_PP_HANDLER_NAME(get_drei), boost::bind(&get_drei, _1, _2));
...
void test_call(void)
{
	...
	{
		bool b = ptr->call(yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(out)));
		yggr_test_assert(b);
		ptr->unregister_handler(YGGR_PP_HANDLER_NAME(out));
		b = ptr->call(yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(out)));
		yggr_test_assert(!b);
	}

	{
		int n = 100;
		bool b = ptr->call(n, yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(get_eins)), n);
		yggr_test_assert(b);
		...
		yggr_test_assert(b);
		b = ptr->call(n, yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(get_eins)), n);
		yggr_test_assert(!b);
	}

	{
		int n = 100;
		int m = 200;
		bool b = ptr->call(n, yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(get_zwei)), n, m);
		yggr_test_assert(b);
...

	{
		...
		int m = 200;
		// use reference
		bool b = ptr->call(yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(get_drei)), boost::cref(n), boost::ref(m));
		yggr_test_assert(b);
...
void test_A(void)
{
	...
	ptr->call(yggr::const_args_anchor(YGGR_PP_HANDLER_NAME(A::foo)));
	ptr->clear();
