//mongo_accesser_test_of_connection_mt.cpp
...
void test_insert(yggr::nsql_database_system::c_mongo_connection& conn,
...
void test_find(yggr::nsql_database_system::c_mongo_connection& conn,
				const boost::function0<void>& handler)
{
	...
	for(;g_is_run.load();)
	{
		...
		yggr::nsql_database_system::mongo_accesser::s_collection_find(
			...
			filter, 0, 0, 
			err_ins,
			boost::bind(&collector_c_bson_list::handler_collecting, boost::ref(bs_list), _1));
...
void test_drop(yggr::nsql_database_system::c_mongo_connection& conn,
				const boost::function0<void>& handler)
{
