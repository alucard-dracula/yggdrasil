...
public:
	...
	public: // queries
		...
		{
			//std::_Mutex::_Mutex
			//auto tmp = &std::_Init_locks::operator=;
			return new (std::nothrow) this_type(*boost::addressof(held));
			//return 0;
		}
		...
		virtual placeholder* clone(u8 (&data_buf)[store_buffer_size_type::value]) const
		{
			...
			//assert(sizeof(*this) <= store_buffer_size_type::value);
		...
		{
			return sizeof(*this) <= store_buffer_size_type::value;
		}
...
private:
	template<typename T, 
				bool IsSmall = (sizeof(holder<T>) <= store_buffer_size_type::value), 
				typename Nil = int>
	...
	public:
		inline static placeholder* create(u8 (&data_buf)[store_buffer_size_type::value], const value_type& val)
		{
			...
			try
			{
				...
				//return reinterpret_cast<placeholder*>(new (pbuf) int(val));
			}
...
{
public:
	virtual const char * what(void) const BOOST_NOEXCEPT //throw()
	{
...
{
	std::cout << (YGGR_PP_DEBUG(_CRTIMP2)) << std::endl;
