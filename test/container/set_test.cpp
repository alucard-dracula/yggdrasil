...
{
	int arr[5] = {1, 1, 2, 2, 3};
	typedef yggr::set<int> set_type;
	typedef yggr::set<set_type::value_type, set_type::key_compare, set_type::allocator_type> isomeric_same_set_type;

#if !(BOOST_VERSION < 105600)
	typedef set_type::base_type base_set_type;
	typedef isomeric_same_set_type::base_type isomeric_same_base_set_type;
#else
	typedef set_type base_set_type;
	typedef isomeric_same_set_type isomeric_same_base_set_type;
...
void test_erase_if(void)
{
	...
	{
		typedef boost::container::set<int> set_type;
...
void test_extract_insert_node(void)
{
	typedef yggr::set<int> set_type;
	typedef yggr::container::node_handle_traits<set_type>::node_type node_type;
	typedef yggr::container::node_handle_traits<set_type>::insert_return_type insert_return_type;

	int arr[5] = {1, 2, 3, 4, 5};

	set_type cont(arr + 0, arr + 5);

	node_type node = yggr::container::extract(cont, 3);
	yggr_test_assert(cont.size() == 4);
	insert_return_type iret = yggr::container::insert(cont, boost::move(node));
