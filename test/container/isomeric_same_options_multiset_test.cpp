// isomeric_same_options_multiset_test.cpp
...
void test_isomeric_same_construct(void)
{
	...
	typedef yggr::options_multiset<int> default_set_type;
	...
	BOOST_MPL_ASSERT_NOT((yggr::is_isomeric_same_options_multiset<default_set_type, regular_set_type>));
#else
	BOOST_MPL_ASSERT_NOT((boost::is_same<default_set_type, regular_set_type>));
	BOOST_MPL_ASSERT((yggr::is_isomeric_same_options_multiset<default_set_type, regular_set_type>));
...
void test_isomeric_same_operator_swap(void)
{
...

	{
		default_set_type def_set(arr + 0, arr + 5);
		regular_set_type set;
		yggr::swap(boost::move(def_set), set);
		yggr_test_assert(set == set0);
	}
