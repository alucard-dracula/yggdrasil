...
void test_safe_other_gernel(SegCont& seg_cont)
{
	...
	// erase iterator
	{
		...
		yggr_test_assert(cont.erase(boost::bind(&begin_and_end_iter<safe_index0_type::base_type>, _1)));
...
void test_construct_of_random_access(void)
{
	// managed_shared_memory
	{
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_multi_index_container_random_access",
								yggr::segment::segment_op::create_only(),
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"test_managed_shared_memory_safe_multi_index_container_random_access") );
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_multi_index_container_random_access",
								yggr::segment::segment_op::create_only(),
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_multi_index_container_random_access"));
...
template<typename Container, typename SegCont>
void test_observers_of_random_access_detail(SegCont& seg_cont)
{
	typedef Container mi_cont_type;
	typedef typename mi_cont_type::allocator_type alloc_type;
...
void test_observers_of_random_access(void)
{
	// managed_shared_memory
	{
		...
		test_observers_of_random_access_detail<mi_cont_type>(seg_cont);
	...
	std::cout << "---------------test_observers_of_random_access_detail end-------------------------" << std::endl;
}

template<typename Container, typename SegCont>
void test_capacity_of_random_access_detail(SegCont& seg_cont)
{
	typedef Container mi_cont_type;
	typedef typename mi_cont_type::allocator_type alloc_type;
	...
	cont.resize(100);
	yggr_test_assert(cont.template get<0>().size() == cont.template get<1>().size());

	cont.template get<1>().resize(100, A(700, 700, "700"));
	yggr_test_assert(cont.template get<0>().size() == cont.template get<1>().size());

	cont.template get<0>().reserve(500);
	base_cont.template get<0>().reserve(500);

	yggr_test_assert(cont.template get<0>().capacity() == base_cont.template get<0>().capacity());
	yggr_test_assert(cont.template get<0>().capacity() != cont.template get<1>().capacity());

	cont.shrink_to_fit();

	yggr_test_assert(cont.template get<0>().capacity() < base_cont.template get<0>().capacity());

	std::cout << "---------------test_capacity_of_random_access_detail end-------------------------" << std::endl;
}
...
void test_capacity_of_random_access(void)
{
	// managed_shared_memory
	{
		...
		test_capacity_of_random_access_detail<mi_cont_type>(seg_cont);
...
template<typename Container, typename SegCont>
void test_modifiers_of_random_access_detail(SegCont& seg_cont)
{
	typedef Container mi_cont_type;
	typedef typename mi_cont_type::allocator_type alloc_type;

	typedef typename mi_cont_type::base_type unsafe_midx_type;
	typedef typename boost::multi_index::nth_index<unsafe_midx_type, 0>::type unsafe_random_access_0_type;
	typedef typename unsafe_random_access_0_type::iterator unsafe_random_access_0_iterator_type;
	typedef unsafe_random_access_0_iterator_type (unsafe_random_access_0_type::* begin_end_foo_unsafe_random_access_0_type)(void);

	typedef typename boost::multi_index::nth_index<unsafe_midx_type, 1>::type unsafe_random_access_1_type;
	typedef typename unsafe_random_access_1_type::iterator unsafe_random_access_1_iterator_type;
	typedef unsafe_random_access_1_iterator_type (unsafe_random_access_1_type::* begin_end_foo_unsafe_random_access_1_type)(void);
	...
	mi_cont_type cont2(alloc);
	A tmp;
	yggr_test_assert(cont2.empty());
	try
	{
		cont2.template get<0>().front();
	...
	yggr_test_assert(!cont2.template get<0>().get_front(tmp));
	yggr_test_assert(!cont2.template get<0>().get_back(tmp));

	yggr_test_assert(!cont2.template get<A::tag_prm2>().get_front(tmp));
	yggr_test_assert(!cont2.template get<A::tag_prm2>().get_back(tmp));

	yggr_test_assert(!cont2.template get<A::tag_prm2>().pop_front(tmp));
	yggr_test_assert(!cont2.template get<A::tag_prm2>().pop_back(tmp));
	yggr_test_assert(!cont2.template get<A::tag_prm2>().pop_front());
	yggr_test_assert(!cont2.template get<A::tag_prm2>().pop_back());
...

	{
		...
		cont1.insert(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::end), _1),
	...
	cont.template get<0>().push_back(A(10, 10, "10"));
	A back;
	yggr_test_assert(cont.template get<0>().get_back(back));
	yggr_test_assert(back.prm1 == 10 && back.prm2 == 10);

	yggr_test_assert(cont.size() == 11);

	cont.template get<0>().push_front(A(20, 20, "20"));
	A front;
	yggr_test_assert(cont.template get<0>().get_front(front));
	yggr_test_assert(front.prm1 == 20 && front.prm2 == 20);
	yggr_test_assert(cont.size() == 12);

	cont.template get<0>().push_back(A(40, 40, "40"));
	cont.template get<0>().push_front(A(60, 60, "60"));

	yggr_test_assert(cont.size() == 14);
	A back2;
	A front2;

	yggr_test_assert(cont.template get<0>().pop_front(front2));
	yggr_test_assert(cont.template get<0>().pop_back(back2));

	yggr_test_assert(back2.prm1 == 40);
	yggr_test_assert(front2.prm1 == 60);
	yggr_test_assert(cont.size() == 12);

	yggr_test_assert(cont.template get<0>().pop_front());
	yggr_test_assert(cont.template get<0>().pop_back());
...

	{
		...
		yggr_test_assert(cont.insert(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1), A(10, 10, "10")));
		A a(10, 10, "10");
		yggr_test_assert(cont.insert(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1), boost::move(a)));
		yggr_test_assert(cont.size() == 2);

		int call_count = 0;

		yggr_test_assert((cont.insert(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1),
							A(10, 10, "10"),
							boost::bind(&handler_insert_value_of_linear
										<
											unsafe_midx_type, index_id0_type, iterator0_type
		...
		yggr_test_assert(cont.insert(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1), arr + 0, arr + 10));
		yggr_test_assert(cont.size() == 16);


		yggr_test_assert(cont.replace(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1), A(100, 100, "100")));

		A tmp1;

		yggr_test_assert(cont.modify(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1),
							boost::bind(&modifier<A, A>, _1, A(1, 100, "100"), boost::ref(tmp1)) ));

		yggr_test_assert(!cont.modify(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::end), _1),
							boost::bind(&modifier<A, A>, _1, A(1, 100, "100"), boost::ref(tmp1)) ));
		...
		yggr_test_assert(cont.emplace(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1),
	...
	// extract and insert
	{
		...
		// 0, 0
		{
			...
			insert_return0_type ins_ret = cont.insert(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::end), _1), boost::move(node));
		...
		// 0, 1
		{
			...
			insert_return1_type ins_ret = cont.template get<1>().insert(boost::bind(static_cast<begin_end_foo_index1_type>(&base_index1_type::end), _1), boost::move(node));
			yggr_test_assert(ins_ret.inserted);
			yggr_test_assert(ins_ret.node.empty());
			yggr_test_assert(cont.size() == old_size);
		}

		{
			...
			insert_return1_type ins_ret =
				cont.template get<1>().insert(
					boost::bind(static_cast<begin_end_foo_index1_type>(&base_index1_type::end), _1),
					boost::move(node),
					boost::bind(
	...
	struct iter_range_t
	{
		typedef typename safe_index0_type::base_type base0_type;
		typedef typename safe_index0_type::iterator iterator0_type;
		static
		std::pair<iterator0_type, iterator0_type>
			iter_range(base0_type& base)
		{
			return std::make_pair(base.begin(), base.end());
		}
		...
		static
		std::pair<iterator0_type, iterator0_type>
			iter_range_1(base0_type& base)
		{
			yggr_test_assert(base.size() > 1);
			iterator0_type iter = base.begin();
			return std::make_pair(++iter, base.end());
		}

		static
		iterator0_type iter_pos_1(base0_type& base)
		{
			yggr_test_assert(base.size() > 1);
			iterator0_type iter = base.begin();
			return ++iter;
	...
	{
		mi_cont_type cont(arr + 0, arr + 10, alloc);
		unsafe_midx_type right(arr + 0, arr + 10, typename mi_cont_type::ctor_args_list(), alloc);
	...
	// splice(iter_handler, base)
	{
		...
		cont.splice(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1), boost::move(right));
...

	{
		...
		cont.splice(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1),
	...
	{
		mi_cont_type cont(arr + 0, arr + 10, alloc);
		mi_cont_type right(arr + 0, arr + 10, alloc);
...

	{
		...
		cont.splice(boost::move(right), boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1));
	...
	// splice(iter_handler, this_type&)
	{
		...
		cont.splice(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::end), _1),
						boost::move(right));
	...
	//cont.template get<0>().remove(A(1, 100, "aaa")); // warning:right
	cont.template get<0>().remove(A(1, 1, "aaa"));

	yggr_test_assert(cont.size() == 9);

	A rm_cmp(3, 0, "");
	cont.template get<0>().remove_if(boost::bind(&less_foo, _1, boost::cref(rm_cmp)));
	yggr_test_assert(cont.size() == 2);
	...
	cont.template get<0>().sort();

	yggr_test_assert(cont.size() == 6);

	cont.template get<0>().unique();
	...
	cont.template get<0>().sort();
	cont.template get<0>().unique(boost::bind(&less_foo, _1, _2));
	yggr_test_assert(cont.size() == 1);
	cont.template get<0>().reverse();
	yggr_test_assert(cont.size() == 1);
	cont.template get<0>().unique(boost::bind(&less_foo, _1, _2));
	yggr_test_assert(cont.size() == 1);

	typename mi_cont_type::base_type base_cont2(arr + 0, arr + 10, typename mi_cont_type::ctor_args_list(), alloc);
	cont.template get<0>().merge(base_cont2);
	...
	A at1;
	yggr_test_assert(cont.template get<A::tag_prm1>().get_at(1, at1));
	yggr_test_assert(at1.prm1 == 0);

	// erase
	yggr_test_assert(cont.erase(boost::bind(static_cast<begin_end_foo_unsafe_random_access_0_type>(&unsafe_random_access_0_type::begin), _1)));
	yggr_test_assert(cont.template get<1>().erase(boost::bind(static_cast<begin_end_foo_unsafe_random_access_1_type>(&unsafe_random_access_1_type::begin), _1),
								boost::bind(static_cast<begin_end_foo_unsafe_random_access_1_type>(&unsafe_random_access_1_type::end), _1)));
	...
	// remove
	{
		...
		cont.template get<0>().push_back(A(10, 10, "10"));
		cont.template get<0>().push_back(A(10, 10, "10"));
		cont.template get<0>().push_back(A(11, 11, "11"));
	...
	// merge
	{
		mi_cont_type cont1(arr + 0, arr + 10, alloc);
		typename mi_cont_type::base_type cont2(arr + 0, arr + 10, typename mi_cont_type::ctor_args_list(), alloc);
	...
	{
		mi_cont_type cont1(arr + 0, arr + 10, alloc);
		mi_cont_type cont2(arr + 0, arr + 10, alloc);
	...
	// relocate
	{
		...
		cont1.relocate(boost::bind(static_cast<begin_end_foo_index0_type>(&base_index0_type::begin), _1),
	...
	// basic_test
	{
		typename mi_cont_type::base_type cont1(arr + 0, arr + 10, typename mi_cont_type::ctor_args_list(), alloc);

		yggr_test_assert(cont1.template get<0>().front() == arr[0]);
		yggr_test_assert(cont1.template get<0>().back() == arr[9]);

		yggr_test_assert(cont1.template get<1>().front() == arr[0]);
		yggr_test_assert(cont1.template get<1>().back() == arr[9]);

		cont1.template get<1>().reverse();

		yggr_test_assert(cont1.template get<0>().front() == arr[0]);
		yggr_test_assert(cont1.template get<0>().back() == arr[9]);

		yggr_test_assert(cont1.template get<1>().front() == arr[9]);
		yggr_test_assert(cont1.template get<1>().back() == arr[0]);
	}
	...
	struct view_foo_t
	{
		...
		typedef aref_vt_type::iterator aref_vt_iter_type;
		typedef typename safe_index0_type::iterator iterator0_type;
		typedef typename safe_index1_type::iterator iterator1_type;
		typedef typename mi_cont_type::base_type now_base_type;

		static
		iterator1_type rearrange_reverse_1(now_base_type& base)
		{
			base.template get<1>().reverse();
			return base.template get<1>().begin();
		}

		static
		iterator0_type non_rearrange_0(now_base_type& base)
		{
			return base.template get<0>().begin();
		}

		static
		aref_vt_iter_type rearrange_vt(now_base_type& base, aref_vt_type& aref_vt)
		{
			for(iterator0_type i = base.template get<0>().begin(), isize = base.template get<0>().end();
				i != isize; ++i)
			{
...

	{
		...
		yggr_test_assert(cont1.template get<0>().front() == arr[9]);
		yggr_test_assert(cont1.template get<0>().back() == arr[0]);
...

	{
		...
		typename view_foo_t::aref_vt_type helper_vt;
		mi_cont_type cont1(arr + 0, arr + 10, alloc);
	...
	std::cout << "---------------test_modifiers_of_random_access_detail end-------------------------" << std::endl;
}
...
void test_modifiers_of_random_access(void)
{
	// managed_shared_memory
	{
		...
		test_modifiers_of_random_access_detail<mi_cont_type>(seg_cont);
...
template<typename Container, typename SegCont>
void test_safe_other_of_random_access_detail(SegCont& seg_cont)
{
	typedef Container mi_cont_type;
	typedef typename mi_cont_type::allocator_type alloc_type;
...
//		yggr_test_assert(!seg_cont.empty());
//
//		test_safe_other_of_random_access_detail<mi_cont_type>(seg_cont);
//
//		seg_cont.free();
//		yggr_test_assert(seg_cont.is_named_enable_object());
//		yggr_test_assert(seg_cont.empty());
