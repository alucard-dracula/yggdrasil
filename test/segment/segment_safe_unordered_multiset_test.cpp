// segment_safe_unordered_multiset_test.cpp
...
void test_constructor(SegCont& seg_cont)
{
	...
	{
		base_set_type base_cont(arr + 0, arr + 10, 5, hasher_type(), key_equal_type(), alloc);
	...
	{
		base_set_type base_cont13(arr + 0, arr + 10, 5, hasher_type(), key_equal_type(), alloc);
						...
						int check)
{
	...
	yggr_test_assert(*(iter_pos.first) == check);
	return (iter_pos.first != base.end())
			 && (*(iter_pos.first) == check);
}

template<typename Cont>
void check_last_element_of_cont(typename Cont::base_type& cont, int check)
{
	//yggr_test_assert(false); // check called
	yggr_test_assert(cont.size() == check);
}
...
void test_element_lookup(SegCont& seg_cont)
{
	...
	int arr[10] = {1, 2, 3, 3, 2, 1, 5, 5, 5, 5};

	set_type cont(arr + 0, arr + 10, alloc);
	yggr_test_assert(cont.count(1) == 2);
	yggr_test_assert(cont.count(2) == 2);
	yggr_test_assert(cont.count(3) == 2);
	yggr_test_assert(cont.count(4) == 0);
	yggr_test_assert(cont.count(5) == 4);
...
void test_modifiers(SegCont& seg_cont)
{
	...
	cont.insert(arr + 0, arr + 10);
	base_cont.insert(arr + 0, arr + 10);
	...
	// merge
	{
		...
		{
			typedef yggr::safe_container::safe_unordered_set<int> now_set_type;
...
bool check_element(const typename Cont::base_type& base, 
					Iter i, int chk_val)
{
	//yggr_test_assert(i != base.end());
	//yggr_test_assert(*i == chk_val);
...
int main(int argc, char* argv[])
{
	...
	// managed_shared_memory
	{
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_unordered_multiset",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>, 
									"test_managed_shared_memory_safe_unordered_multiset") );
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_unordered_multiset",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_unordered_multiset"));
