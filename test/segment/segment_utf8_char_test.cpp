...
void test_constructor(SegCont& seg_cont)
{
	...
	utf8_char_type c3("你", alloc);
	utf8_char_type c3_3(std::string("你").c_str(), std::string("你").size(), alloc);
	std::cout << c3 << std::endl;
	yggr_test_assert(c3 == c3_3);
	yggr_test_assert(c3 == "你");

	utf8_char_type c4(std::string("好"), alloc);

#if 0
	{
		utf8_char_type c4(yggr::string("好a"), alloc); // assert fail
	}
#endif // 0, 1
...
#else
    utf8_char_type c6(yggr::string("字").c_str(), yggr::string("字").size(), alloc);
#endif // YGGR_NO_CWCHAR
