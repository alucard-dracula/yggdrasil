...
void test_constructor(SegCont& seg_cont)
{
	...
	typedef typename queue_type::value_compare value_compare_type;
	...
	{
		queue_type cont(now_cmper, now_alloc);
		yggr_test_assert(cont.empty());
	}

	{
		inner_container_type init_cont(now_alloc);
...

	{
		...
		queue_type cont(boost::move(base_cont.container()));
...

	{
		...
		queue_type cont(base_cont.container());
		yggr_test_assert(cont.size() == 2);
	}

	{
		...
		queue_type cont(base_cont.value_comp(), boost::move(base_cont.container()));
		yggr_test_assert(cont.size() == 2);
	}

	{
		...
		queue_type cont(base_cont.value_comp(), base_cont.container());
		yggr_test_assert(cont.size() == 2);
	}

	{
		...
		queue_type cont(base_cont.value_comp(), boost::move(base_cont.container()), base_cont.get_allocator());
		yggr_test_assert(cont.size() == 2);
	}

	{
		...
		queue_type cont(base_cont.value_comp(), base_cont.container(), base_cont.get_allocator());
		yggr_test_assert(cont.size() == 2);
	}
...
void test_capacity(SegCont& seg_cont)
{
	...
	inner_container_type init_cont(arr + 0, arr + 10, now_alloc);
...
int main(int argc, char* argv[])
{
	// managed_shared_memory
	{
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_priority_queue",
								yggr::segment::segment_op::create_only(),
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"test_managed_shared_memory_safe_priority_queue") );
		// yggr

		{
			typedef
				yggr::safe_container::safe_priority_queue
				<
					int,
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_priority_queue",
								yggr::segment::segment_op::create_only(),
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_priority_queue"));
