//segment_safe_options_multiset_test.cpp
...
void test_constructor(SegCont& seg_cont)
{
	...
	{
		set_type cont0(arr + 0, arr + 10, alloc);
...

	{
		...
		set_type cont(cont0);
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont0.size() == 10);
	}
...
template<typename Cont>
void check_iter_last_element(typename Cont::base_type& base, 
								...
								int check)
{
	...
	yggr_test_assert(*(pos_pair.first) == check);
}
...
void test_modifiers(SegCont& seg_cont)
{
	...
	cont.insert(200, boost::bind(&check_iter_last_element<set_type>, _1, _2, 200));
	base_cont.insert(200);

	val_type m = 300;
	cont.insert(boost::move(m), boost::bind(&check_iter_last_element<set_type>, _1, _2, 300));
	...
	val_type arr2[10] = {1, 2, 2, 2, 1};

	set_type cont2(arr2 + 0, arr2 + 10, alloc);
	cont2.replace(1, 5);
	cont2.replace(2, 0);
	cont2.replace(0, 10);

	yggr_test_assert(cont2.count(1) == 0);
	yggr_test_assert(cont2.count(5) == 2);
	yggr_test_assert(cont2.count(10) == 3 + 5);

	{
		...
		yggr_test_assert(cont.contains(555));
		yggr_test_assert(!cont.contains(777));
	}
	...
	// merge
	{
		...
		// set
		{
			typedef boost::container::set<val_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::set<val_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::options_set<val_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::safe_container::safe_set<val_type, mutex_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::safe_container::safe_options_set<val_type, mutex_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef boost::container::multiset<val_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::multiset<val_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::options_multiset<val_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::safe_container::safe_multiset<val_type, mutex_type, key_comp_type, alloc_type> now_set_type;
		...
		{
			typedef yggr::safe_container::safe_options_multiset<val_type, mutex_type, key_comp_type, alloc_type> now_set_type;
...
typename 
	...
	<
		...
		bool
	>::type
	ins_node_handler(typename Cont::base_type&,
						const std::pair<typename Cont::iterator, bool>&)
{
	return false;
}
...
typename 
	...
	<
		...
		bool
	>::type
	ins_hint_node_handler(typename Cont::base_type&,
							const std::pair<typename Cont::iterator, bool>&)
{
	return false;
}
...
{
	typedef Cont set_type;
	return ins_node_handler<set_type>(base, pair);
}
...
#	if 0 // !!! boost::interprocess::allocator not support operator= !!!
		bool bret = ins_cont.insert(boost::move(node));
#	endif // 0, 1
	}

	{
		set_type ins_cont(alloc);
		ins_cont.insert(arr + 0, arr + 10);
		yggr::size_type old_s = ins_cont.size();
...
#	if 1 // !!! boost::interprocess::allocator not support operator= !!!
		bool bret = ins_cont.insert(boost::move(node));
...
#	endif // 0, 1
...

	{
		...
		yggr_test_assert(old_s == ins_cont.size() + 1);
...
#	if 1 // !!! boost::interprocess::allocator not support operator= !!!
		...
		yggr_test_assert(old_s == ins_cont.size());
#	endif // 0, 1
	}
...
void test_operations(SegCont& seg_cont)
{
	...
	val_type arr[10] = {1, 2, 3, 4, 5};
	set_type cont(arr + 0, arr + 10, alloc);
	yggr_test_assert(cont.count(0) == 5);
...
void test_swap(SegCont& seg_cont)
{
	...
	base_set_type base_cont1(arr1 + 0, arr1 + 10, alloc);
	base_set_type base_cont2(arr2 + 0, arr2 + 5, alloc);
	yggr_test_assert(cont1.size() == base_cont1.size());
...
template<typename Cont, typename Handler>
void set_handler_zwei(typename Cont::base_type& l, 
						const typename Cont::base_type& r, 
						const Handler& ret_handler)
{
	typedef Cont set_type;
...
void test_using_handler(SegCont& seg_cont)
{
	...
	base_set_type base_cont1(arr1 + 0, arr1 + 5, alloc);
	base_set_type base_cont2(arr2 + 0, arr2 + 5, alloc);
	...
	ref_cont.for_each_iterator_reverse(boost::bind(&ref_all_check_handler<typename set_type::reverse_iterator, set_type>,
													_1, boost::cref(base_cont2)));
...
int main(int argc, char* argv[])
{
	// managed_shared_memory
	{
		...
		typedef 
			yggr::safe_container::safe_options_multiset
			<
				int,
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_options_multiset",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>, 
									"test_managed_shared_memory_safe_options_multiset") );
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_options_multiset",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_options_multiset"));
