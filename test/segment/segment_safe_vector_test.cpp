...
void test_constructor(SegCont& seg_cont)
{
	...
	{
		vt_type* pcont = 
			seg_cont.segment().template construct<vt_type>("vector")(
...

	{
		...
		vt_type* pcont2 = 
			seg_cont.segment().template construct<vt_type>("vector2")(
...

	{
		int arr[10] = {1, 2, 3, 4, 5};
		base_vt_type* pcont = 
			seg_cont.segment().template construct<base_vt_type>("vector")(
				arr + 0, arr + 10, alloc_type(seg_cont.segment().get_segment_manager()));
		yggr_test_assert(pcont);
...

	{
		...
		base_vt_type* pcont2 = 
			seg_cont.segment().template construct<base_vt_type>("vector2")(
...
void test_element_access(SegCont& seg_cont)
{
	...
	vt_type* pcont2 = 
		seg_cont.segment().template construct<vt_type>("vector3")(
			alloc_type(seg_cont.segment().get_segment_manager()));
	yggr_test_assert(pcont2);
	...
	{
		vt_type* pcon_ins = 
		seg_cont.segment().template construct<vt_type>("vector4")(
			alloc_type(seg_cont.segment().get_segment_manager()));
...
void test_modifiers(SegCont& seg_cont)
{
...

	{
		base_vt_type* pbase_cont2 = 
			seg_cont.segment().template construct<base_vt_type>("vector3")(
...
void test_stl_swap(SegCont& seg_cont)
{
	...
	base_vt_type* pbase_cont2 = 
		seg_cont.segment().template construct<base_vt_type>("base_vector2")(
			arr2 + 0, arr2 + 5, alloc_type(seg_cont.segment().get_segment_manager()));
	yggr_test_assert(pbase_cont2);
	...
	base_vt_type* pbase_cont4 = 
		seg_cont.segment().template construct<base_vt_type>("base_vector4")(
...
void test_using_handler(SegCont& seg_cont)
{
	...
	base_vt_type* pbase_cont1 = 
		seg_cont.segment().template construct<base_vt_type>("base_vector1")(
...
int main(int argc, char* argv[])
{
	// managed_shared_memory
	{
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_vector",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>, 
									"test_managed_shared_memory_safe_vector") );
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_vector",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_vector"));
