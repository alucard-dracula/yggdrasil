...
void test_constructor(SegCont& seg_cont)
{
...

	{
		...
		// gcc cpp03 construct::operator() not support arr -> int*, so using arr + 0
		deque_type* pcont =
			seg_cont.segment().template construct<deque_type>("deque")(
...
int main(int argc, char* argv[])
{
	...
	// managed_shared_memory
	{
		...
		typedef yggr::safe_container::safe_deque<int, boost::interprocess::interprocess_mutex, alloc_type> deque_type;

		seg_cont_type seg_cont("test_managed_shared_memory_safe_deque",
								yggr::segment::segment_op::create_only(),
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"test_managed_shared_memory_safe_deque") );
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_deque",
								yggr::segment::segment_op::create_only(),
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_deque"));
