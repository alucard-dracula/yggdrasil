//segment_safe_options_map_test.cpp
...
void test_foo(InputIter first, InputIter last)
{
	std::cout << first->first << std::endl;
}

template<typename InputIter> inline
InputIter iterator_trans(InputIter iter)
{
	return iter;
}
...
void test_constructor(SegCont& seg_cont)
{
	...
	{
		map_type cont(key_comp_type(), alloc);
		yggr_test_assert(cont.empty());
	}

	base_map_type base_cont(key_comp_type(), alloc);
	base_cont.insert(val_type(1, 1));
	base_cont.insert(val_type(2, 2));
	base_cont.insert(val_type(3, 3));
	base_cont.insert(val_type(4, 4));
	base_cont.insert(val_type(5, 5));
	base_cont.insert(val_type(0, 0));

	//{
	//	map_type cont(base_cont.begin(), base_cont.end(), alloc);
	...
	//{
	//	map_type cont(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
	//	yggr_test_assert(cont.size() == 6);
	//}

	{
		// 处理到这里 BOOST_CONTAINER_FORCEINLINE 的锅 需要做全面检查和补充
		typename base_map_type::iterator i = base_cont.begin();
		typename base_map_type::iterator isize = base_cont.end();

		base_map_type base_cont(i, isize, key_comp_type(), alloc);

		typename base_map_type::base_type base_base_cont(base_cont.begin(), base_cont.end(),  key_comp_type(), alloc);

		//base_map_type base_cont(iterator_trans(base_cont.begin()), iterator_trans(base_cont.end()), key_comp_type(), alloc);
		base_map_type base_cont2(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
		map_type cont(boost::move(base_cont));
		yggr_test_assert(cont.size() == 6);
	}

	//{
	//	base_map_type base_cont(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
	...
	//{
	//	base_map_type base_cont13(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
...
void test_capacity(SegCont& seg_cont)
{
	...
	map_type cont1(base_cont.begin(), base_cont.end(), alloc);
	base_map_type base_cont1(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
...
void test_modifiers(SegCont& seg_cont)
{
	...
	typedef typename map_type::key_compare key_comp_type;
	typedef typename map_type::allocator_type alloc_type;
	typedef typename map_type::options_type options_type;
	typedef typename map_type::iterator iter_type;
	typedef typename map_type::const_iterator citer_type;
	typedef typename map_type::value_type val_type;
	...
	{
		cont.insert(100, 100);
		base_cont.insert(val_type(100, 100));

		int kn = 90, n = 90;
		cont.insert(kn, boost::move(n));
		base_cont.insert(val_type(kn, 90));
		...
		base_cont.insert(val_type(200, 200));

		int km = 300, m = 300;
		cont.insert(km, boost::move(m), boost::bind(&check_last_element_pair<map_type>, _1, _2, 300));
		base_cont.insert(val_type(km, 300));

		yggr_test_assert(cont.size() == base_cont.size());
	}

	{
		cont.insert(val_type(1000, 1000));
		base_cont.insert(val_type(1000, 1000));

		int kn = 900, n = 900;
		val_type v(kn, n);
		cont.insert(boost::move(v));
		base_cont.insert(val_type(kn, 900));

		cont.using_handler(boost::bind(&check_range_equal<map_type>, _1, boost::cref(base_cont)));

		cont.insert(val_type(2000, 2000), boost::bind(&check_last_element_pair<map_type>, _1, _2, 2000));
		base_cont.insert(val_type(2000, 2000));

		int km = 3000, m = 3000;
		val_type v2(km, m);
		cont.insert(boost::move(v2), boost::bind(&check_last_element_pair<map_type>, _1, _2, 3000));
		base_cont.insert(val_type(km, 3000));
	...
	cont.set_value(val_type(200, 200));
	cont.set_value(val_type(9000, 9000));
	base_cont.insert(val_type(9000, 9000));
	...
	yggr_test_assert(cont.replace(val_type(100, 100)));
	...
	// merge
	{
		...
		// map
		{
			typedef boost::container::map<int, int, key_comp_type, alloc_type> now_map_type;

			map_type cont_eins(key_comp_type(), alloc);
			{
			...
			now_map_type cont_zwei(key_comp_type(), alloc);
			{
		...
		{
			typedef yggr::map<int, int, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::options_map<int, int, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::safe_container::safe_map<int, int, mutex_type, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::safe_container::safe_options_map<int, int, mutex_type, key_comp_type, alloc_type> now_map_type;
		...
		// multiset
		{
			typedef boost::container::multimap<int, int, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::multimap<int, int, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::options_multimap<int, int, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::safe_container::safe_multimap<int, int, mutex_type, key_comp_type, alloc_type> now_map_type;
		...
		{
			typedef yggr::safe_container::safe_options_multimap<int, int, mutex_type, key_comp_type, alloc_type> now_map_type;
...
#	if 1 
		...
		yggr_test_assert(ins_cont.find(3));
#	endif // !defined(YGGR_NOT_USE_SAFE_CONTAINER_OPTIMIZATION)
	}
...
void test_swap(SegCont& seg_cont)
{
	...
	base_map_type init_cont1(key_comp_type(), alloc);
	init_cont1.insert(val_type(1, 1));
	init_cont1.insert(val_type(2, 2));
	init_cont1.insert(val_type(3, 3));
	init_cont1.insert(val_type(4, 4));
	init_cont1.insert(val_type(5, 5));

	base_map_type init_cont2(key_comp_type(), alloc);
	init_cont2.insert(val_type(6, 6));
	init_cont2.insert(val_type(7, 7));
	init_cont2.insert(val_type(8, 8));
	init_cont2.insert(val_type(9, 9));
	init_cont2.insert(val_type(0, 0));

	map_type cont1(init_cont1.begin(), init_cont1.end(), alloc);
	map_type cont2(init_cont2.begin(), init_cont2.end(), alloc);

	base_map_type base_cont1(init_cont1.begin(), init_cont1.end(), key_comp_type(), alloc);
	base_map_type base_cont2(init_cont2.begin(), init_cont2.end(), key_comp_type(), alloc);
	yggr_test_assert(cont1.size() == base_cont1.size());
	yggr_test_assert(cont1.size() == base_cont2.size());


	base_map_type base_cont3(base_cont1, alloc);
	base_map_type base_cont4(base_cont2, alloc);
...
void test_using_handler(SegCont& seg_cont)
{
	...
	init_cont2.insert(val_type(8, 8));
	init_cont2.insert(val_type(9, 9));
	init_cont2.insert(val_type(10, 10));
...
int main(int argc, char* argv[])
{
	...
	{
		typedef yggr::safe_container::safe_options_map<int, int>::value_type map_value_type;
		...
		typedef 
			yggr::safe_container::safe_options_map
			<
				int, int,
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_map",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>, 
									"test_managed_shared_memory_safe_map") );
