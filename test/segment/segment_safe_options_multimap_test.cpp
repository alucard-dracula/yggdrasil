//segment_safe_options_multimap_test.cpp
...
void test_constructor(SegCont& seg_cont)
{
	...
	{
		base_map_type arg_base_cont(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
		map_type cont(boost::move(arg_base_cont));
		yggr_test_assert(cont.size() == 6);
	}

	{
		base_map_type arg_base_cont(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
		map_type cont(arg_base_cont);
	...
	{
		base_map_type arg_base_cont(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
		map_type cont(boost::move(arg_base_cont), arg_base_cont.get_allocator());
		yggr_test_assert(cont.size() == 6);
	}

	{
		base_map_type arg_base_cont(base_cont.begin(), base_cont.end(), key_comp_type(), alloc);
		map_type cont(arg_base_cont, arg_base_cont.get_allocator());
		yggr_test_assert(cont.size() == 6 && base_cont.size() == 6);
		cont.using_handler(boost::bind(&check_range_equal<map_type>, _1, boost::cref(base_cont)));
	}
...
void test_modifiers(SegCont& seg_cont)
{
...

	{
		...
		cont.insert(200, 200, boost::bind(&check_last_element<map_type>, _1, _2, 200));
		base_cont.insert(val_type(200, 200));

		int km = 300, m = 300;
		cont.insert(km, boost::move(m), boost::bind(&check_last_element<map_type>, _1, _2,  300));
...

	{
		...
		cont.insert(val_type(2000, 2000), boost::bind(&check_last_element<map_type>, _1, _2,  2000));
		base_cont.insert(val_type(2000, 2000));

		int km = 3000, m = 3000;
		val_type v2(km, m);
		cont.insert(boost::move(v2), boost::bind(&check_last_element<map_type>, _1, _2,  3000));
...

	{
		...
		yggr_test_assert(ins_cont.emplace_hint(
				boost::bind(static_cast<begin_end_foo_type>(&base_map_type::begin), _1), yggr::non_handler));
		yggr_test_assert(ins_cont.size() == 1);
		yggr_test_assert(ins_cont.find(0));
	}

	{
		...
		yggr_test_assert(ins_cont.emplace_hint(
					boost::bind(
						static_cast<begin_end_foo_type>(&base_map_type::begin), _1), yggr::non_handler, 
						std::make_pair(10, 10)));
		yggr_test_assert(ins_cont.size() == 1);
		yggr_test_assert(ins_cont.find(10));
	}
...
typename 
	boost::enable_if
	<
		boost::is_base_of<yggr::mplex::null_type, typename Cont::node_type>,
		bool
	>::type
...
int main(int argc, char* argv[])
{
	...
	{
		typedef yggr::safe_container::safe_options_multimap<int, int>::value_type map_value_type;
		...
		typedef 
			yggr::safe_container::safe_options_multimap
			<
				int, int,
		...
		seg_cont_type seg_cont("test_managed_shared_memory_safe_multimap",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>, 
									"test_managed_shared_memory_safe_multimap") );
	...
	// test_mamaged_mapped_file
	{
		...
		seg_cont_type seg_cont("../test_out/segment/test_mamaged_mapped_file_safe_multimap",
								yggr::segment::segment_op::create_only(), 
								65536,
								boost::bind(
									&delete_segment<seg_type>,
									"../test_out/segment/test_mamaged_mapped_file_safe_multimap"));
