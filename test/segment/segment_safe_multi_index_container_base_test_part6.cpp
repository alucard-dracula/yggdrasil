...
void test_modifiers_of_random_access_detail(SegCont& seg_cont)
{
	...
	{
		static 
		std::pair<typename safe_index0_type::iterator, typename safe_index0_type::iterator>
			iter_range(typename safe_index0_type::base_type& base)
		{
			return std::make_pair(base.begin(), base.end());
		}
		...
		static 
		std::pair<typename safe_index0_type::iterator, typename safe_index0_type::iterator>
			iter_range_1(typename safe_index0_type::base_type& base)
		{
			yggr_test_assert(base.size() > 1);
			typename safe_index0_type::iterator iter = base.begin();
			return std::make_pair(++iter, base.end());
		}

		static 
		typename safe_index0_type::iterator
			iter_pos_1(typename safe_index0_type::base_type& base)
		{
			yggr_test_assert(base.size() > 1);
			typename safe_index0_type::iterator iter = base.begin();
			return ++iter;
	...
	yggr_test_assert(cont.template get<1>().erase(
				boost::bind(static_cast<begin_end_foo_unsafe_random_access_1_type>(&unsafe_random_access_1_type::begin), _1), 
	...
	struct view_foo_t
	{
		...
		static
		typename safe_index1_type::iterator rearrange_reverse_1(typename mi_cont_type::base_type& base)
		{
		...
		static
		typename safe_index0_type::iterator non_rearrange_0(typename mi_cont_type::base_type& base)
		{
			return base.template get<0>().begin();
		}

		static
		typename aref_vt_type::iterator rearrange_vt(typename mi_cont_type::base_type& base, aref_vt_type& aref_vt)
		{
			for(typename safe_index0_type::iterator i = base.template get<0>().begin(), isize = base.template get<0>().end();
				i != isize; ++i)
			{
