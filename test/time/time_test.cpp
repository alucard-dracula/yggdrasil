...
void test_string_time_formatting_combe(void)
{
	...
	// string version
	{
...

		{
			...
			yggr_test_assert(str == yggr::charset::utf8_string("1970-01-01T08:00:00.000001中国夏令时"));
...
void test5(void)
{
	...
	// stream version
	{
...

		{
			...
			std::cout << ss.str() << std::endl;
#if defined(YGGR_AT_WINDOWS)
			yggr_test_assert(ss.str() == yggr::charset::utf8_string("1970-01-01T08:00:00.000001中国夏令时"));
#elif defined(YGGR_AT_LINUX)
...
#else
			yggr::time::time_formatting_local(ss, L"%Y-%m-%dT%H:%M:%S%F", tm); // mingw using %Z 
