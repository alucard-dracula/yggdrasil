...
#	endif // (YGGR_SYSTEM_64)
...

	{
		...
		yggr_test_assert(std::equal(cont.begin(), cont.end(), chk));
	}
...
#	endif //(YGGR_SYSTEM_64)
...

	{
		...
		yggr_test_assert(std::equal(cont.begin(), cont.end(), arr));
	}
...
void test_element_access(void)
{
...

	{
		...
		yggr_test_assert(cont.wraped_front() == 1);
...
void test_modify(void)
{
...

	{
		...
		yggr_test_assert(std::equal(yggr::advance_copy(cont.begin(), tmp_cont.size() * 1), cont.end(), tmp_cont.begin()));

		cont.insert(cont.end(), tmp_cont.wraped_begin(), tmp_cont.wraped_end());
		yggr_test_assert(std::equal(yggr::advance_copy(cont.begin(), tmp_cont.size() * 2), cont.end(), tmp_cont.begin()));

		cont.insert(cont.end(), tmp_cont.cbegin(), tmp_cont.cend());
		yggr_test_assert(std::equal(yggr::advance_copy(cont.begin(), tmp_cont.size() * 3), cont.end(), tmp_cont.begin()));

		cont.insert(cont.end(), tmp_cont.wraped_cbegin(), tmp_cont.wraped_cend());
		yggr_test_assert(std::equal(yggr::advance_copy(cont.begin(), tmp_cont.size() * 4), cont.end(), tmp_cont.begin()));
	...
	{
		wrap_cont_type cont;
		cont.push_front(boost::move(arr[0]));
		yggr_test_assert(cont.front() == arr[0]);

		cont.push_front(10);
		yggr_test_assert(cont.front() == 10);

		cont.emplace_front();
		yggr_test_assert(cont.front() == 0);

		cont.emplace_front(boost::move(arr[1]));
		yggr_test_assert(cont.front() == arr[1]);
...
void test_operations(void)
{
	...
	// merge
	{
		wrap_cont_type cont(boost::begin(arr), boost::end(arr));
		cont_type cont2(cont.begin(), cont.end());

		cont.merge(boost::move(cont2));
...

	{
		...
		cont.merge(boost::move(cont2), std::less<value_type>());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}

	{
		...
		cont.merge(cont2, std::less<value_type>());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}
	...
	{
		wrap_cont_type cont(boost::begin(arr), boost::end(arr));
		wrap_cont_type cont2(cont.begin(), cont.end());
	...
	// splice
	{
		...
		cont.splice(cont.end(), boost::move(cont2));
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}
...

	{
		...
		cont.splice(cont.end(), boost::move(cont2), cont2.begin());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 6);
...

	{
		...
		cont.splice(cont.end(), cont2, cont2.begin());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 6);
		yggr_test_assert(cont2.size() == 4);
	}

	{
		...
		cont.splice(cont.end(), boost::move(cont2), cont2.begin(), cont2.end());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}

	{
		...
		cont.splice(cont.end(), cont2, cont2.begin(), cont2.end());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}
...

	{
		...
		cont.splice(cont.end(), boost::move(cont2), cont2.wraped_begin());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 6);
		yggr_test_assert(cont2.size() == 4);
	}

	{
		...
		cont.splice(cont.end(), cont2, cont2.wraped_begin());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 6);
		yggr_test_assert(cont2.size() == 4);
	}

	{
		...
		cont.splice(cont.end(), boost::move(cont2), cont2.wraped_begin(), cont2.wraped_end());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}

	{
		...
		cont.splice(cont.end(), cont2, cont2.wraped_begin(), cont2.wraped_end());
		std::cout << cont << std::endl;
		yggr_test_assert(cont.size() == 10);
		yggr_test_assert(cont2.size() == 0);
	}
	...
	// remove_if
	{
		...
		std::size_t n = cont.remove_if(boost::bind(std::less<value_type>(), _1, 3));
	...
	// reverse
	{
		...
		std::cout << cont << std::endl;
		yggr_test_assert(std::equal(boost::begin(arr), boost::end(arr), cont.rbegin()));
	}
...

	{
		...
		std::size_t n = cont.unique(std::equal_to<value_type>());
...

	{
		...
		std::size_t n = cont.unique(std::not_equal_to<value_type>());
...

	{
		...
		std::size_t n = cont.unique(std::less_equal<value_type>());
...

	{
		...
		std::cout << cont << std::endl;
		yggr_test_assert(std::equal(boost::begin(arr), boost::end(arr), cont.begin()));

		cont.sort(std::greater<value_type>());
	...
	std::cout << "------test_operations [" << typeid(wrap_cont_type).name() << "] end------" << std::endl;
}
...
{
	test_construct<yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_construct<yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_operator_set< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_operator_set< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_swap< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_swap< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_assign< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_assign< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_element_access< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_element_access< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_iterators< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_iterators< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_capacity< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_capacity< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_modify< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_modify< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_operations< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_operations< yggr::lua_wrap::wrap_list<yggr::u64> >();

	test_operator_op< yggr::lua_wrap::wrap_list<yggr::s64> >();
	test_operator_op< yggr::lua_wrap::wrap_list<yggr::u64> >();
