...
#ifndef __BALANCE_ASYNC_TCP_SVR_CALCULATOR_HPP__
#define __BALANCE_ASYNC_TCP_SVR_CALCULATOR_HPP__
...
public:
	...
	void static print_socket_info(session_mgr_type::smap_type::base_type& base,
									session_mgr_type::smap_iter_type& iter);

	template<typename Runner, typename Handler>
	void cal_test_pak_type(const owner_info_type& owner, const test_pak_type& cdt, Runner* prunner, Handler handler)
	{
			...
			if(!(ptr->op_handler()
					.get_session_mgr()
						->access(owner.owner_id(), 
									boost::bind(&Calculator::print_socket_info, _1, _2))))
			{
...
#endif // __BALANCE_ASYNC_TCP_SVR_CALCULATOR_HPP__