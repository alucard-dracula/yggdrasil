...
#ifndef __UUID_STATIC_TASK_CENTER_BALANCE_ASYNC_TCP_SVR_CONFIG_HPP__
#define __UUID_STATIC_TASK_CENTER_BALANCE_ASYNC_TCP_SVR_CONFIG_HPP__
...
YGGR_PP_REGISTER_SESSION_ID_CREATOR_BEGIN(test_pak_type, test_pak, const yggr::ids::uuid& ) // 定义会话ID的创建器类型
	test_pak.id()
...
typedef yggr::ids::pointer_id_generator<yggr::ids::uuid> session_id_generator_type; // 定义会话ID生成器类型（该例题使用UUID作为会话ID）
...
typedef yggr::server::basic_svr_handler
		< 
			yggr::server::server_config::server_tcp_config_type,
...
#endif // __UUID_STATIC_TASK_CENTER_BALANCE_ASYNC_TCP_SVR_CONFIG_HPP__